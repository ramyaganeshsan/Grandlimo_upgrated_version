...
exports.get_latestapp_version = async function (q) {
  var deferred = q.defer();
  var collection = db.get().collection(t.MDB_SITEINFO);
    try {
    const results = await collection.find({}, { projection: { ios_app_version: 1 } }).toArray();
...
exports.get_passenger_details_by_phone = async function (q, phone) {
  var deferred = q.defer();
...
exports.driver_past_trips = async function (q, userid) {
  //console.log("heree");
  var deferred = q.defer();
...
exports.coupon_package_details = async function (q, package) {
  ...
  var collection = db.get().collection(t.MDB_COUPON_PACKAGE);
    try {
    const results = await collection
    .find({ _id: package }, { projection: { passenger_commission: 1, corporate_commission: 1, fixed_fare_apply: 1 } }).toArray();
          deferred.resolve(results);
