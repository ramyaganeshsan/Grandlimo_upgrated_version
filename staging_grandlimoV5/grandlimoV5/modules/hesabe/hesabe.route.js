...
const {
  ...
  insertErrorLog,
  insertLog,
  updatePassengerWalletAmount,
  getPassengerDetails,
} = require("./hesabe.model");
const axios = require("axios");

const { cancelCard, decrypteData, encrypteData } = require("./hesabe");
...
//ramya may 15 2026
hesabeRoutes.get("/getSavedCards", async (req, res) => {
  try {
    ...
    // encrypt payload
    const encryptedData = encrypteData(payload);

    console.error("encryptedData :", encryptedData);

    // API call
    const response = await axios.get(
      `https://sandbox.hesabe.com/api/direct-payment/cards?data=${encryptedData}`,
      {
        headers: {
          accessCode,
    ...
    console.error("hesabe response :", response.data);

    if (!response || !response.data) {
      return res.send({
        status: -1,
        message: "Invalid response from Hesabe",
      });
    }

    // decrypt response
    const decrypted = decrypteData(response.data);

    console.error("decrypted response :", decrypted);
    ...
    return res.send({
      ...
      cards: decrypted.response.cards || [],
    });
  } catch (err) {
...
//ramya may 15 2026
hesabeRoutes.post("/deleteSavedCard", async (req, res) => {
  try {
    ...
    const response = await axios.delete(
      `https://sandbox.hesabe.com/api/direct-payment/cards/${cardId}`,
      {
        headers: {
          accessCode,
          Accept: "application/json",
    ...
    console.error("decrypted response :", JSON.stringify(decrypted, null, 2));

    if (!decrypted || !decrypted.status) {
      return res.send({
        status: -1,
        message: decrypted.message || "Failed to delete saved card",
      });
    }

    return res.send({
      status: 1,
      message: decrypted.message || "Card removed successfully",
    });
  } catch (err) {
...
//ramya may 15 2026
hesabeRoutes.get("/saveCardSuccess", async (req, res) => {
  try {
    console.error("calling success redirect url save card.");
    console.error("response : ", req.query.data);
    ...
    let decrypted = decrypteData(data);
    ...
    if (!decrypted) {
      return res.send({
        status: -1,
        message: "Invalid transaction. Please contact support.",
      });
    }
    ...
    console.error("saveCardSuccess Error :", err);
...
//ramya may 15 2026
hesabeRoutes.get("/saveCardFailed", async (req, res) => {});

hesabeRoutes.get("/success", async (req, res) => {
  // return res.status(200).send("success");
  console.error("calling success rediect url.");
  ...
  let response = decrypted.response;
  let { paymentToken, variable1, variable2, paymentId, amount, resultCode } =
    response;
  let passengerId = variable2;

  if (variable1 === "WALLET" && resultCode === "CAPTURED") {
    ...
    var token = chargeDetails.token;
    let status = decrypted.status;
    let metadata = transactionDetails;

    if (status && response.resultCode !== "FAILED") {
      ...
      var message = "Failed to process your payment, please try again.";
      res.type("text/json");
      return res.send(message);
    }
  }
...
hesabeRoutes.get("/failed", async (req, res) => {
  // console.error(decrypteData("e38af954807ef9bc59ada896e30eaf973c729ee41e75b900f1bee338786362229bb5519af501455a43852083fdf6cb3301f3482b5b9ac2f371be054488686f3a"));
...
hesabeRoutes.post("/success", async (req, res) => {
  console.error("calling post hesabe success redirect url");
  if (!req.query.data) return res.send("failed");
  ...
  let resp = {
    status: 0,
    message: "Invalid transaction. Please contact our support team.",
  };
  if (!decrypted) {
  ...
  if (variable1 === "WALLET" && resultCode === "CAPTURED") {
    ...
    let transactionDetails = variable2;
    var hostnameRedirect = req.headers.host;
    var protocol = req.headers.protocol;
    if (protocol == undefined) protocol = "http://";
    ...
    console.error(status, response.resultCode);
    ...
    if (status && response.resultCode !== "FAILED") {
      ...
      try {
        ...
        console.error(metadata.tripId, typeof metadata.tripId);
        await apimodel
          .get_log_temp_tap_payment_gateway(q, metadata.tripId)
          .then(function (logtempresults) {
            console.error(logtempresults);
  ...
  // resp = {
  //   status: 0,
  //   message: "Failed to process your response. please contact our team.",
  // };
  // return res.send(resp);
});

hesabeRoutes.post("/failed", async (req, res) => {
  if (!req.query.data) return res.send("failed");
