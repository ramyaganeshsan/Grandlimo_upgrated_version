var dispatchermodel = require('../../models/dispatchermodel');
...
console.log("GetBookingList");
		...
		if(tripresults.length > 0)
		{
            ...
            forEach(tripresults, function(element, index) 
            {
                ...
                /* Sasi */
                var discount_wallet_amount_used  = element.discount_wallet_amount_used;
                walletAmountUsed += discount_wallet_amount_used;
...
console.log('approxDuration.......',approxDuration);
                    if(travelStatus == 2 && approxDuration != 0)
                    {
                       ...
                       /* if(estimateTimeRemaining == 0 || (estimateTimeRemaining > 280 && estimateTimeRemaining < 315) || (estimateTimeRemaining > 580 && estimateTimeRemaining < 615) || (estimateTimeRemaining > 880 && estimateTimeRemaining < 915))
                        {
                            try
                            {
                                common.ApproximateDurationLatLang(q,driverId,tripId,dropLatitude,dropLongitude).then(function(approxDurationResult){
                        ...
                        {
                            ////console.log("estin001");
                          estimateTimeRemaining = "<span class='pending_payment_class'>"+convsecstoformat(estimateTimeRemaining)+"</span>";

                        }
                        else
                        {
                            ////console.log("estin003");
                            try
                            {
                                common.ApproximateDurationLatLang(q,driverId,tripId,dropLatitude,dropLongitude).then(function(approxDurationResult){

                                    estimateTime = currentTime.getTime()-currentTime.getTime();
                                    estimateTime = estimateTime/1000;
                                    ////console.log('approxDurationResult',approxDurationResult);
                ...
                notesImg = '<a href="javascript:void(0)" style="color:#000;text-decoration:none;cursor:default;" title="'+ +'"><img src="'+base_url+'/public/images/notes.jpg"></a>';
    ...
    //             tripObj[i].booking_time = created_date.format('d-M-Y h:i:s A');
            ...
            catch(err)
            {
                ...
                if (travelStatus == 1) {
                   tripObjHtml += '<td width="5%" style="color:blue;text-decoration:none;"><a style="text-decoration:none;" target="_blank" href="'+base_url+'/transaction/transaction_details/' +tripId+'">'+tripId+'</a></td>';
                } else {
                    tripObjHtml += '<td width="5%" style="color:blue;">'+tripId+'</td>';
                }
            	...
            	tripObjHtml += '<td width="7%">No Driver</td>';
