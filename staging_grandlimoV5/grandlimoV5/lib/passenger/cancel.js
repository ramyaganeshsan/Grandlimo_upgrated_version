	...
	else
	{
		...
		apimodel.get_trip_detail(q,trip_id).then(function(tripresults){
    		...
    		if(tripresults.length > 0)
    		{
    			...
    			if(travel_status != 9 && travel_status != 0 && travel_status != 7 && travel_status != 3 && travel_status != 6)
    			{
    	...
    	// 		}
    			else
    			{
	    			if(cancellation_fare == 0 || travel_status == 0)
	    			{
	    				...
	    				apimodel.update_driver_status(q,driver_id,update_driver_array).then(function(updateresults){
	    										//console.log("cancel00005");
	    					apimodel.update_request_details(q,trip_id,update_request_array).then(function(updateresults){
	    						    			//console.log("cancel00006");
	    						apimodel.update_log_details(q,trip_id,update_logs_array).then(function(updateresults){
	    							    		//console.log("cancel00007");
	    							common.update_cancel_trip_det(q,trip_id).then(function(updateresults){
	    								    	//console.log("cancel00008");
										common.send_sms(q,passenger_phone,10,replace_array).then(function(checkresults){
											...
											try
											{
												...
												var socket_msg = {
														message:i18n.__('passenger_trip_cancelled'),
											...
											message.cancellation_from = req.__('free_label');
					...
					{
						apimodel.get_cancellation_fare(q,trip_id).then(function(fareresults){
							...
							if(fareresults.length > 0)
							{
								...
								if(pay_mod_id == 3 && passenger_wallet_amount > cancellation_amount)
								{
				    					...
				    					apimodel.update_request_details(q,trip_id,update_request_array).then(function(updateresults){
				    						...
				    						apimodel.update_log_details(q,trip_id,update_logs_array).then(function(updateresults){
				    							...
				    							common.update_cancel_trip_det(q,trip_id).then(function(updateresults){
				    								...
				    								apimodel.get_auto_id(q,t.MDB_TRANS).then(function(autoresults){
				    									...
				    									if(autoresults.length > 0)
				    									{
															...
															"admin_amount" : 0,
															"company_amount" : cancellation_amount,
															"trans_packtype" : 'N',
															...
															apimodel.insert_passenger_details(q,insert_array).then(function(insertresults){
																...
																common.send_sms(q,passenger_phone,10,replace_array).then(function(checkresults){
																	...
																	message.message = req.__('trip_cancelled_passenger');
																	message.cancellation_from = req.__('wallet_label');
																	message.cancellation_amount = cancellation_amount;
																	message.status = 1;
																	deferred.resolve(message);
																	deferred.makeNodeResolver()
																	message=null;
