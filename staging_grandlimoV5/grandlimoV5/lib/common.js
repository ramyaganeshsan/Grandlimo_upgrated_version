...
var passapimodel = require("../models/passapimodel");
var push = require("../lib/push/push");
    ...
    if (global.settings.sms_enable == 1) {
      if (detailsresults.length > 0) {
        try {
          ...
          if (replace_array.wallet_amt != undefined) {
            msg = msg.replace("##WALLET_AMT##", replace_array.wallet_amt);
            console.log("sms msg for pending payment", msg, id, to);
          }
  ...
  apimodel.knet_details(q).then(function (detailsresults) {
    if (detailsresults.length > 0) {
      ...
      try {
        ...
        success_url = protocol + hostname + "/knet/wallet_response";
        failure_url = protocol + hostname + "/knet/wallet_response";
        ...
        var success_url = (failure_url =
          protocol + hostname + "/knet/wallet_response");
...
exports.hesabe_pay = function (
  ...
  apimodel.knet_details(q).then(function (detailsresults) {
    if (detailsresults.length > 0) {
      ...
      try {
        amount = amount.toFixed(3);
        success_url = protocol + hostname + "/knet/payment_response";
        failure_url = protocol + hostname + "/knet/payment_response";
        ...
        var success_url = (failure_url =
          protocol + hostname + "/knet/payment_response");
            ...
            markers: [
              {
                ...
                //icon: 'http://www.q8grandlimo.com/public/images/marker-start.png'
                label: "A",
                color: "red",
              },
              {
                location: drop_location,
                //icon: 'http://www.q8grandlimo.com/public/images/marker-end.png'
                label: "B",
                color: "green",
              },
            ],
            ...
            path: [
              {
                color: "0x1f1e1e",
                weight: "5",
                points: location_array,
              },
            ],
          };
          //gmAPI.staticMap(params); // return static map URL
          gmAPI.staticMap(params, function (err, binaryImage) {
            //console.log(Buffer.from(binaryImage));
...
exports.conversation_details = function (conversationId) {
  var conversationsArray = global.settings["conversation"];

  var suggestionForConversation = "";
  for (var i = 0; i < conversationsArray.length; i++) {
    var conversationItem = conversationsArray[i];

    if (conversationItem.value == conversationId) {
      suggestionForConversation = conversationItem.key;
      break;
    }
  }
  return suggestionForConversation;
};

// Ramya 24_nov_2023

exports.door_assistance_details = function (door_assistanceId) {
  var doorAssistArray = global.settings["door_assistance"];

  var suggestionForDoorAssist = "";
  for (var i = 0; i < doorAssistArray.length; i++) {
    var doorAssistItem = doorAssistArray[i];

    if (doorAssistItem.value == door_assistanceId) {
      suggestionForDoorAssist = doorAssistItem.key;
      break;
    }
  }

  return suggestionForDoorAssist;
};
