...
// exports.send_push_notification_ios = async (body, query, req, res) => {
...
//   let pushmessage = {};
//   try {
//     pushmessage = JSON.parse(query.pushmessage || "{}");
//   } catch (e) {
...
exports.send_push_notification_ios = async function () {
  console.error("calling node api from php");

  const req = Array.from(arguments).find(
    (arg) => arg && arg.query && arg.originalUrl
  );

  if (!req) {
    console.error("❌ Express req not found");
  ...
  if (!deviceToken) {
    console.error("❌ deviceToken missing");
    return { success: false, error: "deviceToken missing" };
  }

  try {
    await helper(deviceToken, pushmessage);
    console.error("✅ APN sent");
  } catch (err) {
    console.error("❌ APN error", err);
    return { success: false, error: "APN failed" };
  }
        ...
        apimodel.company_model_details(q).then(function (modelresults) {
          if (modelresults.length > 0) {
            ...
            modelresults.forEach(function (val) {
              ...
              modelresults[i].model_fare_image = model_image_exists(
                hostname,
        ...
        apimodel.driver_login(q, inputParams).then(function (results) {
          if (results.length > 0) {
            ...
            if (user_status == "D") {
              ...
              apimodel
                .update_driver_phone(q, update_array, userid)
                .then(function (updatedriver) {
                  //if(updatedriver.length > 0 )
                  //{
                  let details = {};
                  apimodel
                    .driver_profile(q, userid)
                      ...
                      apimodel
                        ...
                        .then(function (statisticsresults) {
                          ...
                          if (statisticsresults.length > 0) {
                            ...
                            message.status = 1;
                            message.q8taxi_enable = parseInt(
                              config.Q8TAXI_ENABLE
                            );
                            deferred.resolve(message);
                            deferred.makeNodeResolver();
                            message = null;
                        ...
                        .then(function (statisticsresults) {
                          ...
                          if (statisticsresults.length > 0) {
                            ...
                            try {
                              ...
                              var time_driver_array =
                                statisticsresults[0].waiting_time;
                              ...
                              time_driver_array.forEach(function (val) {
                                if (val != "") {
                                  ...
                                  if (
                                    waitingtime[0] != "" &&
                                    waitingtime[0] != undefined
                                  ) {
                                    total_waiting_time += parseInt(
                                      waitingtime[0]
                                    );
                                  }
                            ...
                            details.statistics.waiting_time =
                              total_waiting_time;
                            details.statistics.waiting_time =
                              convsecstoformat(total_waiting_time);
                            details.statistics.today_earnings =
                              statisticsresults[0].today_earnings.toFixed(2);
                            details.statistics.time_driven = convsecstoformat(
                              statisticsresults[0].time_driven
                            );
  ...
  if (validate_error != undefined) {
    ...
    apimodel.check_qr_scan(q, code).then(function (checkresults) {
      if (checkresults.length > 0) {
        ...
        apimodel
          .tabTokenUpdate(q, taxi_id, tab_device_token)
      ...
      if (checkresults.length > 0) {
        ...
        if (login_status == "S") {
          apimodel.driver_profile(q, userid).then(function (profileresults) {
            if (profileresults.length > 0) {
              apimodel
                .get_driver_ratings(q, userid)
                .then(function (ratingresults) {
                  var avg_rating = 0;

                  if (ratingresults.length > 0) {
                    var avg_rating =
                      ratingresults[0].total_rating / ratingresults[0].count;
  ...
  if (validate_error != undefined) {
    ...
    apimodel.get_driver_status(q, userid).then(function (checkresults) {
      if (checkresults.length > 0) {
        ...
        if (login_status == "S") {
          apimodel
            .driver_taxi_assign(q, userid, time_range)
            .then(function (assignresults) {
              if (assignresults.length > 0) {
                ...
                if (shiftstatus == "IN") {
                  //Shift In Update
                  apimodel
                    .update_driver_shift(q, updateShiftArray, userid)
                    .then(function (assignresults) {
                      apimodel
                        .get_auto_id(q, t.MDB_SHIFT_HISTORY)
                        .then(function (autoidresults) {
                          if (autoidresults.length > 0) {
                            ...
                            apimodel
                              .ifTaxiReachService(q, taxi_start_km)
                              .then(function (taxiserviceresults) {
                  ...
                  apimodel
                    ...
                    .then(function (pendingresults) {
                      if (pendingresults.length == 0) {
                        ...
                        apimodel
                          .update_driver_shift(q, updateShiftArray)
                          .then(function (assignresults) {
                            let insertArray = {
                              shift_end: new Date(),
                              status: "Shift OUT",
  ...
  if (validate_error != undefined) {
    ...
    apimodel.get_driver_status(q, userid).then(function (checkresults) {
      if (checkresults.length > 0) {
        ...
        apimodel
          .check_taxi_assign(q, userid)
          .then(function (taxiassignresults) {
            apimodel
              .check_driver_assign(q, taxi_id)
              .then(function (driverassignresults) {
                //console.log('taxi',taxiassignresults.length);
                //console.log('driver',driverassignresults.length);

                if (
                  taxiassignresults.length > 0 &&
                  driverassignresults.length > 0
                ) {
                  ...
                  apimodel
                    .ifTaxiReachService(q, start_km)
                    .then(function (taxiserviceresults) {
                      apimodel
                        .get_auto_id(q, t.MDB_TAXIMAPPING)
                        .then(function (autoidresults) {
                          apimodel
                            .get_auto_id(q, t.MDB_SHIFT_HISTORY)
                            .then(function (shiftautoidresults) {
                              if (autoidresults.length > 0) {
                                if (shiftautoidresults.length > 0) {
                                  var mapping_last_id =
                                    autoidresults[0]._id + 1;
                                  var shift_last_id =
                                    shiftautoidresults[0]._id + 1;
                                  var km_reach_status = 0;
                                  var taxi_service_id = 0;
                                  if (taxiserviceresults.length > 0) {
                                    km_reach_status = 1;
                                  ...
                                  apimodel
                                    .insert_mapping_taxi(q, taxi_insert_array)
                                    .then(function (insertresults) {
                                      apimodel
                                        .insert_shift_history(q, insertArray)
                                        .then(function (insertresults) {
                                          apimodel
                                            ...
                                            .then(function (profileresults) {
                                              if (profileresults.length > 0) {
                                                var profile_details = {};
                                                profile_details.driver_details =
                                                  profileresults[0];
                                                ...
                                                apimodel
                                                  ...
                                                  .then(function (
                                                    ...
                                                    if (
                                                      ...
                                                      profile_details.statistics.waiting_time =
                                                        total_waiting_time;
                                                      profile_details.statistics.waiting_time =
                                                        convsecstoformat(
                                                          total_waiting_time
                                                        );
                                                      profile_details.statistics.today_earnings =
                                                        statisticsresults[0].today_earnings.toFixed(
                                                          2
                                                        );
                                                      profile_details.statistics.time_driven =
                                                        convsecstoformat(
                                                          statisticsresults[0]
                                                            .time_driven
                                                        );
                                                      ...
                                                      profile_details.statistics.shift_status =
                                                        "IN";
                                                      profile_details.statistics.time_driven =
                                                        "00:00";
                                                      profile_details.statistics.waiting_time =
                  ...
                  if (statisticsresults.length > 0) {
                    ...
                    try {
                      ...
                      statistics.today_earnings =
                        statisticsresults[0].today_earnings.toFixed(2);
                      statistics.time_driven = convsecstoformat(
                        statisticsresults[0].time_driven
                      );
                    } catch (err) {
                      console.log("error", err);
            ...
            fs.writeFile(
              ...
              function (err) {
                ...
                if (err != "" && err != undefined) {
                  ...
                  apimodel
                    .update_driver_profile(q, updateArray, userid)
                    .then(function (drivercheckresults) {
                      apimodel
                        .update_taxi_km(q, taxi_km, userid)
                        .then(function (kmcheckresults) {
          ...
          fs.writeFile(
            ...
            function (err) {
              ...
              if (err != "" && err != undefined) {
                ...
                apimodel
                  .update_passenger_profile(q, updateArray, userid)
                  .then(function (drivercheckresults) {
