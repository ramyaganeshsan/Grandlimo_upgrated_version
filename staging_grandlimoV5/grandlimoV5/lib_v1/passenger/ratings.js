...
exports.update_ratings_comments= function(q,req){
	...
	else
	{
		...
		let userComments = inputParams.comments

		if(userComments){
			userComments = userComments.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '');
		}
		var driver_tags = urlencode.decode(inputParams.driver_tags);
		var comments = urlencode.decode(userComments);

		apimodel.get_trip_detail(q,trip_id).then(function(trip_results){

			apimodel.get_trip_detail_complete_table(q,trip_id).then(function(trip_results2){

			if(trip_results.length > 0 || trip_results2.length > 0)
			{	
				try
				{
				if(driver_tags != '')
