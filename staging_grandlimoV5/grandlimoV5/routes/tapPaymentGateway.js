...
module.exports = function (app) {
  router.post("/wallet", async function (req, res) {
    console.error("yes calling here !!!!!!!!!!1");

    let chargeDetails = req.body;
    console.error("chargeDetails : ", chargeDetails);

    if (
      !chargeDetails ||
      Object.keys(chargeDetails).length <= 0 ||
      !chargeDetails.hasOwnProperty("response") ||
      !chargeDetails.hasOwnProperty("metadata") ||
      !chargeDetails.hasOwnProperty("reference")
    ) {
    ...
    let paymentToken = chargeDetails._id;
    ...
    var sourceObj = chargeDetails.source ? chargeDetails.source : {};
    var cardObj = chargeDetails.card ? chargeDetails.card : {};
      ...
      apimodel
        ...
        .then(function (walletLog) {
          ...
          if (walletLog.length > 0 && existingPaymentToken != "") {
            var message = "Transaction already completed.";
            res.type("text/json");
            return res.send(message);
          } else {
            update_wallet_logs(
  ...
  //   // Get host/protocol if needed
  //   var hostname = req.headers.host;
  //   var protocol = req.headers.protocol || "http://";
  ...
  //   try {
  //     const { passenger_id, amt, tapTransactionId } = inputParams;
  ...
  //         message.detail = {};
  //         message.status = 0;
  //         return res.json(message); // ✅ RESPOND HERE
  //       }

  //       try {
  //         let walletUpdateResult = await update_wallet_logs(
  //           q,
  //           parseInt(passenger_id),
  //           parseFloat(amt),
  //           0,
  ...
  //         console.error("walletUpdateResult : ", walletUpdateResult);

  //         if (walletUpdateResult.status === 1) {
  ...
  router.post("/walletInitiate", async function (req, res) {
    res.status(200).send("OK - webhook post");

    try {
      console.error(
        "=========================================================="
      );
      console.error("Webhook Payload:", req.body);

      const chargeDetails = req.body;

      // ✅ 1. Always acknowledge receipt to Tap

      // ✅ 2. Extract essential fields safely without optional chaining
      const chargeId = chargeDetails.id;
      const status = chargeDetails.status; // e.g., 'CAPTURED', 'FAILED', etc.
      const responseMessage =
        chargeDetails.response && chargeDetails.response.message
          ? chargeDetails.response.message
          : "UNKNOWN";
      const reference =
        chargeDetails.reference && chargeDetails.reference.track
          ? chargeDetails.reference.track
          : null;
      const metadata = chargeDetails.metadata ? chargeDetails.metadata : {};
      var sourceObj = chargeDetails.source ? chargeDetails.source : {};
      var cardObj = chargeDetails.card ? chargeDetails.card : {};

      console.log("Charge ID: " + chargeId + " | Status: " + status);

      // ✅ 3. Handle successful capture
      if (status === "CAPTURED" || status === "AUTHORIZED") {
        const passengerId = metadata.passengerId;
        const addAmount = metadata.amount
          ? metadata.amount
          : chargeDetails.amount;

        if (!passengerId) {
          console.error("⚠️ Missing passengerId in metadata!");
          return;
        }

        // Check for duplicate entry
        const existingLog = await apimodel.checkIfPaymentTokenExists(chargeId);
        if (existingLog && existingLog.length > 0) {
          console.log("Duplicate transaction, skipping update.");
          return;
        }
        ...
        console.log(
          "Wallet recharged successfully for Passenger ID: " + passengerId
        );
      } else if (
        status === "FAILED" ||
        status === "CANCELLED" ||
        ...
        console.error("Payment failed for Charge ID: " + chargeId);
        // Optional: update DB for failed transaction
      } else {
  ...
  router.post("/TripInitiate", async function (req, res) {
    try {
      console.error(
        "=========================================================="
      );
      ...
      console.error("Charge ID: " + chargeId + " | Status: " + status);

      // ✅ 3. Handle successful capture
      if (status === "CAPTURED" || status === "AUTHORIZED") {
        const passengerId = metadata.passengerId;
        const trip_id = metadata.trip_id;
        const addAmount = metadata.amount
          ? metadata.amount
          : chargeDetails.amount;
        ...
        var insert_array = {
          ...
          change_amount: parseFloat(addAmount),
  ...
  router.post("/walletInitiateSaveCard", async function (req, res) {
    try {
      console.log("==========================================================");
  ...
  router.post("/walletInitiateCompleteTrip", async function (req, res) {
    try {
      console.error(
        "=========================================================="
      );
  ...
  router.get("/success", function (req, res) {
    console.error("YES CALLING  /sucess");
  ...
  router.get("/walletInitiate", async function (req, res) {
    const tapId = req.query.tap_id; // capture tap_id from query
    console.error("Tap Payment ID:", tapId);
    ...
    console.error("Charge Details:", chargeData);

    if (chargeData.status === "CAPTURED" || chargeData.status === "SUCCESS") {
      // Payment success
      res.redirect("/processPayment/success");
    } else {
      // Payment failed or pending
      res.redirect("/processPayment/failed");
    }
  });

  function renderTapResultPage(status, message) {
    const safeMessage = (message || status).replace(/</g, "&lt;");
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
...
<body>
  <p>${safeMessage}</p>
  <script>
    // Mobile app deep link - change to your app's scheme
    setTimeout(function () {
      window.location.href = "grandlimo://payment/${status}?message=${encodeURIComponent(
      safeMessage
    )}";
    }, 500);
  </script>
  ...
  router.get("/walletInitiateRedirect", async function (req, res) {
    console.error("calling :  walletInitiateRedirect");
    console.error("@@@@@@@@@@@@@@@@@@@@@@@@@@");
    const tapId = req.query.tap_id;
    if (!tapId) {
      return res
        .status(200)
        .send(renderTapResultPage("failed", "Missing payment reference"));
    }
    try {
      const SECRET_KEY =
        global.settings.tapPaymentGatewayMode === "L"
      ...
      const message = (charge.response && charge.response.message) || status;
      console.error("Tap redirect:", tapId, status, message);
      const isSuccess = status === "CAPTURED" || status === "AUTHORIZED";
      // IMPORTANT: return 200 HTML directly - do NOT res.redirect()
      return res
        .status(200)
        .send(renderTapResultPage(isSuccess ? "success" : "failed", message));
    } catch (err) {
      console.error("Tap redirect error:", err.message);
      return res
        .status(200)
        .send(renderTapResultPage("failed", "Payment verification failed"));
    }
  });

  router.get("/TripInitiate", async function (req, res) {
    const tapId = req.query.tap_id; // capture tap_id from query
    console.error("Tap Payment ID:", tapId);
  ...
  router.get("/walletInitiateAuth", async function (req, res) {
    const tapId = req.query.tap_id;
    console.error("Tap Payment ID:", tapId);
    ...
    const response = await axios.get(
      `https://api.tap.company/v2/authorize/${tapId}`,
      { headers: HEADERS }
    );

    const authorizeData = response.data;
    console.error("Authorize Details:", authorizeData);
    console.error("____________________________________");
    console.error("authorizeData.status : ", authorizeData.status);

    if (
      authorizeData.status === "AUTHORIZED" ||
      authorizeData.status === "CAPTURED"
    ) {
  ...
  router.post("/trip", async function (req, res) {
    ...
    var token = chargeDetails._id;
    let response = chargeDetails.response;
    let status = response.message;
    let code = response.code;
    let metadata = chargeDetails.metadata;
    let paymentId = chargeDetails.reference.track;
    var sourceObj = chargeDetails.source ? chargeDetails.source : {};
    var cardObj = chargeDetails.card ? chargeDetails.card : {};
    ...
    if (status === "Captured" || status === "Authorized") {
      ...
      try {
        apimodel
          .get_log_temp_tap_payment_gateway(q, metadata.tripId)
          .then(function (logtempresults) {
            console.error("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
            console.error("logtempresults : ", logtempresults);
            console.error("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
            ...
            if (logtempresults.length > 0) {
              ...
              var passenger_pending_amt =
                logtempresults[0].passenger_pending_amt;
              ...
              var fare_calculation_type =
                logtempresults[0].fare_calculation_type;
...
function ValidateTripCompleteRecharge(q, input) {
  var constraints = {
    tripId: {
      presence: { allowEmpty: false, message: "not empty" },
    },
...
function update_wallet_logs_trip(
  q,
  passenger_id,
  add_amt,
  negative,
                        ...
                        if (transresults.length > 0) {
                          apimodel
                            ...
                            .then(function (updateresults) {
                              console.error("updateresults : ", updateresults);
...
function update_wallet_used_amount(
  ...
  console.error("yes calling this =========================");
  var deferred = q.defer();
  var details = {};
  try {
