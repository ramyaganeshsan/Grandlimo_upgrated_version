...
//echo "<pre>";print_r($listings);exit;
		...
		<tr class="<?php echo $trcolor; ?>">
			...
			<td><?php if(isset($listings['transaction_id']) && $listings['transaction_id'] != "")
				{?>
				<a href="<?php echo URL_BASE.'transaction/transaction_details/'.$listings['passengers_log_id'];?>"><?php echo $listings['transaction_id']; ?></a>
				<?php } else { ?>
				- <?php } ?></td>
			<td><?php if($listings['payment_type'] == 2) { echo __('credit_card_paypal'); } else if($listings['payment_type'] == 4) { echo __('account'); } else if($listings['payment_type'] == 3){ echo __('new_credit_card'); } else { echo __('cash'); } ?></td>
			<td><a href="<?php echo URL_BASE.'transaction/transaction_details/'.$listings['passengers_log_id'];?>">
			<?php echo $listings['passengers_log_id']; ?></a></td>
			<?php } ?>

			
			<td><?php echo (isset($listings['passenger_name'])) ?  ucfirst($listings['passenger_name']) : "--";; ?></td>
			...
			<td><?php echo Commonfunction::convertphpdate('Y-m-d h:i:s A',$listings['createdate']);?></td>
			<?php 
			...
			<td><?php if(isset($listings['nightfare']) == 0) { echo '-'; } else { echo $ccur.round($listings['nightfare'],2);}?></td>
			<td><?php if(isset($listings['eveningfare']) == 0) { echo '-'; } else { echo $ccur.round($listings['eveningfare'],2);}?></td>
			<?php } ?>
			 ...
			 */ ?>
			<td><?php if(isset($listings['fare']) == 0) { echo '-'; } else { echo $ccur.round($listings['fare'],2);}?></td>
<td><?php if(isset($listings['fare']) == 0) { echo '-'; $convet_amt = 0; } else {
		...
		<tr>
			...
			<td></td>
			<?php } ?>
			<?php if($list == 'success') { ?><td></td><?php }?><?php if($list != 'cancelled') { ?><td></td><?php } ?><td></td><td></td><td><?php echo __('total').'('.CURRENCY.')';?></td><td><?php echo $totalfare; ?></td><td></td>
		</tr>
		<?php } 
