...
<div class="container_content fl clr">
	<div class="cont_container mt15 mt10">
		...
		<form method="get" class="form" name="manage_zone" id="manage_zone" action="zone">
...
<tbody>
       ...
       <?php /* <td align="left"><?php echo isset($group_name) ? wordwrap(ucfirst($group_name),30,'<br/>',1) : '--' ; ?></td> */ ?>
		<td align="center" width="20" colspan='3' ><?php echo '<a href='.URL_BASE.'edit/zone/'.$listings['_id'].' " title ="'.__("edit").'" class="editicon"></a>' ; ?></td>
		...
		var selected_val= $('#more_action').val();
			...
			switch (selected_val){
					...
					if($('input[type="checkbox"]').is(':checked'))
					{
				   		 ...
				   		 if(ans){
							 document.manage_zone.action="<?php echo URL_BASE;?>manage/block_zone_request";
					...
					var confirm_msg =  "<?php echo __('areyousure_wantto_activate_request');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.manage_zone.action="<?php echo URL_BASE;?>manage/active_zone_request";
								 document.manage_zone.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
					...
					var confirm_msg =  "<?php echo __('are_yousure_wanttomove_request_to_trash');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.manage_zone.action="<?php echo URL_BASE;?>manage/trash_zone_request";
								 document.manage_zone.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
