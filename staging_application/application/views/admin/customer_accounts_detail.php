...
<div class="container_content fl clr">
	<div class="cont_container mt15 mt10">
		...
		<form method="get" class="form" name="managedriver" id="managedriver" action="<?php echo URL_BASE; ?>manage/customer_wallet_transactions">

				<div id="response"></div>
                		<div class="widget">
        <div class="title"><img src="<?php echo IMGPATH; ?>icons/dark/frames.png" alt="" class="titleIcon" /><h6><?php echo $page_title; ?></h6>
        <span style="color: green;margin-top: 9px;float: left;font-weight: bold;"><?php echo __('from').' '.date("d-m-Y", strtotime($startdate)).' '.__('to').' '.date("d-m-Y", strtotime($enddate)); ?></span>
...
<thead>
	<tr>
		...
		<td align="left" style="text-align:left;" width="3%"><?php echo ucfirst(__('date')); ?></td>
    ...
    <td align="left" style="text-align:left;" width="3%"><?php echo __('passenger_phone'); ?></td>
        <td align="left" style="text-align:left;" width="1%"><?php echo __('opening_balance'); ?></td>
        <td align="left" style="text-align:left;" width="1%"><?php echo __('transactions'); ?></td>
        <td align="left" style="text-align:left;" width="1%"><?php echo __('recharge'); ?></td>
        <td align="left" style="text-align:left;" width="1%"><?php echo __('closing_balance'); ?></td>
	</tr>
</thead>
<tbody>	
		<?php
		...
		$sno=$Offset; 
         ...
         //~ {
			//~ $trip_fare = isset($listings['tripfare'])?CURRENCY.$listings['tripfare']:'-';
		 //~ }else{
			 //~ $trip_fare = '-';
		 //~ }
		 
		 $balance_wallet_amount = $listings['wallet_amount'];
		 
        ?>    
			<?php 
				if($listings['recharge_type'] == 4 || $listings['recharge_type'] == 6) { // Trip 
					$trip_amt += $listings['fare'];
					$tfare = CURRENCY.round($listings['fare'],3); 
					$calt_amt = $listings['fare'];
				} else { // Wallet
					$tfare = '-';
					$calt_amt = 0;
				}		
			?>
			<?php 
				if($listings['recharge_type'] == 8) { // Recharge 
					$recharge_amt += $listings['fare'];
					$rfare = CURRENCY.round($listings['fare'],3); 
					$calr_amt = $listings['fare'];
				} else { // Wallet
					$rfare = '-';
					$calr_amt = 0;
				}		
			...
			<?php $opening_amount = ( $listings['wallet_amount'] + $calt_amt ) - $calr_amt; ?>
			<td align="center"><?php echo $sno; ?></td>
			<td><?php echo isset($listings['activity_date']) ? commonfunction::convertphpdate("d-m-Y H:i:s",$listings['activity_date']) : '-'; ?></td>
			<td align="left"><a title="<?php echo ucfirst($listings['name']); ?>" href="<?php echo URL_BASE.'manage/passengerinfo/'.$listings['passenger_id'];?>"><?php echo wordwrap($listings['name'],30,'<br/>',1); ?></a></td>
       <td><?php echo isset($listings['email']) ? wordwrap($listings['email'],25,'<br />',1) : '-'; ?></td> 
			<td><?php echo isset($listings['phone']) ? $listings['phone'] : '-'; ?></td>
			<td><?php echo ($listings['recharge_type'] == 6) ? CURRENCY."0": CURRENCY.round($opening_amount,3); ?></td>
			<td>
				<?php echo $tfare; ?>
			</td>
            <td>
            ...
            <td><?php echo CURRENCY.round($listings['wallet_amount'],3); ?></td>
		</tr>
		<?php } ?> 
		<tr>
			<td colspan="6"></td>
			<td><?php echo CURRENCY.round($trip_amt,3);?></td>
			<td><?php echo CURRENCY.round($recharge_amt,3);?></td>
			<td><?php echo CURRENCY.round($balance_wallet_amount,3); ?></td>
		</tr>
 		 <?php //echo $trip_amt; echo "<br>"; echo $recharge_amt;?>
 		 <?php } 
