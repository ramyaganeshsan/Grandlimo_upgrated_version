...
?>
		   ...
		   <div class="new_input_field">	
              <input type="checkbox"  maxlength="20" minlength="7"  title="<?php echo __('vip_user'); ?>" id="vip_user" name="vip_user" <?php if($vip_user == 1){ echo "checked"; } ?> value="1" />
              <?php if(isset($errors) && array_key_exists('vip_user',$errors)){ echo "<span class='error'>".ucfirst($errors['vip_user'])."</span>";}?>
		   </div>
	   </td>   	 
       </tr>
	<tr id="vip_user_section"  <?php if($vip_user == 0){ echo "style='display:none'"; }  ?>>
	...
	if(isset($login_detail['wallet_block']) &&!array_key_exists('wallet_block',$postvalue))
	{
		$wallet_block  = isset($login_detail['wallet_block'])?$login_detail['wallet_block']:'0';
	}
	else
	{
		$wallet_block = isset($postvalue['wallet_block'])?$postvalue['wallet_block']:'0';
	}
	?>
       <tr>
	   ...
	   <td>
		   <div class="new_input_field">
            ...
            <?php if(isset($errors) && array_key_exists('notes',$errors)){ echo "<span class='error'>".ucfirst($errors['notes'])."</span>";}?>
		   </div>
           </td>   	
           </tr>
        <tr>
        ...
        <tr>
			<td valign="top" width="20%"><label><?php echo __('wallet_block'); ?></label></td>        
			   <td>
			   <div class="new_input_field">	
	              <input type="checkbox"  maxlength="20" minlength="7"  title="<?php echo __('wallet_block'); ?>" id="wallet_block" name="wallet_block" <?php if($wallet_block == 1){ echo "checked"; } ?> value="1" />
	              <?php if(isset($errors) && array_key_exists('wallet_block',$errors)){ echo "<span class='error'>".ucfirst($errors['wallet_block'])."</span>";}?>
			   </div>
		   </td>   	 
	    </tr>
