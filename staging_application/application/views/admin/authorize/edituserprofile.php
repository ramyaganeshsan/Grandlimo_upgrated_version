...
<div class="container_content fl clr">
	...
	<td></td>	          
	   ...
	   <td>
		   <div class="new_input_field">
              <input type="text" title="<?php echo __('enterfirstname'); ?>" name="firstname" id="firstname" value="<?php echo isset($login_detail['name']) &&!array_key_exists('firstname',$postvalue)? trim($login_detail['name']):$postvalue['firstname']; ?>"  minlength="4" maxlength="30" />
	   ...
	   <td>
		   <div class="new_input_field">
              <input type="text" title="<?php echo __('enterlastname'); ?>" name="lastname" id="lastname" value="<?php echo isset($login_detail['lastname']) &&!array_key_exists('lastname',$postvalue)? trim($login_detail['lastname']):$postvalue['lastname']; ?>" minlength="4"  maxlength="50" />
	...
	<td valign="top" width="20%"><label><?php echo __('mobile'); ?></label><span class="star">*</span></td>        
	   <td>
		   <div class="new_input_field">	
              <input type="text"  maxlength="30" minlength="7"  title="<?php echo __('entermobileno'); ?>" id="phone" name="phone" value="<?php echo isset($login_detail['phone']) &&!array_key_exists('phone',$postvalue)? trim($login_detail['phone']):$postvalue['phone']; ?>" />
	...
	<?php $country_field =''; $country_field =  isset($login_detail['login_country']) &&!array_key_exists('country',$postvalue)? trim($login_detail['login_country']):$postvalue['country']; ?>
	<?php /* <tr>
	...
	<span><?php echo __('select_label'); ?></span>
              ...
              <option value="<?php echo $country_list['_id']; ?>" <?php if($country_field == $country_list['_id']) { echo 'selected=selected'; } ?>><?php echo $country_list['country_name']; ?></option>
              <?php } ?>
              </select>
        </div>
	</div>
	...
	<tr>
	<?php $state_field ='';
		if(isset($login_detail['login_state'])){
			$state_field =  isset($login_detail['login_state']) &&!array_key_exists('state',$postvalue)? trim($login_detail['login_state']):$postvalue['state'];
		} ?>
	...
	<div id="state_list">
		...
		<option value="<?php echo $state_list['state_id']; ?>" <?php if($state_field == $state_list['state_id']) { echo 'selected=selected'; } ?> ><?php echo $state_list["state_name"]; ?></option>
		<?php	} ?>
		</select>
	</div>	
		</div></div>
	...
	*/ ?>
	...
	<tr>
	<?php $city_field ='';
	if(isset($login_detail['login_city'])){
		$city_field =  isset($login_detail['login_city']) &&!array_key_exists('city',$postvalue)? trim($login_detail['login_city']):$postvalue['city'];
	} ?>
	...
	<div id="city_list">
		...
		<option value="<?php echo $city_list['city_id']; ?>" <?php if($city_field == $city_list['city_id']) { echo 'selected=selected'; } ?> ><?php echo $city_list["city_name"]; ?></option>
		<?php	} ?>
		</select>
	</div>	
		</div></div>
...
<script type="text/javascript">
 $(document).ready(function(){
	...
	//change_state('<?php /* echo $country_field; */ ?>','<?php /*echo $state_field;*/ ?>');
	//change_city_drop('<?php /* echo $country_field; */ ?>','<?php /* echo $state_field; */ ?>','<?php /* echo $city_field;*/ ?>');
});
