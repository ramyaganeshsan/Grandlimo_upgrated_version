	...
	<tbody>
		...
		<tr class="<?php echo $trcolor; ?>">
		    ...
		    <td align="center"><?php echo $sno; ?></td>
                    <td align="center">
                         <?php if($listings['payment_status']=='A'){
			...
			<td align="center"><?php echo '<a href='.URL_BASE.'edit/paymentgateway/'.$listings['_id'].' " title ="Edit" class="editicon"></a>' ; ?></td>
			<td align="center"><input type="radio" name="default_payment" <?php if($listings['default_payment_gateway'] == 1){?>checked = "checked"<?php }?> value="<?php echo $listings['_id']; ?>"/></td>
		</tr>
		<?php }
		if($sno >=1){?>
			<tr>
...
<?php if($total_company > 0) { ?>
   <div class="select_all" style="margin-top:-25px;">
		 ...
		 <span style="padding-left:10px;">
			 <select name="more_action" id="more_action">
				 ...
				 <option value="block_gateway_request" ><?php echo __('Block'); ?></option>
				 <option value="active_gateway_request" ><?php echo __('Active'); ?></option>
				 <?php /* <option value="trash_gateway_request" ><?php echo __('Trash'); ?></option> */ ?>
			 </select>
		  </span>
	 </div>
 <?php } ?>
...
$('#more_action').change(function() {
	...
	switch (selected_val){
		...
		if($('input[type="checkbox"]').is(':checked')) {
			...
			if(ans){
				document.manage_payment.action="<?php echo URL_BASE;?>manage/block_gateway/";
		...
		case "active_gateway_request":
			...
			if($('input[type="checkbox"]').is(':checked'))
			{
				 var ans = confirm(confirm_msg)
				 if(ans){
					 document.manage_payment.action="<?php echo URL_BASE;?>manage/active_gateway/";
					 document.manage_payment.submit();
				 }else{
					$('#more_action').val('');
				 }
		...
		case "trash_gateway_request":
			...
			if($('input[type="checkbox"]').is(':checked')){
				 var ans = confirm(confirm_msg)
				 if(ans){
					 document.manage_payment.action="<?php echo URL_BASE;?>manage/trash_gateway/";
					 document.manage_payment.submit();
				 }else{
					$('#more_action').val('');
				 }
