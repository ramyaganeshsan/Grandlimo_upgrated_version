                         ...
                         <form id = "pay_submit_form" method="post" action="" enctype="multipart/form-data">
                            ...
                            <!--<input type="hidden" name="travel_status" id = "travel_status">-->
                                    <div>
                                            ...
                                            <tr><td><label><?php echo __('online_knet'); ?>:&nbsp;</label></td><td><input class="form-control pay" type="text" name="knet_pay" id = 'knet_pay'  value = 0></td></tr>
                                            <tr><td><label><?php echo __('wallet_label'); ?>:&nbsp;</label></td><td><input class="form-control pay" type="text" name="wallet_pay" id = 'wallet_pay'  value = 0></td></tr>
    ...
    $("#pay_submit").on('click',function(e)
     {
        ...
        {
            $("#add_err").html("");
            if(isNaN(cash_tot) || add_amt == '' || fare_note == ''){
        ...
        if($("#add_err").text()== "")
        {
            ...
            var data = "add_amt="+add_amt+"&cash_pay="+cash+"&card_pay="+card+"&knet_pay="+knet+"&fare_note="+fare_note+"&edit_pass_logid="+findid+"&pending_pay="+pending_pay+"&wallet_pay="+wallet_pay;
             ...
             $.ajax({
                ...
                success:function(response){
                    if(response == 1){
                       ...
                       $("#cash_pay #card_pay #knet_pay #add_amt").value(0);
                       $("#fare_note").value('');
                           ...
                           /* if(details.approx_duration != '') {
                            ...
                            //alert(fare);
                                if( typeof details.pay == "undefined")
                                {
                                 ...
                                 if(details.faretype == 1){$("#cash_pay").val(fare);}
                                 else{$("#cash_pay").val(0);}
                                 if(details.faretype == 6){$("#card_pay").val(fare);}
                                 else{$("#card_pay").val(0);}
                                  if(details.faretype == 3){$("#knet_pay").val(fare);} 
                                  else{$("#knet_pay").val(0);}
                                  $("#add_amt").val(0);
                                  $("#fare_note").val("");                    
                                }
...
<div class="modal fade" id="passenger_admin_notes" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" >
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&nbsp;</span></button>
                   ...
                   <div class="modal-body">
                      <p id="passenger_admin_notes_msg"></p>
                  <div class="modal-footer">
                    <input type="submit" name="cancel_reason_submit" class="btn btn-primary" id='cancel_reason_submit' value="Submit" style="background: #f9c213;border: none;color: #000;"/>
                    <button  style="margin-right: 15px; background: #f9c213;border: none;color: #000;" type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
                  </div>
