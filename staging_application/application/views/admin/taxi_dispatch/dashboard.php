...
<div style="display:none;width:100%;background-color:rgba(0,0,0,0.6);height:100dvh;z-index: 999999;position: absolute;justify-content: center;align-items: center;" id="driver_on_going_model">
   <div style="min-width:500px; max-width:500px; background-color:white;padding:16px 16px; border-radius: 8px;">
        <div>
            <h2 style="margin: 0px;font-size: 18px;letter-spacing: 1px;font-weight: bold;">Drivers</h2>
        </div>

        <div id="on_going_driver_container" style="display: flex; flex-direction: column; gap : 4px; margin: 30px 0px; max-height: 300px;overflow-y: auto;">
            <div id="driver_on_going_list_loader" style="display: flex;min-height: 300px; justify-content: center; align-items:  center;">
                <svg style="width:50px; height:50px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><radialGradient id="a12" cx=".66" fx=".66" cy=".3125" fy=".3125" gradientTransform="scale(1.5)"><stop offset="0" stop-color="#000000"></stop><stop offset=".3" stop-color="#000000" stop-opacity=".9"></stop><stop offset=".6" stop-color="#000000" stop-opacity=".6"></stop><stop offset=".8" stop-color="#000000" stop-opacity=".3"></stop><stop offset="1" stop-color="#000000" stop-opacity="0"></stop></radialGradient><circle transform-origin="center" fill="none" stroke="url(#a12)" stroke-width="15" stroke-linecap="round" stroke-dasharray="200 1000" stroke-dashoffset="0" cx="100" cy="100" r="70"><animateTransform type="rotate" attributeName="transform" calcMode="spline" dur="2" values="360;0" keyTimes="0;1" keySplines="0 0 1 1" repeatCount="indefinite"></animateTransform></circle><circle transform-origin="center" fill="none" opacity=".2" stroke="#000000" stroke-width="15" stroke-linecap="round" cx="100" cy="100" r="70"></circle></svg>
            </div>
        </div>
        
        <div style="display:flex;gap:4px;justify-content:space-between;">
            <button id="driver_on_going_model_cancel_button" onclick="handleCloseDriverOnGoingPopup()" style="width:100%;background-color:tomato;border-radius:6px;color:white;border: none;outline: none;padding: 10px 0px;">Cancel</button>
        </div>
    </div>
</div>

<!-- Sasidharan Jan 17 2025 -->
<div style="display:none;width:100%;background-color:rgba(0,0,0,0.6);height:100dvh;z-index: 999999;position: absolute;justify-content: center;align-items: center;" id="couponModel">
   <div style="min-width:300px;background-color:white;padding:16px 16px;">
        <div>
            <h2 style="margin: 0px;font-size: 18px;letter-spacing: 1px;font-weight: bold;">Add Coupon Code</h2>
        </div>
        <div style="display: flex;flex-direction: column;padding: 16px 0px;">
            <label>Coupon Code</label>
            <input id="couponCode" placeholder="Coupon code" style="padding:8px 12px" />
            <label id="couponCodeError" style="color:tomato;letter-spacing:1px;padding-top: 8px;font-size: 12px;"></label>
        </div>
        <div style="display:flex;gap:4px;justify-content:space-between;">
            <button id="coupon_submit_button" onclick="handleSubmitCouponPopup()" style="width:48%;background-color:black;border-radius:6px;border: none;color:white;outline: none;padding: 10px 0px;">Add Coupon</button>
            <button id="coupon_cancel_button" onclick="handleCloseCouponPopup()" style="width:48%;background-color:tomato;border-radius:6px;color:white;border: none;outline: none;padding: 10px 0px;">Cancel</button>
        </div>
    </div>
</div>
                        ...
                        <div class="payment_info_content">
                            <table>
                                <tr>
                                    ...
                                    <td><input class="form-control pay" type="text" name="cash_pay_show" id="cash_pay" value=0></td>
                                </tr>
                                <tr>
                                    <td><label><?php echo __('card'); ?>:&nbsp;</label></td>
                                    <td><input class="form-control pay" type="text" name="card_pay_show" id="card_pay" value=0></td>
                                </tr>
                                <tr>
                    ...
                    <div class="total-earnings-outer">
                        <div class="tab-content">
                            <div class="tab-pane" id="total-earnings" style="display:block;visibility:visible;">
                                ...
                                <ul>
                                    <?php
                                    $total_amt = isset($total_sum[0]['total_sum']) ? round($total_sum[0]['total_sum'],3) : 0;
                                    $total_pending_amt = isset($total_sum[0]['total_pending']) ? round($total_sum[0]['total_pending'],3) : 0;
                                    ?>
        ...
        function all_booking_manage_list() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.oddtr').bind('click', function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                //alert(travel_status);
                                GetFavPlace(details.passengers_id);
                    ...
                    $('.view_last_four_drivers').click(function() {
                        $("#last_four_drivers_list").html("");
                        var thisid = this.id;
                        //var pass_logid = thisid.split('_').pop();
                        var passid = thisid.split('_');
                        var data = "id=" + passid[1] + "&trip_id=" + passid[2] + "&model_id=" + passid[3];
                        var url_path = "<?php echo URL_BASE; ?>taxidispatch/get_last_four_drivers_list";
                        $.ajax({
                            type: "GET",
                            url: url_path,
                            data: data,
                            ...
                            success: function(res) {
                                if (res) {
                                    $("#last_four_drivers").modal({
                                        show: true
                                    });
                                    $("#last_four_drivers_list").html(res);
                                    //
                                } else {
                                    $("#last_four_drivers").modal({
                                        show: true
                                    });
                                    $("#last_four_drivers_list").html("<li>No trips completed yet</li>");
                                }
                            },
                            error: function() {
                                //alert('failed'); 
...
<script type="text/javascript">
   ...
   const handleDriverOnGoingPopup = () => {
        let element = document.getElementById("driver_on_going_model");
        var url_path = "<?php echo URL_BASE; ?>taxidispatch/get_on_going_driver_list";
        if(element) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            let spinner = document.getElementById("driver_on_going_list_loader");
            if(spinner) {
                spinner.style.display = "none";
            }

            if(response && response.length > 0) {
                const container = document.getElementById('on_going_driver_container');
                response.forEach((driver) => {
                  const wrapper = document.createElement('div');
                  wrapper.style.display = 'flex';
                  wrapper.style.flexDirection = 'row';

                  let image = "/public/images/gps_green.png";
                  if(!driver.gps_enable) {
                    image = "/public/images/gps_red.png";
                  }

                  wrapper.innerHTML = `
                    <p style="width:40%;">${driver.name} (${driver.driver_code})</p>
                    <p style="width:30%; display: flex; flex-direction: row; gap: 4px;">
                      <span><img src="${image}" /></span>
                      <span><img src="/public/images/internet_red.png" /></span>
                    </p>
                    <p style="width:40%;">${driver.trip_id} (${getStatus(driver.travel_status)})</p>
                  `;

                  container.appendChild(wrapper);
                });
            } else {
                alert("No drivers were found.");
            }
          },
          error : function(err) {
            alert("Failed to load data.");
   ...
   const handleCloseDriverOnGoingPopup = () => {
        let element = document.getElementById("driver_on_going_model");
        let spinner = document.getElementById("driver_on_going_list_loader");
        if(element) {
   ...
   let couponTripId = null;
   let couponDriverId = null;
   const handleCouponPopup = (passengerPhone, tripId, driverId) => {
        let element = document.getElementById("couponModel");
        if(element) {
            phoneNumber = passengerPhone;
            couponTripId = tripId;
            couponDriverId = driverId;
            element.style.display = "flex";
        }
   }
   ...
   const handleSubmitCouponPopup = async() => {
        let couponCode = document.getElementById("couponCode").value;
        let couponError = document.getElementById("couponCodeError");

        if(!couponCode) {
            if(couponError) {
                couponError.innerHTML = "Coupon should not empty.";
            }
            return;
        }

        couponError.innerHTML = "";
        let submitButton = document.getElementById("coupon_submit_button");
        let cancelButton = document.getElementById("coupon_cancel_button");
        ...
        let params = {
            driver_id : couponDriverId,
            trip_id : couponTripId,
            phone : phoneNumber,
            coupon_code : couponCode
        }
        var url_path = "<?php echo URL_BASE; ?>add/add_passenger_coupon";
        let response = await $.ajax({
            ...
            success: function(resultData) { 
                ...
                if(resultData.status) {
                    ...
                    couponError.innerHTML = resultData.message;
                    if(submitButton) {
                        submitButton.disabled = false;
                        submitButton.innerHTML = "Add Coupon"
                    }
