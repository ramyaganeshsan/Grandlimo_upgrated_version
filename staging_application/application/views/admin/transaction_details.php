...
if(SHOW_MAP !=1) {
		if(count($driver_tracking) > 0) { 
				...
				/****** Location history extra param issues start**/
				if(count($driver_default)){
					foreach ($driver_default as $key => $value) {
						if($value==1 || $value == 2 ){
							unset($driver_default[$key]);
						}
					}
				}
				$driver_default = array_values($driver_default);
				/****** Location history extra param issues end **/
