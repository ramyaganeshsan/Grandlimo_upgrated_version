...
<div class="container_content fl clr">
	<div class="cont_container mt15 mt10">
		...
		<div class="error alert-message">
		    ...
		    { ?>
			<p><?php echo __('kindly_deactivate_any').substr($availabilitycount,1).__('corporate_from_list'); ?> </p>
		    <?php }
		    else
		    {  
		    	if($total_company > 0) { ?>
		    	<p><?php echo __('kindly_deactivate_any').substr($availabilitycount,1).__('corporate_from_list').__('or_contact_company_owner'); ?> </p>
		    	<?php }
		    	else { ?>
		...
		?>		 
        <form method="get" class="form" name="managecorporate" id="managecorporate" action="corporatesearch">
					...
					<tr>
                        ...
                        <td>
                            ...
                            <div class="button blueB">
                                <input type="button" value="<?php echo __('button_cancel'); ?>" title="<?php echo __('button_cancel'); ?>" onclick="location.href = '<?php echo URL_BASE; ?>manage/corporate'" />
                            </div>
                        </td>
                    </tr>
                </table>
...
<tbody>	
        ...
        <tr class="<?php echo $trcolor; ?>">
                   ...
                   <!-- <td> 
                    ...
                    <td align="center"> 
                         <?php 
                             if($listings['corporate_status']=='A')
                             {  $txt = "Active"; $class ="unsuspendicon";    }
				elseif($listings['corporate_status']=='T')
			...
			<td align="left"><?php echo wordwrap($listings['name'],30,'<br/>',1); ?></td>
			
			<?php /*<td><a href="<?php echo URL_BASE.'manage/corporate_unavailable/'.$listings['id'];?>"><?php echo __('mark_unavailable'); ?></a></td>*/ ?>
			...
			<td align="center"><a href="<?php echo URL_BASE.'manage/corporateinfo/'.$listings['id'];?>">
	
			<?php  if(file_exists($_SERVER["DOCUMENT_ROOT"].'/public/uploads/corporate_image/'.$listings['photo']) &&($listings['photo'] != "")){  ?> 
					<img width="75" height="75" src="<?php echo URL_BASE.SITE_CORPORATE_IMGPATH.$listings['photo'];?>?q=<?php echo time();?>"/>
				<?php }else{ ?>
			...
			<td align="center" colspan='3' ><?php echo '<a href='.URL_BASE.'edit/corporate/'.$listings['id'].' " title ="Edit" class="editicon"></a>' ; ?></td>
...
<?php if(count($all_company_list) > 0)
       { ?>
          <div class="select_all">
                <b><a href="javascript:selectToggle(true, 'managecorporate');"><?php echo __('all_label');?></a></b><span class="pr2 pl2">|</span><b><a href="javascript:selectToggle(false, 'managecorporate');"><?php echo __('select_none');?></a></b>
                ...
                <span style="padding-left:10px;">
                    <select name="more_action" id="more_action">
                        <option value=""><?php echo __('Change Status'); ?></option>
                         ...
                         <option value="active_corporate_request" ><?php echo __('Active'); ?></option>
                        <?php if(COMPANY_CID!=1 || SUBDOMAIN!='demo') { ?> 
						<option value="block_corporate_request" ><?php echo __('Block'); ?></option>
                        <option value="trash_corporate_request" ><?php echo __('Trash'); ?></option> 
                        <!--<option value="mute_corporate_request" ><?php //echo __('Mute'); ?></option>-->
                        <?php } ?>
                    </select>
                 </span>
	        </div>
		...
		var selected_val= $('#more_action').val();
			...
			switch (selected_val){
					...
					if($('input[type="checkbox"]').is(':checked'))
					{
				   		 ...
				   		 if(ans){
							 document.managecorporate.action="<?php echo URL_BASE;?>manage/block_corporate_request/index";
							 document.managecorporate.submit();
						 }else{
						 	$('#more_action').val('');
						 }
					...
					var confirm_msg =  "<?php echo __('Are you sure want to Activate Request(s)?');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.managecorporate.action="<?php echo URL_BASE;?>manage/active_corporate_request/index";
								 document.managecorporate.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
					...
					var confirm_msg =  "<?php echo __('Are you sure want to move Request(s) to Trash ?');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.managecorporate.action="<?php echo URL_BASE;?>manage/trash_corporate_request/index";
								 document.managecorporate.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
					...
					var confirm_msg =  "<?php echo __('Are you sure want to Mute the Driver(s) ?');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.managecorporate.action="<?php echo URL_BASE;?>manage/mute_corporate_request/index";
								 document.managecorporate.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
