...
<thead>
	...
	<?php
		/* For Serial No */
		$sno=$Offset;
		//echo '<pre>';print_r($all_countries_list); echo '</pre>';exit;
		foreach($all_countries_list as $listings) { 
			//S.No Increment
			$sno++;        
		   //For Odd / Even Rows
	...
	?>
		<tr class="<?php echo $trcolor; ?>">
			...
			<td align="center"><?php echo '<a href='.URL_BASE.'edit/country/'.$listings['_id'].' " title ="Edit" class="editicon"></a>' ; ?></td>
			<td align="center"><input type="radio" name="default_country" <?php if($listings['default'] == 1){?>checked = "checked"<?php }?> value="<?php echo $listings['_id']; ?>"/></td>
		</tr>
		<?php }
		if($sno>=1){
		?>
			<tr><td colspan="10" align="right"><div class="button blackB"><input type="submit" name="update" value="<?php echo __('update');?>" /></div></td></tr>   
		<?php }else { ?>
			<tr><td colspan="7" align="center"><?php echo __('no_data');?></td></tr>
		<?php }
 		}
		//For No Records
	     else{ ?>
...
<div class="clr">&nbsp;</div>
	...
	<?php if(count($all_countries_list) > 0) { ?>
