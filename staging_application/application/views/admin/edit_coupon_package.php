...
<div class="container_content fl clr">
	...
	<tr>
    <td valign="top" width="20%"><label><?php echo __('coupon_package_name_ar'); ?></label><span class="star">*</span></td>        
    <td>
        <div class="new_input_field">
            <input type="text" title="<?php echo __('enter_coupon_package_name_ar'); ?>" class="required" name="coupon_package_name_ar" id="coupon_package_name_ar" 
            value="<?php echo isset($company_details['coupon_package_name_ar']) && !array_key_exists('coupon_package_name_ar', $postvalue) ? trim($company_details['coupon_package_name_ar']) : (isset($postvalue['coupon_package_name_ar']) ? $postvalue['coupon_package_name_ar'] : ''); ?>" maxlength="100" />
            <?php if(isset($errors) && array_key_exists('coupon_package_name_ar', $errors)) { echo "<span class='error'>".ucfirst($errors['coupon_package_name_ar'])."</span>"; } ?>
        </div>
    </td>    
</tr>

<tr>
    <td valign="top" width="20%"><label><?php echo __('coupon_package_description_ar'); ?></label><span class="star">*</span></td>        
    <td>
        <div class="new_input_field">
            <textarea name="coupon_package_description_ar" id="coupon_package_description_ar" class="required" title="<?php echo __('enter_coupon_package_description_ar'); ?>" rows="7" cols="35"><?php echo isset($company_details['coupon_package_description_ar']) && !array_key_exists('coupon_package_description_ar', $postvalue) ? trim($company_details['coupon_package_description_ar']) : (isset($postvalue['coupon_package_description_ar']) ? $postvalue['coupon_package_description_ar'] : ''); ?></textarea>
            <?php if(isset($errors) && array_key_exists('coupon_package_description_ar', $errors)) { echo "<span class='error'>".ucfirst($errors['coupon_package_description_ar'])."</span>"; } ?>
        </div>
    </td>    
</tr>
	...
	<?php
	if(isset($postvalue['fixed_fare_apply']) &&!array_key_exists('fixed_fare_apply',$postvalue))
	{
		$fixed_fare_apply  = isset($postvalue['fixed_fare_apply'])?$postvalue['fixed_fare_apply']:'0';
	}else{
		$fixed_fare_apply = isset($company_details['fixed_fare_apply'])?$company_details['fixed_fare_apply']:'0';
	}
	?>

	<tr>
	<td valign="top" width="20%"><label><b><?php echo __('fixed_fare_apply_this_coupon'); ?></b></label></td>        
	<td>
	<div class="new_input_field">
		     <input type="checkbox"  maxlength="20" minlength="7"  title="<?php echo __('fixed_fare_apply'); ?>" id="fixed_fare_apply" name="fixed_fare_apply" <?php if($fixed_fare_apply == 1){ echo "checked"; } ?> value="1" />
	              <?php if(isset($errors) && array_key_exists('fixed_fare_apply',$errors)){ echo "<span class='error'>".ucfirst($errors['fixed_fare_apply'])."</span>";}?>
	</div>
	</td>   	
	</tr> 
