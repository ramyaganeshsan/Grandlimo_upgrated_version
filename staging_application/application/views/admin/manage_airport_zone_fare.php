...
<div class="container_content fl clr">
 ...
 <tr>
                        ...
                        <td >
                            ...
                            <span class="search_info_label"><?php echo __('searchbyzone_name'); ?></span>
                        </td>
...
<thead>
	<tr>
		...
		<td align="left" width="5%"><?php echo __('zone_name'); ?></td>
		<td align="left" width="5%"><?php echo __('model_name'); ?></td>
		<td align="left" width="5%"><?php echo __('zone_fixed_fare'); ?></td>
...
<tbody>
		...
		<?php
		 ...
		 foreach($zone_details as $listings ) { 
		 //S.No Increment
		 //==============
		 $sno++;
         //For Odd / Even Rows
       ...
       //  echo '<pre>'; print_r($listings); exit();
        ...
        <td align="left"><?php echo wordwrap(ucfirst($listings['zone_name']),30,'<br/>',1); ?></td>     
		<td align="left"><?php echo wordwrap(ucfirst($listings['model_name']),30,'<br/>',1); ?></td>           
		<td align="left"><?php echo $listings['zone_fixed_fare']; ?></td>        
		<td align="center" width="20" colspan='3' ><?php echo '<a href='.URL_BASE.'zonefare/edit/'.$listings['_id'].' " title ="'.__("edit").'" class="editicon"></a>' ; ?></td>
		...
		var selected_val= $('#more_action').val();
			...
			switch (selected_val){
					...
					if($('input[type="checkbox"]').is(':checked'))
					{
				   		 ...
				   		 if(ans){
							 document.manage_zone.action="<?php echo URL_BASE;?>zonefare/block_zone_fare_request";
					...
					var confirm_msg =  "<?php echo __('areyousure_wantto_activate_request');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.manage_zone.action="<?php echo URL_BASE;?>zonefare/active_zone_fare_request";
								 document.manage_zone.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
					...
					var confirm_msg =  "<?php echo __('are_yousure_wanttomove_request_to_trash');?>";
						...
						if($('input[type="checkbox"]').is(':checked'))
						{
					   		 var ans = confirm(confirm_msg)
					   		 if(ans){
								 document.manage_zone.action="<?php echo URL_BASE;?>zonefare/trash_zone_fare_request";
								 document.manage_zone.submit();
							 }else{
							 	$('#more_action').val('');
							 }		
