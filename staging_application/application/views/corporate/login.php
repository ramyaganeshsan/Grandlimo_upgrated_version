...
<div class="loginWrapper">
	...
	<?php } ?>
        ...
        <?php if($controller == 'company') { echo __('companyad_login'); } elseif($controller == 'manager') { echo __('managerad_login');   } if($controller == 'admin') { echo __('page_login_title'); }if($controller == 'corporate') { echo __('corporate_login_title'); }
        	?>
        </h6>
        </div>
        <?php
                ...
                <div class="formRow">
                    ...
                    <div class="loginInput">
						<?php if(SUBDOMAIN == 'demo'){?>
												<!--- STATIC EMAIL---->
						<input type="text" id="email" name="email" value="dispatcher@taximobility.com" maxlength="50" />
						<?php } else { ?>
...
<?php if(isset($errors) && array_key_exists('email',$errors)){ echo "<span class='error'>".__('please_provide_email')."</span>";}?>
                ...
                <div class="formRow">
                    ...
                    <?php if(SUBDOMAIN == 'demo'){?>
						<!--- STATIC PASSWORD---->
						<input type="password" name="password" value="manager" maxlength="15" />
						<?php } else { ?>
...
<?php if(isset($errors) && array_key_exists('password',$errors)){ echo "<span class='error'>".ucfirst($errors['password'])."</span>";}?>
				...
				<?php if(SUBDOMAIN != 'demo'){ ?>
                    <div class="rememberMe"><a href="<?php echo URL_BASE; ?>corporate/forgot_password" class="frgtpsd"><?php echo __('forgot_password'); ?></a></div>
					<?php }  ?>
