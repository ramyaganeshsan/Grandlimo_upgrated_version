...
//~ echo $id;
    ...
    public function action_today_unassigned_taxi()
    {
        ...
        $count_company_list        = $this->admin_model->free_taxi_list_all_pag($keyword,NULL,NULL,$cid,TRUE);
        ...
        $all_company_list           = $this->admin_model->free_taxi_list_all_pag($keyword,$offset, REC_PER_PAGE, $cid, FALSE);
        //****pagination ends here***//
        $get_allcompany             = $this->manage_model->get_allcompany();
        $details                    = '';
        //send data to view file 
        $view                       = View::factory('admin/manage_today_unassigned_taxi')->bind('all_company_list', $all_company_list)->bind('get_allcompany', $get_allcompany)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $count_company_list)->bind('srch', $_REQUEST)->bind('Offset', $offset);
    ...
    public function action_freetaxi()
    {
        ...
        $keyword = isset($_REQUEST['keyword']) ? $_REQUEST['keyword'] : "";
        $count_company_list = $this->manage_model->all_freetaxi_list($keyword,NULL,NULL,$cid,TRUE);
        //pagination loads here
        //-------------------------
        $page_no            = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
        ...
        //$all_company_list = $this->manage_model->all_assigntaxi_list($offset, REC_PER_PAGE);
        $all_company_list           = $this->manage_model->all_freetaxi_list($keyword,$offset, REC_PER_PAGE, $cid, FALSE);
        //****pagination ends here***//
        $get_allcompany             = $this->manage_model->get_allcompany();
        $details                    = '';
        //send data to view file 
    ...
    public function action_unassign_driver()
    {
        ...
        $availabilitycount         = $this->add_model->validate_packagedriver($cid);
        $keyword = isset($_REQUEST['keyword']) ? $_REQUEST['keyword'] : "";
        $count_company_list        = $this->manage_model->all_free_driver_list($keyword,null,null,$cid,true);
        //pagination loads here
        //-------------------------
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
        ...
        $all_company_list           = $this->manage_model->all_free_driver_list($keyword, $offset, REC_PER_PAGE, $cid, FALSE);
        //****pagination ends here***//
        $get_allcompany             = $this->manage_model->get_allcompany();
        $details                    = '';
        //send data to view file 
...
?>],
 ...
 public function action_delete_id_image()
    ...
    public function action_pending_customer_transactions() 
    {
        ...
        else
        {
           $count_company_list = $this->manage_model->pending_customer_transactions('','','','','','',NULL,NULL,TRUE);  
            ...
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->pending_customer_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {                    
                    $xls_output[$key]['balance_wallet_amount'] = round($val['balance_wallet_amount'],3);
    ...
    public function action_ooredo_testt(){

        //$soapUrl = "http://www.ooredoo.com.kw/VoucherStatusCheck/"; // asmx URL of WSDL

        $soapUrl = "http://10.5.1.138/LoyaltyServices_SB/ProxyService/VoucherStatusService?wsdl";

        $voucher_number = "1-87005799106";
        $partner_id = "1-13YWVSII";


        $soapUser = "grandlimo";  //  username
        $soapPassword = "grandlimo@2018"; // password

        $xml_post_string = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:vouc="'.$soapUrl.'">
                            <soapenv:Header>
        ...
        $headers = array(
            ...
            if (isset($error_msg)) {
               echo $error_msg.'error part--->';exit;
            }
    ...
    public function action_lost_customer_report() 
    {
        ...
        else
        {
           $count_company_list = $this->manage_model->lost_customer_report('','','','',NULL,NULL,TRUE);  
            ...
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->lost_customer_report(trim(Html::chars($keyword)), trim(Html::chars($status)),$start_date,$end_date, 0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {   
    ...
    public function action_customer_accounts_report() 
    {
        ...
        if(isset($_SESSION['download_set']))
		{
			$all_company_list = $this->manage_model->customer_accounts_report($id, $start_date, $end_date, $keyword, REC_PER_PAGE, $offset, FALSE);	
			$xls_output = $all_company_list;
			foreach($all_company_list as $key => $val)
			{   
    ...
    public function action_fleet_sales_report()
    {
        ...
        if (isset($_REQUEST['keyword']))
        {
            $all_company_list = $this->manage_model->fleet_sales_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),trim(Html::chars($_REQUEST['taxi_model'])),$start_date,$end_date,$year,$month, $company, $offset, REC_PER_PAGE);
        }
        else
        {
            $all_company_list = $this->manage_model->fleet_sales_report('','','',$start_date,$end_date,$year,$month,'', $offset, REC_PER_PAGE);
        }
          if(isset($_SESSION['download_set']))
            {
                $xls_output = $all_company_list;
    ...
    public function action_send_notification()
    {
        ...
        if ($signup_submit) {
            ...
            $form_values = Arr::extract($_POST, array('content','title','sub_title','template','start_date','expire_date'));
            $values      = $form_values;
            $image_name = '';
            ...
            if ($validator->check()) { 
                ...
                $notification_status = $this->manage_model->admin_add_notification($pushmessage,$template,$title,$sub_title,$start_date,$expire_date,$image_only,$image_name);
     ...
     public function action_advance_customer_transactions() 
    {
        ...
        else
        {
           $count_company_list = $this->manage_model->advance_customer_transactions('','','','','','',NULL,NULL,TRUE);  
            ...
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->advance_customer_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {                    
                    $xls_output[$key]['balance_wallet_amount'] = round($val['balance_wallet_amount'],3);                    
