         ...
         if(ENABLE_LOGS == 2)
        {
            ...
            switch ($method) {
                ...
                case 'getcoreconfig':
                    ...
                    $config_array['iOS_Version_Number']     = 4.4;
                        ...
                        if ($status != 'A') {
                            ...
                            if ($driver_trip_count == 0) {
                                ...
                                $shiftupdate_arrary  = array(
                                    ...
                                    "login_end" => new MongoDate(strtotime($this->currentdate)),
                                    "status" => "Logout",
                                    "end_km" => (int)$end_km,
                ...
                $trip_details['ratings']              = isset($journey['ratings']) ? $journey['ratings']:'';
                            $trip_details['tags']                 = isset($journey['tags']) ? $journey['tags']:array();
                               ...
                               $trip_details['waiting_time_mins'] = $trip_details['waiting_time'] * 60;
                                ...
                                $o_fare = isset($journey['o_fare'])?$journey['o_fare']:0;

				$driver_edit_status = isset($journey['driver_edit_status'][0])?$journey['driver_edit_status'][0]:0;
                                $add_amt = isset($journey['add_amt'][0])?$journey['add_amt'][0]:0;
                                $tripfare = isset($journey['tripfare'][0])?$journey['tripfare'][0]:0;
                                $actual_paid_amt = isset($journey['actual_paid_amt'][0])?$journey['actual_paid_amt'][0]:0;

                                if($driver_edit_status > 0 )
                                {
                                    $trip_fare = $tripfare+$add_amt;
                                }
                                else
                                {
                                    $trip_fare = $actual_paid_amt+$wallet_used_amount;
                                }

                                //$trip_fare = $fare+$wallet_used_amount;

                                $trip_details['trip_fare'] = round($trip_fare,2);
                                $trip_details['amt'] = round($trip_fare,2);
                                //ride fare
                                $minutes_fare = $trip_fare - $waiting_cost; 
                                $trip_details['minutes_fare'] = round($minutes_fare,2);
                                //cash / Wallet
                                $paid_amt = $trip_fare - $wallet_used_amount; 
                                $trip_details['paid_amt'] = round($trip_fare,2);
                                ...
                                else
                                {
                                    ...
                                    $location_data = $api_model->get_location_details($trip_id);
                                     $path   = isset($location_data[0]['active_record'])?$location_data[0]['active_record']:array();
                ...
                case 'update_ratings_comments':
                    ...
                    if ($rating_array['pass_id'] != null) {
                        ...
                        if ($validator->check()) {
                            ...
                            $driver_tags = isset($rating_array['driver_tags'])?urldecode($rating_array['driver_tags']):array();
                                   ...
                                   // print_r($get_passenger_log_details);exit;
                                    ...
                                    if (($travel_status == 2) || ($travel_status == 5)) {
                                       ...
                                       /* WAITING TIME UPDATE - NEW CHANGES  - ENDS */
                                        ...
                                        /*$taxi_edit_modelid = isset($get_passenger_log_details[0]['taxi_edit_modelid'])?$get_passenger_log_details[0]['taxi_edit_modelid']:'';
                                        if($taxi_edit_modelid!=''){
                                            $fare_model_id = $taxi_edit_modelid;
                                        }else{
                                            $fare_model_id = $taxi_model_id;
                                        }*/
                                        $fare_model_id = $taxi_model_id;

                                        $taxi_fare_details  = $api->get_model_fare_details($company_id, $fare_model_id, $get_passenger_log_details[0]['search_city']);  
                                    ...
                                    $passenger_pending_amt = 0;
                                      ...
                                      foreach($gateway_details as $k => $g){
                                        ...
                                        */
                                        ...
                                        $update_query = array('passenger_pending_amt'=>abs($passenger_pending_amt));
                                        ...
                                        if($lateral_end_date != '')
                                        {
                                            $lateral_end_date_sec = isset($lateral_end_date->sec)?$lateral_end_date->sec:0;
                                        }
                                        ...
                                        if(($passenger_wallet >= $total_fare && $passenger_wallet != 0 && $passenger_wallet > 0 && WALLET_ENABLE == 1) || ($vip_user == 1 && $passenger_wallet >= WALLET_ADVANCE_LIMIT && strtotime($this->currentdate) < $lateral_end_date_sec && WALLET_ENABLE == 1))
                                        {
                                            ...
                                            try {
                                                ...
                                                if($advance_payment == 1)
                                                {
                                                     ...
                                                     $insert_array['fare'] = (double)$used_wallet_amount;
                                                ...
                                                $update_query = array('wallet_amount_used'=>abs($used_wallet_amount));
...
unset(MangoDB::$instances['default']);
                                ...
                                if($update_taxi_info == 1){
                                        ...
                                        if (count($getTaxiforDriver) > 0) {
                                            ...
                                            $insert_array                 = array(
                                                ...
                                                'taxi_id' => (int)$taxi_id,
                                                'shift_start' => new MongoDate(strtotime($company_all_currenttimestamp)),
                                                'login_start' => new MongoDate(strtotime($company_all_currenttimestamp)),
                                                'shift_end' => null,
                                                'reason' => null,
                                                'createdate' => new MongoDate(strtotime($this->currentdate)),
            ...
            case 'get_recent_places':
                ...
                $update = array('travel_status'=>1,"payment_chosen_flag"=>1,'forceclose_status'=>1);
               ...
               // $get_passenger_log_details = $api->get_passenger_log_detail('249');
                ...
                if ($array['id'] != null) {
                    ...
                    if ($validator->check()) {
                        ...
                        if (count($result) > 0) {
                            ...
                            $wallet_block = isset($pass_result['wallet_block'])?$pass_result['wallet_block']:0;
                            /*************************** Passenger Image ************************************/
                            if ((!empty($passenger_image)) && file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . PASS_IMG_IMGPATH . 'thumb_' . $passenger_image)) {
                                $profile_image = URL_BASE . PASS_IMG_IMGPATH . $passenger_image;
                            } else {
                            ...
                            $pass_promo = $api->get_passenger_added_promocode($array['id']);

                            if(isset($pass_promo['status']) && $pass_promo['status']==1){
                                $pass_result['promo_code'] = $pass_promo['promo_code'];
                            }else{
                                $pass_result['promo_code'] = '';
                            }
                            ...
                            $pass_result['wallet_block'] = $wallet_block;
                        }
                        //$passengers_trips['pending_bookings'] = array_merge($pending_bookings,$pending_bookings_zero);
                        $passengers_trips    = $past_bookings;
                        if (count($passengers_trips) > 0) {
                ...
                if ($array['id'] != null) {
                    ...
                    if ($validator->check()) {
                        ...
                        $last_wallet_update =isset($wallet_detail->sec)?date('Y-m-d',$wallet_detail->sec):'';
                ...
                case 'past_booking_places':                
                $array = $mobiledata;
                //print_r($array);exit;
                $ids = array();
                if ($array['id'] != null) {

                        $current_lat = isset($mobiledata['latitude'])?$mobiledata['latitude']:'';
                        $current_lng = isset($mobiledata['longitude'])?$mobiledata['longitude']:'';
                        //$current_lat = '11.0317'; $current_lng = '77.0187';

                        if($current_lat!='' && $current_lng=!''){
                            $get_recent = $api->get_recent_trip_places($array['id'],$current_lat,$mobiledata['longitude']); 
                            //print_r($get_recent);exit;
                            //$get_recent = $api->calculate_distance();
                            $message = array("message" => __('success'),"detail"=>$get_recent,"status"=>1);

                        }else{

                            $message = array(
                                    "message" => __('current_location_missing'),
                                    "status" => -1
                                );
                        }                   
                ...
                if ($array['id'] != null) {
                        ...
                        $get_promo = $api->get_promo_codes();

                        if(count($get_promo)){

                            $result = array();

                            foreach($get_promo as $k=>$val){

                                $expiry_date      = commonfunction::convertphpdate('d-M-Y h:i:s A',$val['expiry_date']);
                                $get_promo[$k]['expiry_date'] = $expiry_date;
                            }                            

                            $message = array("message" => __('success'),"detail"=>$get_promo,"status"=>1);

                        }else{

                            $message = array("message" => __('no promocode available'),"status"=>-1);
                        }
                ...
                case 'check_promo_codes':                
                $array = $mobiledata;
                $promo_code = isset($array['promo_code'])?$array['promo_code']:'';
                $passenger_number = isset($array['phone_number'])?$array['phone_number']:'';
                $passenger_id = isset($array['id'])?$array['id']:'';
                ...
                if ($passenger_id!='' && $promo_code!='' ) {
                        ...
                        if ($check_promo == 0) {
                            ...
                            $msg = array(
                                "message" => __('promo_applied_successfully'),
                                "status" => 1
                            );
                        }
                    ...
                    $msg = array(
                        "message" => __('invalid_user_and_promocode'),
                        "status" => -1
                    );
                }
                echo json_encode($msg);
                ...
                if ($passenger_id!='' && $promo_code!='' ) {
                    ...
                    $check_promo = $api->check_already_promocode_added($passenger_id);

                    if($check_promo==2){

                        $msg = array(
                                    "message" => __('already_you_added_promocode'),
                                    "status" => -1
                                );
                    }else{
                        ...
                        if ($check_promo == 0) {
                            ...
                            $added = $api->add_passenger_promo($promo_code,$passenger_id);

                            if($added==1){

                                 $msg = array("message" => __('promo_applied_successfully'),"status" => 1);                                

                            }else{

                                $msg = array("message" => __('promocode_not_found'),"status" => -1);
                            }
                ...
                case 'added_promocode_list':
                $array = $mobiledata;                               
                $passenger_id = isset($array['id'])?$array['id']:'';
                if ($passenger_id!='') {

                    $promo_list = $api->get_added_promocodes($passenger_id);

                        if(count($promo_list)){

                            $message = array("message" => __('success'),"detail"=>$promo_list,"status"=>1);

                        }else{

                            $message = array("message" => __('no_promocode_available'),"status"=>-1);
                        }
                ...
                case 'get_passenger_notifications':
                    $notification_list = $api->get_passenger_notifications();
                    //print_r($notification_list);exit;
                    if(count($notification_list)){

                        foreach($notification_list as $k=>$notify){

                            $notification_list[$k] = $notify;

                            //echo $_SERVER["DOCUMENT_ROOT"].'/'.NOTIFY_IMG_IMGPATH.$notify['image'];exit;
                            if(isset($notify['image']) && file_exists($_SERVER["DOCUMENT_ROOT"].'/'.NOTIFY_IMG_IMGPATH.$notify['image']) && !empty($notify['image'])) 
                            {
                                $notification_list[$k]['image'] = URL_BASE.NOTIFY_IMG_IMGPATH.$notify['image'];
                            }else{
                                $notification_list[$k]['image'] = '';
                            }

                            $created_date = commonfunction::convertphpdate('Y-m-d H:i:s',$notify['created_date']);
                            $notification_list[$k]['created_date'] = $created_date;

                        }

                        $message = array("message" => __('push_messags'),"notifications"=>$notification_list,"status" => 1);

                    }else{                    

                        $message = array("message" => __('no_notification'),"status" => -1);
                    }
                    echo json_encode($message);
                    break;

                case 'passenger_update_drop_location':
                $array = $mobiledata;                               
                $trip_id = isset($array['trip_id'])?$array['trip_id']:'';
                $drop_location = isset($array['drop_location'])?$array['drop_location']:'';
                $drop_latitude = isset($array['drop_latitude'])?$array['drop_latitude']:'';
                $drop_longitude = isset($array['drop_longitude'])?$array['drop_longitude']:'';                
                if($trip_id!='' && $drop_location!='' && $drop_latitude !='' && $drop_longitude!='') {

                    $update_data = array('trip_id'=>$trip_id,'drop_location'=> urldecode($drop_location),'drop_latitude'=>$drop_latitude,'drop_longitude'=>$drop_longitude);
                    $res = $api->passenger_update_drop_location($update_data);

                    if($res==1){

                         $message = array("message" => __('drop_location_update_sucessfully'),"status" => 1);

                    }else if($res==2){

                         $message = array("message" => __('trip_not_found'),"status" => -1);

                    }else if($res==3){

                         $message = array("message" => __('drop_location_already_update'),"status" => -1);

                    }else{

                         $message = array("message" => __('connection_error'),"status" => -1);
                    }
                }else{
                     $message = array("message" => __('invalid_drop_location'),"status" => -1);
                }
                echo json_encode($message);
                break;
