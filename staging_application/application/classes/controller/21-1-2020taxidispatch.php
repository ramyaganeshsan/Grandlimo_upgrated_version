...
?>" id="no_passengers" />
	...
	$bk_type = isset($_GET['bk_t']) ? $_GET['bk_t'] : "";
		...
		$actpickup_time = ($listings['act_pickuptime'] != '')?commonfunction::convertphpdate('d-M-Y h:i:s A',$listings['act_pickuptime']):'';
                ...
                $zone_fare_applicable        = (!empty($listings['zone_fare_applicable'])) ? $listings['zone_fare_applicable'] : 0;
                ...
                $hourly_booking  = (!empty($listings['hourly_booking'])) ? $listings['hourly_booking'] : 0;
                $hourly_booking_hrs  = (!empty($listings['hourly_booking_hrs'])) ? $listings['hourly_booking_hrs'] : 0;
                $apply_estimate_fare  = (!empty($listings['apply_estimate_fare'])) ? $listings['apply_estimate_fare'] : 0;
                ...
                // }
               	//$time = ' - ';
                $tripDistance = isset($listings['tripDistance'])?$listings['tripDistance']:'';
                $distanceUnit = isset($listings['distanceUnit'])?$listings['distanceUnit']:'';
                ...
                if($drop_time != '' & $actpickup_time != '')
                {
                    ...
                    if($tripDistance && $tripDistance>0){
                        $tripDistance = number_format((float)$tripDistance, 2, '.', '');
                        $time .= '('.$tripDistance.' '.$distanceUnit.')';
                    }
                }
		...
		if(isset($listings['dispatch_time']) && $listings['dispatch_time']!=''){
                ...
                if ($listings['travel_status'] == 0) {
                  ...
                  //  $edit          = '<a href="javascript:;" class="edit-ico status ' . $trcolor . '" name="edit" id="addtr_' . $pass_logid . '" value="' . __('dispatch') . '" ><i class="glyphicon glyphicon-edit">&nbsp;</i></a>';
                    ...
                    $status_button = '<button type="submit" class="btn btn-outline btn-primary btn-xs inprogress" name="update_dispatch" id="update_dispatch_' . $pass_logid . '" value="' . __('dispatch') . '" >' . __('confirmed') . '</button><button type="submit" class="btn btn-outline btn-primary btn-xs update_dispatch" style="margin-top:5px" name="update_dispatch" id="update_dispatch_' . $pass_logid . '_' . $this->company_id . '" value="' . __('dispatch') . '" >' . __('reassign') . '</button>';
                 //   $edit          = '<a href="javascript:;" class="edit-ico status ' . $trcolor . '" name="edit" id="addtr_' . $pass_logid . '" value="' . __('dispatch') . '" ><i class="glyphicon glyphicon-edit">&nbsp;</i></a>';
                    $cancel        = '<a href="javascript:;" class="remove-ico status cancelBtn" name="cancel" id="cancel_' . $pass_logid . '_'.$driver_id.'" value="' . __('dispatch') . '" ><i class="glyphicon glyphicon-remove">&nbsp;</i></a>';
                ...
                if ($driver_name != "") {
                    ...
                    if($passenger_id != '') {
                        $taxi_model_id = isset($listings['model_id'])?$listings['model_id']:0;
                        $passenger_last_four_drivers ='<a href="javascript:void(0)" class="view_last_four_drivers" title="Click to view last four drivers" id="passengerdrivers_'.$passenger_id.'_'.$pass_logid.'_'.$taxi_model_id.'" style="color:#000;text-decoration:none;cursor:default;" ><img src="' . URL_BASE . 'public/images/passenger-notes.png"></a>';
                    }
                ...
                if($listings['book_tag'] == "N") {
                    ...
                    if($zone_fare_applicable == 1)
                    {
                        $class  = "style='color: #f91313;'";
                        $link   = __('zone').'<br>'; 
                    } 
                    else if($airport_pickup == 1)
                    {
                        ...
                        if($hourly_booking==1 && $hourly_booking_hrs){
                            $link = $link.'-'.$hourly_booking_hrs.'hrs';
                        }else if($apply_estimate_fare==1){
                            $link = $link.' (Estimate fare applied)';
                        }
    ...
    public function action_all_booking_list_manage_all()
    {
        ...
        if (count($get_all_booking_list) > 0) {
            foreach ($get_all_booking_list as $listings) {
                ...
                $approx_distance = number_format((float)$approx_distance, 2, '.', '').' KM';
		   ...
		   if(isset($listings['dispatch_time']) && $listings['dispatch_time']!=''){
                ...
                if ($listings['travel_status'] == 0) {
                    ...
                    $tr_icon_title = __('confirmed');
                    $travel_status = '<div style="color:' . $status_color . ';">' . __('confirmed') . '</div>';
                    $status_button = '<button type="submit" class="btn btn-outline btn-primary btn-xs inprogress" name="update_dispatch" id="update_dispatch_' . $pass_logid . '" value="' . __('dispatch') . '" >' . __('confirmed') . '</button><button type="submit" class="btn btn-outline btn-primary btn-xs update_dispatch" name="update_dispatch" id="update_dispatch_' . $pass_logid . '_' . $this->company_id . '" value="' . __('dispatch') . '" >' . __('reassign') . '</button>';
                    $edit          = '<button type="submit" class="status ' . $trcolor . '" name="edit" id="addtr_' . $pass_logid . '" value="' . __('dispatch') . '" ></button>';
                } elseif ($listings['travel_status'] == 9 && ($listings['driver_reply'] == 'C' || $listings['driver_reply'] == 'R')) {
                    $status_color  = "#844d06";
                    $tr_icon_class = "cancel_icon";
...
public function action_pay_details()
    ...
    public function action_get_past_places(){
        ...
        if(count($res)){
            ...
            foreach($res as $val){
                ...
                $select_field .= '<option data-lat="'.$val['latitude'].'" data-lng="'.$val['longitude'].'" value="'.$val['place'].'">'.$val['place'].'</option>';
            }
        }
    ...
    public function action_get_last_four_drivers_list(){
        ...
        $pass_id = isset($_REQUEST["id"])?$_REQUEST["id"]:'';
        $trip_id = isset($_REQUEST["trip_id"])?$_REQUEST["trip_id"]:'';
        $model_id = isset($_REQUEST["model_id"])?$_REQUEST["model_id"]:'';


        $res = $this->tdispatch_model->get_last_four_drivers_list($pass_id,$trip_id,$model_id);
        echo $res;exit;        
    }
