    ...
    public function action_consolidated_driver_report()
    {
       ...
       // $availabilitycount         = $this->add_model->validate_packagedriver($cid);
        ...
        if (isset($_REQUEST['keyword']))
        {
            ...
            $all_shift_logs = $this->transaction_model->driver_shift_logs(trim(Html::chars($_REQUEST['keyword'])),'',$start_date,$end_date);
        }
        else
        {
            $all_company_list = $this->transaction_model->consolidated_sales_report('','',$start_date,$end_date,'', $offset, REC_PER_PAGE);

            $all_shift_logs = $this->transaction_model->driver_shift_logs('','',$start_date,$end_date);
        }
