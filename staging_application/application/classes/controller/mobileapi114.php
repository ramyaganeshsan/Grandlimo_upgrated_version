						   ...
						   if($history_validator->check())
						   {
								...
								if($location_array['driver_id']!=""){
									...
									if(count($driver_request_details) > 0){
										$msg = array("message" => __('passenger_trip_cancelled'),"status"=>10);
										$update_driver_request_array  = array("notification_status" => '5'); // Passenger Cancelled
								...
								if($driver_id != '') {
										 $driver_request_details = $api->get_dispatcher_cancel_data($driver_id,$default_companyid);
										  if(count($driver_request_details) == 1)
											{
												$msg = array("message" => __('dispatcher_trip_cancelled'),"device_id"=>$device_token,"status"=>10);
												$update_driver_array  = array("notification_status" => '5');
								...
								else if($driver_status == 'A')
								{
									...
									if($result[0] == 1)
									{
										$msg = array("message" => __('driver_history_updated'),"status" => 1,"distance"=>$distance);	
									}
									else if($result == -1)
									{
							...
							if($logout_notification==1 && $logout_status=="N" && $admin_logout_status == 0){
								$msg = array("message" => __('driver_logout_via_admin'),"status"=>15,"display"=>1);
								$update_notification_status  = array("notification_status"=>'0');
								...
								$msg = array("message" => __('driver_assigned_expired'),"status"=>15,"display"=>1);
								$update_notification_status  = array("notification_status"=>'0');
								$api->update_table(PEOPLE,$update_notification_status,'id',$location_array['driver_id']);
							}else{
								$msg = array("message" => __('driver_not_login'),"status"=>15,"display"=>0);
							}
						}
					}
					else
			...
			case 'invite_with_referral':
				...
				if(!empty($passenger_id)) {
					...
					if(count($passengerReferral) > 0) {
						$detail = array("referral_code" => $passengerReferral[0]['referral_code'],"referral_amount" => $passengerReferral[0]['referral_code_amount']);
			...
			case 'savebooking':
					...
					if($promo_code != "")
					{
						...
						{							
							...
							$passengerInTrip = $api->check_passenger_in_trip($passenger_id,$company_id);
							if($passengerInTrip > 0) {
								$msg = array("message" => __('passenger_in_journey'),"status" => 3);
								echo json_encode($msg);
								break;
							}	
							...
							if($notification_time != 0 ){ $timeoutseconds = $notification_time;}else{$timeoutseconds = 15;}
							//Form Values//
									$formvalues = Arr::extract($mobiledata, array('pickupplace','dropplace','pickup_time','driver_id','passenger_id','roundtrip','passenger_phone','cityname','distance_away','sub_logid','drop_latitude','drop_longitude','promo_code','now_after','motor_model'));
							if($total_count > 0)
							{										
			...
			case 'nearestdriver_list':
					...
					if($validator->check())
					{
						if($search_array['latitude'] !='0' && $search_array['longitude'] !='0')
						{
							...
							if($total_count > 0)
							{
								...
								//$driver_details['city_id'] = $city_id;								
									if(count($driver_details) > 0)
										$message = array("detail" => $driver_details,"fare_details"=>$fare_details,"driver_around_miles"=>DEFAULT_DRIVER_MILE,"status" => 1,"message" => 'success','metric'=>UNIT_NAME);
									else
										$message = array("message" => $no_vehicle_msg,"fare_details"=>$fare_details,"driver_around_miles"=>DEFAULT_DRIVER_MILE,"status" => 0);	
						  ...
						  {
							  $msg = array("message" => $no_vehicle_msg,"fare_details"=>$fare_details,"status" => 3);
							  echo json_encode($msg);
							  exit;							  
						  }
					  }
			...
			$driver_id = $mobiledata['driver_id'];			
					if($driver_id != null)
					{
						...
						if(count($driver_current_status) > 0)
						{
								...
								{
									$update_array  = array("login_from"=>"","login_status"=>"N","device_id" => "","device_token" => "","device_type" => "","notification_setting"=>"0","notification_status"=>"0");
					...
					if($driver_list_array['driver_id'] != null)
					{
						...
						{						
							...
							*/
							...
							$detail = array("pending_booking"=>$driver_pending_bookings,"past_booking"=>$pending_booking);
							...
							if($request_type == 0)
							{
								...
								if(count($get_passenger_log_det) == 1)
								{
									...
									if(($driver_reply == 'A') && ($travel_status == 9))
									{
										...
										$message = array("message" => __("request_confirmed_passenger"),"detail"=>$detail,"driver_latitute"=>$driver_latitute,"driver_longtitute"=>$driver_longtitute,"status"=>1);
									}
									...
									{										
										...
										$message = array("message"=>__('passenger_on_board'),"trip_id"=>$trip_id,"driver_latitute"=>$driver_latitute,"driver_longtitute"=>$driver_longtitute,"status"=>2,"display"=>$arrived_display);
									...
									elseif(($driver_reply == 'A') && ($travel_status == 2))
									{
										...
										//$update_trip_array  = array("status"=>'4');
										//$result = $api->update_table(DRIVER_REQUEST_DETAILS,$update_trip_array,'trip_id',$passenger_tripid);		
										$message = array("message" =>__('journey_started'),"pickup_time"=>$actual_pickup_time,"trip_id"=>$trip_id,"driver_status"=>$trip_status,"driver_latitute"=>$driver_latitute,"driver_longtitute"=>$driver_longtitute,"status"=>3,"display"=>$tripstart_display);	
										$update_trip_array  = array("notification_status"=>'2');
										$result = $api->update_table(PASSENGERS_LOG,$update_trip_array,'passengers_log_id',$trip_id);										
									}
									...
									elseif(($driver_reply == 'A') && ($travel_status == 1) && $transId != 0)
									{
										...
										$message = array("message" => __('trip_fare_updated'),"fare" => $amt,"trip_id"=>$trip_id,"pickup" => $pickup_location, "status"=>5,"display"=>$tripfare_update_display,"driver_status"=>$trip_status,"driver_latitute"=>$driver_latitute,"driver_longtitute"=>$driver_longtitute);	
							...
							if(count($current_driver_status) > 0)
							{
												...
												{								
													...
													/*********************************************************************************/
														...
														$update_passenger_log_array = array('travel_status' => $travel_status,'actual_pickup_time'=>$actual_pickup_time,'current_location'=>$act_pickup_location);									
														...
														$trip_details = array("pickup_location"=>$act_pickup_location,"drop_location"=>$drop_location);
							...
							{ 
							   ...
							   /********Minutes fare calculation *******/
								...
								if($travel_status != 5) {//condition checked to avoid amount detection while trip is in waiting for payment status
									...
									$update_usedAmount_array = array("used_wallet_amount" => $usedAmount);
