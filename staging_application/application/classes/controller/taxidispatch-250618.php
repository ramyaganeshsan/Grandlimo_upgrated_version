    ...
    public function action_driver_list_with_status()
    {
        ...
        if (count($all_company_map_list) > 0) {
            ...
            foreach ($all_company_map_list as $key => $val) {
                ...
                $driver_page = URL_BASE . "manage/driverinfo/" . $val['driver_id'];
                $output .= "<li class=" . $class . ">";
                $output .= "<span><a href='" . $driver_page . "' target='_blank' title='".$val['taxi_no']."'>" . ucfirst($val['name']). "</a> - ". (isset($val['driver_code']) ? $val['driver_code'] : "")."<p style='color:GoldenRod;'>".$val['model_name']."</p>"."<p style='color:".$gps_color.";'>GPS :".$gps_enable."</p></span>";
