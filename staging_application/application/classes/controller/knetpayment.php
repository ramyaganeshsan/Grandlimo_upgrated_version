...
class Controller_Knetpayment extends Controller_Website
{
    ...
    public function action_tap_success()
    {
        ...
        $api_model = Model::factory('mobileapi120');
        ...
        $used_wallet_amount = isset($passenger_log_details[0]['wallet_amount_used'])?$passenger_log_details[0]['wallet_amount_used']:0;
        $passenger_pending_amt = isset($passenger_log_details[0]['passenger_pending_amt'])?$passenger_log_details[0]['passenger_pending_amt']:0;
        ...
        if ($_GET['Result'] == 1) {
            ...
            "minutes_fare" => isset($tempdata['minutes_fare']) ? $tempdata['minutes_fare'] : 0,
                ...
                "driver_edit_status" => isset($tempdata['driver_edit_status']) ? $tempdata['driver_edit_status'] : 0,
                "actual_paid_amt" => isset($tempdata['actual_paid_amt']) ? $tempdata['actual_paid_amt'] : 0,
                "pending_amt" => isset($tempdata['pending_amt']) ? $tempdata['pending_amt'] : 0,
                "passenger_pending_amt" => $passenger_pending_amt,
                "fare_update_status"=>(int)0
            );
            ...
            $pending_amt = isset($tempdata['pending_amt']) ? $tempdata['pending_amt'] : 0;
            //$passenger_pending_amt = isset($tempdata['passenger_pending_amt']) ? $tempdata['passenger_pending_amt'] : 0;
            $driver_edit_status = isset($tempdata['driver_edit_status']) ? $tempdata['driver_edit_status'] : 0;


            if($used_wallet_amount > 0)
            {
               // $passenger_id = isset($passenger_log_details[0]['passengers_id']) ? $passenger_log_details[0]['passengers_id'] : '';
                $update_wallet = $api_model->update_wallet_without_log($passengers_id,$used_wallet_amount,1,$passenger_log_id);
            }
            ...
            if($passenger_pending_amt > 0)
            {
                ...
                $update_wallet = $api_model->update_wallet_without_log($passengers_id,$passenger_pending_amt,0,$passenger_log_id,6);
            }
            ...
            if($pending_amt > 0)
            {
                ...
                {
                //$passenger_id = isset($passenger_log_details[0]['passengers_id']) ? $passenger_log_details[0]['passengers_id'] : '';
                $update_wallet = $api_model->update_wallet_without_log($passengers_id,$pending_amt,1,$passenger_log_id,5);
                }
            }
            ...
            $payment_array =array(
                ...
                $api_model->payentry($passenger_log_id,$payment_array);

                 $update_complete = $api_model->update_complete_trip_det($passenger_log_id);
    ...
    public function action_knet_wallet_response(){
        ...
        if ( $presult != 0 )
        {
            $result_url = URL_BASE."wallet_success.html";              
            $result_params = "?PaymentID=" . $PaymentID . "&Result=" . $presult . "&PostDate=" . $postdate . "&TranID=" . $tranid . "&Auth=" . $auth . "&Ref=" . $ref . "&TrackID=" . $trackid ."&UDF1=" . $udf1 . "&UDF2=" .$udf2  . "&UDF3=" . $udf3  . "&UDF4=" . $udf4 . "&UDF5=" . $udf5 . "&token=". $token;
        }
        else
        {
            $result_url = URL_BASE."wallet_error.html";
            $result_params = "?PaymentID=" . $PaymentID . "&Result=" . $presult . "&PostDate=" . $postdate . "&TranID=" . $tranid . "&Auth=" . $auth . "&Ref=" . $ref . "&TrackID=" . $trackid ."&UDF1=" . $udf1 . "&UDF2=" .$udf2  . "&UDF3=" . $udf3  . "&UDF4=" . $udf4 . "&UDF5=" . $udf5 . "&token=". $token;
        }           
    ...
    public function action_wallet_success()
    {
        $api_model = Model::factory('mobileapi120');
        $PaymentID = isset($_REQUEST['PaymentID'])?$_REQUEST['PaymentID']:0;
        $postdate = isset($_REQUEST['PostDate'])?$_REQUEST['PostDate']:0;
        $tranid = isset($_REQUEST['TranID'])?$_REQUEST['TranID']:0;
        $auth = isset($_REQUEST['Auth'])?$_REQUEST['Auth']:0;
        $presult = isset($_REQUEST['Result'])?$_REQUEST['Result']:0;
        $ref = isset($_REQUEST['Ref'])?$_REQUEST['Ref']:0;
        $trackid = isset($_REQUEST['TrackID'])?$_REQUEST['TrackID']:0;
        $id = isset($_REQUEST['UDF2'])?$_REQUEST['UDF2']:0;
        $add_amt = isset($_REQUEST['UDF1'])?$_REQUEST['UDF1']:0;

        if($PaymentID != 0 && $id != 0)
        {
            $update_wallet = $api_model->update_wallet_without_log($id,$add_amt,0,$PaymentID,8);
