...
?>
    ...
    public function action_get_citymodel_fare_details()
    {
        ...
        $drop_lat = isset($_REQUEST['drop_lat'])?$_REQUEST['drop_lat']:'';
        $drop_lng = isset($_REQUEST['drop_lng'])?$_REQUEST['drop_lng']:'';
        $pickup_lat = isset($_REQUEST['pickup_lat'])?$_REQUEST['pickup_lat']:'';
        $pickup_lng = isset($_REQUEST['pickup_lng'])?$_REQUEST['pickup_lng']:'';
        ...
        {
            $zone_pickup    = $this->tdispatch_model->check_zones($pickup_lat,$pickup_lng,false);
            $airport_pickup    = $this->tdispatch_model->get_airport_zone($pickup_lat,$pickup_lng);
        }
 
        if($drop_lat !='' && $drop_lng != '')
        {
            $zone_drop    = $this->tdispatch_model->check_zones($drop_lat,$drop_lng,true);
            $airport_drop    = $this->tdispatch_model->get_airport_zone($drop_lat,$drop_lng);
        }

        if(count($zone_pickup) > 0)
        {
            $zone_id = isset($zone_pickup['_id'])?$zone_pickup['_id']:0;
            if($zone_id != 0){
                $zone_fare    = $this->tdispatch_model->get_zone_fare($zone_id,$model_id);
                if($zone_fare > 0) {
        ...
        if(count($zone_drop) > 0)
        {
            $zone_id = isset($zone_drop['_id'])?$zone_drop['_id']:0;
            if($zone_id != 0){
            $zone_drop_fare    = $this->tdispatch_model->get_zone_fare($zone_id,$model_id);
                if($zone_drop_fare > 0) {
                    $zone_fare_applicable = 1;
                    $zone_fare_type =2;
                }
        ...
        if($airport_drop || $airport_pickup)
        {
            $airport_trip = 1;
            $airport_type = ($airport_drop)?2:(($airport_pickup)?1:0);
        }
        ...
        $cancellation_fare    = isset($taxi_fare_details['cancellation_fare'])?$taxi_fare_details['cancellation_fare']:0;
        $minutes_fare         = $taxi_fare_details['minutes_fare'];
        $min_km        		  = $taxi_fare_details['min_km'];
        $min_fare             = $taxi_fare_details['min_fare'];
        $below_above_km             = $taxi_fare_details['below_above_km'];
        $below_km             = $taxi_fare_details['below_km'];
        $above_km             = $taxi_fare_details['above_km'];
        $base_fare             = $taxi_fare_details['base_fare'];
        $airport_pickup_fare  = $taxi_fare_details['airport_pickup_fare'];
        $airport_drop_fare    = $taxi_fare_details['airport_drop_fare'];
        ...
        if (FARE_CALCULATION_TYPE == 1) {
			if ($distance <= $min_km) {
				$total_fare = $min_fare;
			} else if ($distance < $below_above_km) {
				//$total_fare = $distance * $below_km;
                $total_fare = ($distance-$min_km) * $below_km;
				$total_fare = $total_fare + $base_fare;
			} else if ($distance > $below_above_km) {
				//$total_fare = $distance * $above_km;
                $total_fare = ($distance-$min_km) * $above_km;
				$total_fare = $total_fare + $base_fare;
			}
        }

        if($zone_fare_applicable == 1 && $zone_fare_type == 1 && $zone_fare > 0){
            $total_fare = $zone_fare;   
        }

        if($zone_fare_applicable == 1 && $zone_fare_type == 2  && $zone_drop_fare > 0){
            $total_fare = $zone_drop_fare;   
        }

        if($airport_type == 1) {
            $total_fare = $airport_pickup_fare;
        }

        if($airport_type == 2) {
            $total_fare = $airport_drop_fare;
        }

        echo $total_fare."-".$airport_trip."-".$airport_type."-".$zone_fare_applicable."-".$zone_fare_type;
        exit;
    }
