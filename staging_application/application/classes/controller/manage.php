...
class Controller_Manage extends Controller_Siteadmin
{
    ...
    public function __construct(Request $request, Response $response)
    {
		...
		$this->passenger_model      = Model::factory('passengers');
	...
	public function action_taxiinfo()
    {
		...
		$count_taxi_logs_completed_transaction = 0;
		//echo '<pre>';
    ...
    public function action_assigntaxi()
    {
        ...
        $all_company_list           = $this->manage_model->all_assigntaxi_list($offset, 100);
        //****pagination ends here***//
        $get_allcompany             = $this->manage_model->get_allcompany('A');
        $details                    = '';
        //send data to view file 
...
?>],
    ...
    public function action_delete_promocode()
    {
        $id          = $this->request->param('id');
        //echo $id;exit;
        $promocode        = $this->manage_model->delete_promocode($id);
        if ($promocode) {
            Message::success(__('Promocode was deleted.'));
 ...
 public function action_ratings()
 ...
 
    {
        ...
        // {
            ...
            $all_company_list = $this->manage_model->passenger_wallet_logs(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
        // }
        // else
        // {
        ...
        // }
            ...
            {
                $all_company_list = $this->manage_model->passenger_wallet_logs(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {
                    if($val['change_amount'] > 0)
                    {
                    $xls_output[$key]['amount_credit'] =round($val['change_amount'],3);
                    }
                    else
                    {
                    ...
                    if($val['change_amount'] < 0)
                    {
                    $xls_output[$key]['amount_debit'] =round($val['change_amount'],3);
                    }
                    else
                    {
                    ...
                    if($val['recharge_type'] == 4 || $val['recharge_type'] == 5)
					 {
						$xls_output[$key]['trip_fare'] = isset($val['tripfare'])?$val['tripfare']:'-';
					 }else{
						$xls_output[$key]['trip_fare'] = '-';
					 }
                    

                    /* Sasidharan Oct 01 2024 */
                    $fromTap = isset($val['fromTap']) ? $val['fromTap'] : 0;
                    $initiated_payment = isset($val['initiated_payment']) ? $val['initiated_payment'] : 0;
                    $tapPaymentGayeway = isset($val['tapPaymentGayeway']) ? $val['tapPaymentGayeway'] : 0;
                    $paid_status = isset($val['paid_status']) ? $val['paid_status'] : 0;
                    $paymentToken = (isset($val['paymentToken']) && !empty($val['paymentToken'])) ? $val['paymentToken'] : "";
                    $dateTime = new DateTime('2024-09-07 00:00:00');
                    $logDate = new DateTime(Commonfunction::convertphpdate('Y-m-d h:i:s',$val['created_date']));
                    $isValidDate = $logDate > $dateTime;
                    ...
                    switch($val['recharge_type'])
                    {
                        ...
                        case 8:
                            ...
                            /* Sasidharan Oct 01 2024 */
                            if($fromTap == 1) {
                                $recharge_type = "Tap Payment";
                            } else if($paid_status && $paymentToken == "" && $isValidDate){
                                $recharge_type = "Tap Payment";
                            }
                        break;
                        case 9:
                        /* June 8 2023 */
                        case 12:
                        $recharge_type = __('redeem_points');
                        break;
                        default:
                            $recharge_type = __('manual_pay'); 
                            /* Sasidharan Oct 01 2024 */
                            if($initiated_payment && $tapPaymentGayeway) {
                                $recharge_type = "Tap Payment Initiated";
                            }
                        break;
                    }
                ...
                $export_table_header = array(__('passenger_name'),__('passenger_phone'),__('current_wallet_amt'),__('amount_credit'),__('amount_debit'),__('recharge_type'),'Trip fare',__('trip_id'),__('redeem_points'),__('notes'),__('created_date'));
                $export_table_field_select = array('passenger_name','passenger_phone','wallet_amount','amount_credit','amount_debit','recharge_type','trip_fare','trip_id','redeem_points','notes','created_date');
    ...
    public function action_update_passenger_wallet()
    {
        ...
        if ($validator->check()) {
            ...
            $wallet_type = isset($data['wallet_type'])?(int)$data['wallet_type']: 1;

            $update_wallet = $this->manage_model->update_wallet_amount($data['passenger'],$data['amount'],$negative,$notes,$wallet_type);
            if($update_wallet > 0)
            {
    ...
    public function action_firstname_load_new()
    {
        ...
        foreach ($user_details as $key => $details) {
            ...
            /* Sasidharan June 17 2023 */
            $name[$i]['discount_wallet'] = isset($details['discount_wallet'])?CURRENCY." ".$details['discount_wallet']:CURRENCY." 0";
     ...
     public function action_vip_payment_invoice()
    {
        ...
        // }
            ...
            if(isset($_SESSION['download_set']))
            {
                 $all_company_list = $this->manage_model->vip_pending_payments(trim(Html::chars($keyword)), trim(Html::chars($status)), $company,0,1000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {
                    if($val['pay_by'] == 1)
                    {
                        $xls_output[$key]['pay_by'] =  __('weekly_label');
                    }
                    else
                    {
                         $xls_output[$key]['pay_by'] =  __('monthly_label');
                    }

                    $xls_output[$key]['last_payment_update'] = Commonfunction::convertphpdate('Y-m-d h:i:s A',$val['last_payment_update']);
    ...
    public function action_normal_payment_invoice()
    {
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->normal_pending_payments(trim(Html::chars($keyword)), trim(Html::chars($status)), $company,NULL,NULL,TRUE);
        }
        else
        {
        ...
        // }
         ...
         if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->normal_pending_payments(trim(Html::chars($keyword)), trim(Html::chars($status)), $company,0,1000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {
                    if($val['pay_by'] == 1)
 ...
 public function action_delete_id_image()
    {
       $id= $this->request->param('id');
       $this->manage_model->delete_id_image($id);
       Message::success('Id Image Deleted');
       $this->request->redirect("/manage/passengerinfo/".$id);
    }
    ...
    public function action_wallet_transactions() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('passenger_wallet_transactions');
        $this->selected_page_title = __('passenger_wallet_transactions');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->passenger_wallet_transactions(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->passenger_wallet_transactions('','','','','','',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
            ...
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->passenger_wallet_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
      
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->passenger_wallet_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {
                    if($val['change_amount'] > 0)
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/passenger_wallet_transactions')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }
    
     // Accounts wallet data
    public function action_customer_wallet_transactions() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('passenger_wallet_report');
        $this->selected_page_title = __('passenger_wallet_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->customer_wallet_transactions(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->customer_wallet_transactions('','','','','','',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
            ...
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->customer_wallet_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
      
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->customer_wallet_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {                    
                    $xls_output[$key]['balance_wallet_amount'] = round($val['balance_wallet_amount'],3);
                    $xls_output[$key]['total_credit'] = round($val['total_credit'],3);
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/passenger_wallet_list')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }
    
    // Accounts wallet data
    public function action_pending_customer_transactions() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('pending_customers_report');
        $this->selected_page_title = __('pending_customers_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->pending_customer_transactions(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->pending_customer_transactions('','','','','','',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
            ...
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->pending_customer_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
      
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->pending_customer_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,4000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {                    
                    $xls_output[$key]['balance_wallet_amount'] = round($val['balance_wallet_amount'],3);
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/pending_customers')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }

    // Accounts wallet data
    public function action_pending_customer_trips() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('paid_pending_customers_report');
        $this->selected_page_title = __('paid_pending_customers_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->pending_customer_trips(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->pending_customer_trips('','','','','','',NULL,NULL,TRUE);  
        }
        //echo $count_company_list.'-----------';exit;
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
            if ($page_no == 0 || $page_no == 'index')
            ...
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->pending_customer_trips(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
      
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->pending_customer_trips(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                ...
                {    
                     ...
                     $xls_output[$key]['trip_id'] = $trip_id; 
                    ...
                    $xls_output[$key]['cash'] = $cash;
                    $xls_output[$key]['knet'] = $knet;
                    $xls_output[$key]['online'] = $online;
                    $xls_output[$key]['wallet'] = $wallet;
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/pending_customers_trips')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }
    
    
    public function action_pending_customer_trips_record() 
    {
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->pending_customer_trips_record(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->pending_customer_trips_record('','','','','','',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
        ...
        // {
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->pending_customer_trips_record(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
        // }
        // else
        // {
        //     $all_company_list = $this->manage_model->passenger_wallet_logs('','','','','','', $offset, REC_PER_PAGE);
        // }
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->pending_customer_trips_record(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {
                    if($val['change_amount'] > 0)
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/passenger_wallet_logs_pending')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }
    
    
    public function action_dispatcher_performance(){

        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('dispatcher_performance');
        $this->selected_page_title = __('dispatcher_performance');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
       ...
       // $availabilitycount         = $this->add_model->validate_packagedriver($cid);
        ...
        $start_date = isset($_REQUEST['start_date'])?$_REQUEST['start_date']:date('Y-m-d 00:00:00',strtotime($current_time));
        $end_date = isset($_REQUEST['end_date'])?$_REQUEST['end_date']:date('Y-m-d 23:59:59',strtotime($current_time));
        $search_date = isset($_REQUEST['search_date'])?$_REQUEST['search_date']:date('Y-m-d',strtotime($current_time));
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->dispatcher_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->dispatcher_report('','',$start_date,$end_date,'',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
        ...
        if (isset($_REQUEST['keyword']))
        {
            $all_company_list = $this->manage_model->dispatcher_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date, $company, $offset, REC_PER_PAGE);
        }
        else
        {
            $all_company_list = $this->manage_model->dispatcher_report('','',$start_date,$end_date,'', $offset, REC_PER_PAGE);
        }
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/report/dispatcher_performance')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;

    }
    public function action_view_dispatcher_perform(){
       ...
       // $availabilitycount         = $this->add_model->validate_packagedriver($cid);
        ...
        $start_date = isset($_REQUEST['from'])?$_REQUEST['from']:date('Y-m-d 00:00:00',strtotime($current_time));
        $end_date = isset($_REQUEST['to'])?$_REQUEST['to']:date('Y-m-d H:i:s',strtotime($current_time));
        $id = isset($_REQUEST['id'])?$_REQUEST['id']:date('Y-m-d',strtotime($current_time));
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->view_dispatcher_report($id,trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->view_dispatcher_report($id,'','',$start_date,$end_date,'',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
        ...
        if (isset($_REQUEST['keyword']))
        {
            $all_company_list = $this->manage_model->view_dispatcher_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date, $company, $offset, REC_PER_PAGE);
        }
        else
        {
            $all_company_list = $this->manage_model->view_dispatcher_report($id,'','',$start_date,$end_date,'', $offset, REC_PER_PAGE);
        }
        ...
        if(isset($_SESSION['download_set']))
        {
            ...
            $this->action_dispatcher_performance_export($xls_output,$export_table_header,$export_table_field_select,$heading,$total_fare,$sub_heading,$start_date,$end_date);
        }
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/report/dispatcher_performance_view')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;

    }
    
    public function action_ratingpassengers()
    {
        $user_createdby            = $this->user_createdby;
        $usertype                  = $this->usertype;
        $this->page_title          = __('manage_rating_passenger');
        $this->selected_page_title = __('manage_rating_passenger');
        
        $count_rating_drivers      = $this->manage_model->all_rating_passengers('',null,null,true);
        //pagination loads here
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
            $page_no = PAGE_NO;
        ...
        $get_rate_company           = $this->manage_model->get_rating_company();
        //print_r($all_rating_drivers);exit;
        //****pagination ends here***//
        //send data to view file 
        //echo $count_rating_drivers;exit;
        $view                       = View::factory('admin/managerating_passengers')->bind('all_rating_drivers', $all_rating_drivers)->bind('pag_data', $pag_data)->bind('ListCount', $count_rating_drivers)->bind('get_rate_company', $get_rate_company)->bind('Offset', $offset);
        $this->template->title      = SITENAME . " | " . __('manage_rating_passenger');
        $this->template->page_title = __('manage_rating_passenger');
        $this->template->content    = $view;
    }
    public function action_ratingpassengers_search()
    {
        $user_createdby            = $this->user_createdby;
        //Page Title
        $this->page_title          = __('manage_rating_taxi');
        ...
        if ($_REQUEST) {
            ...
            if((!empty($keyword) && isset($search_post)) || !empty($company_id)){
                ...
                Message::error('Your search keyword must not be empty');
                $this->request->redirect('manage/ratingpassengers');
            }
        }
        $get_rate_company           = $this->manage_model->get_rating_company();
        //set data to view file 
        $view                       = View::factory('admin/managerating_passengers')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('pag_data', $pag_data)->bind('srch', $_REQUEST)->bind('ListCount', $count_rating_drivers)->bind('get_rate_company', $get_rate_company)->bind('all_rating_drivers', $all_rating_drivers);
        $this->template->page_title = __('manage_rating_drivers');
        $this->template->content    = $view;
    }
    public function action_managerating_passengersview()
    {
        $user_createdby            = $this->user_createdby;
        $usertype                  = $this->usertype;
        $uid                       = $this->request->param('id');
        $this->page_title          = __('rating_passengers');
        $this->selected_page_title = __('rating_passengers');
        $count_rating_drivers      = $this->passenger_model->get_passenger_logs1($uid, 'R', 'A', '1',null,null,true);
        $driver_profile            = $this->passenger_model->get_my_profile_details_by_id($uid);
        if (count($driver_profile) == 0) {
            $this->request->redirect("manage/ratingdrivers");
        }
        //pagination loads here
        ...
        $this->template->title      = SITENAME . " | " . __('rating_passengers');
        $this->template->page_title = __('rating_passengers');
        $this->template->content    = $view;
    }
    
    public function action_trips_ratings_drivers()
    {
        $user_createdby            = $this->user_createdby;
        $usertype                  = $this->usertype;
        $this->page_title          = __('manage_ratings_list');
        $this->selected_page_title = __('manage_ratings_list');
        ...
        $count_rating_drivers      = $this->manage_model->get_all_rating_drivers($keyword,$start_date,$end_date,null,null,true);
        //echo $count_rating_drivers;exit;
        //pagination loads here
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
        ...
        $all_rating_drivers         = $this->manage_model->get_all_rating_drivers($keyword,$start_date,$end_date,$offset, REC_PER_PAGE);
        
        $get_rate_company           = array();//$this->manage_model->get_rating_company();
        

        if(isset($_SESSION['download_set']))
        {
            $all_company_list = $this->manage_model->get_all_rating_drivers($keyword,$start_date,$end_date,0,3000);
            $xls_output = $all_company_list;
            foreach($all_company_list as $key => $listings)
            {
                 $rate = isset($listings['rating_points']) ? trim($listings['rating_points']) : 0;
                 $comments = isset($listings['comments']) ? $listings['comments'] : '-';
                 $driver_tags = isset($listings['driver_tags']) ? $listings['driver_tags'] :array();

                 $ratings_title = isset($listings['ratings_title']) ? trim($listings['ratings_title']) :'';
                 ...
                 if($driver_tags == '' ){ $driver_tags = array(); }

                 $rating_tags = implode(",",$driver_tags);

                 $created_date = commonfunction::convertphpdate("d-M-Y",$listings['actual_pickup_time']);

                 if($rate && $rate >0 ){
                    $rating_data = $ratings_title.'<br>'.wordwrap(number_format($rate,1)." Out of 5");
                 }else{
                    $rating_data = '-';
                 }

                $xls_output[$key]['trip_id'] = $trip_id;
                $xls_output[$key]['passenger_name'] = $passenger_name;
                $xls_output[$key]['passenger_phone'] = $listings['passenger_phone'];
                $xls_output[$key]['driver_name'] = $driver_name;
                $xls_output[$key]['driver_phone'] = $listings['driver_phone'];
                $xls_output[$key]['rating_data'] = $rating_data;
                $xls_output[$key]['rating_tags'] = $rating_tags;
                $xls_output[$key]['comments'] = $comments;
                $xls_output[$key]['created_date'] = commonfunction::convertphpdate("d-M-Y",$listings['actual_pickup_time']);
            }

            $export_table_header = array(__('trip_id'),__('passenger_name'),__('passenger_phone'),__('driver_name'),__('driver_phone'),__('rating_points'),__('attributes'),__('comments'),__('created_date'));
            $export_table_field_select = array('trip_id','passenger_name','passenger_phone','driver_name','driver_phone','rating_data','rating_tags','comments','created_date');

            $heading = __('manage_ratings_list');
        ...
        //send data to view file 
        $view                       = View::factory('admin/managerating_drivers_all')->bind('all_rating_drivers', $all_rating_drivers)->bind('pag_data', $pag_data)->bind('ListCount', $count_rating_drivers)->bind('get_rate_company', $get_rate_company)->bind('Offset', $offset)->bind('srch', $_REQUEST);
        $this->template->title      = SITENAME . " | " . __('manage_ratings_list');
        $this->template->page_title = __('manage_ratings_list');
        $this->template->content    = $view;
    }
    
    public function action_website_certificate(){
        ...
        //Page Title
        $this->page_title          = __('manage_website_certificate');
        $this->selected_page_title = __('manage_website_certificate');
        $count_menu_list           = $this->manage_model->count_menu_list();
        //pagination loads here
        //-------------------------
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        ...
        $website_certificates              = $this->manage_model->get_website_certificates();

        //Find page action in view
        $action                     = $this->request->action();
        //send data to view file 
        $view                       = View::factory('admin/manage_webpage_certificate')->bind('all_menu_list', $all_menu_list)->bind('pag_data', $pag_data)->bind('srch', $_REQUEST)->bind('Offset', $offset)->bind('website_certificates', $website_certificates);
        $this->template->title      = SITENAME . " | " . __('manage_website_certificate');
        $this->template->page_title = __('manage_website_certificate');
        $this->template->content    = $view;
    ...
    public function action_edit_website_certificates(){       
        ...
        if ($signup_submit && Validation::factory($_POST, $_FILES)) {
            ...
            $validator   = $this->manage_model->validate_website_certificate($post_values);
            ...
            if ($validator->check()) {
                ...
                if(!empty($_FILES['website_photo']['name'])){
                    ...
                    $web_filename         = Upload::save($_FILES['website_photo'], $web_image_name,DOCROOT.CERTIFICATE_PATH);
                    $web_image       = Image::factory($web_filename);
                    $web_path11           = DOCROOT . CERTIFICATE_PATH;                   
                    ...
                    if($web_image_old && $web_image_old!=''){
                        $old_path = DOCROOT.CERTIFICATE_PATH.$web_image_old;
                ...
                $post_values['image'] = $web_image_name;
                $signup_id = $this->manage_model->update_website_certificate($post_values,$id);
                
                    Message::success('Successfully update website certificate');
                    $this->request->redirect("manage/website_certificate");
            } else {
                $errors = $validator->errors('errors');
                //print_r($errors);exit;
            }
        ...
        $website_certificates = $this->manage_model->get_website_certificate($id);
        //print_r($website_content);exit;

        $view                       = View::factory('admin/edit_website_certificates')->bind('website_certificates', $website_certificates)->bind('postvalue',$postvalue)->bind('website_content',$website_content)->bind('errors',$errors)->bind('validator', $validator);
        $this->template->title      = SITENAME . " | " . __('manage_website_certificate');
        $this->template->page_title = __('manage_website_certificate');
        $this->template->content    = $view;
	...
	public function action_website_booking_confirm_content(){
        ...
        if ($signup_submit && Validation::factory($_POST, $_FILES)) {
            ...
            $validator   = $this->manage_model->validate_website_booking_confirm_content($post_values);

            if ($validator->check()) {
                $signup_id = $this->manage_model->update_website_booking_confirm_content($post_values);
        ...
        $view                       = View::factory('admin/manage_website_booking_confirm_content')->bind('website_content', $website_content)->bind('postvalue',$postvalue)->bind('website_content',$website_content)->bind('errors',$errors);
        $this->template->title      = SITENAME . " | " . __('manage_website_booking_confirm_content');
        $this->template->page_title = __('manage_website_booking_confirm_content');
        $this->template->content    = $view;

    }
    
    public function action_driver_shift_logs_list()
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
       ...
       // $availabilitycount         = $this->add_model->validate_packagedriver($cid);
        ...
        if(isset($_SESSION['download_set']))
        {
                $all_company_list = $this->manage_model->driver_shift_logs_list($driver_id,trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date,$search_date,'',0,3000);

                if(count($all_company_list)){

                    $xls_output = array(); $i=0;

                    $f = fopen('php://memory', 'w');
                    $delimiter = ",";

                    $fields = array(__('driver_code'),__('Day'),__('shift'),__('shift type'));
                    //fputcsv($f, $fields, $delimiter);

                    foreach($all_company_list as $val)
                    {
                        $i++;
                        $driver_id = isset($val['driver_id'])?$val['driver_id']:0;
                        $driver_code = isset($val['driver_code'])?$val['driver_code']:0;
                        $driver_lastname = isset($val['driver_lastname'])?$val['driver_lastname']:'';
                        ...
                        //$shift_start = isset($val['shift_start'])?Commonfunction::convertphpdate('Y-m-d H:i:s',$val['shift_start']):'';
                        //$shift_end = isset($val['shift_end'])?Commonfunction::convertphpdate('Y-m-d H:i:s',$val['shift_end']):''; 
                        $shift_start = isset($val['shift_start'])?Commonfunction::convertphpdate('H:i:s',$val['shift_start']):'';
                        $shift_end = isset($val['shift_end'])?Commonfunction::convertphpdate('H:i:s',$val['shift_end']):''; 
                        ...
                        $xls_output[$i]['driver_code'] = $driver_code;
                        $xls_output[$i]['create_day'] = $create_day;
                        $xls_output[$i]['shift_start'] = $shift_start;
                        $xls_output[$i]['shift_type'] = 1;

                        fputcsv($f, $xls_output[$i], $delimiter);

                        if($shift_end != ''){
                            $i++;
                            $xls_output[$i]['driver_code'] = $driver_code;
                            $xls_output[$i]['create_day'] = $create_day;
                            $xls_output[$i]['shift_start'] = $shift_end;
                            $xls_output[$i]['shift_type'] = 0;

                            fputcsv($f, $xls_output[$i], $delimiter);
                        }
                    }

                    $heading = 'driver_in_out_logs';
                    $filename = $heading."_".date("Y-m-d_H-i",time()).'.csv';
                    ...
                    $export_table_header = array(__('driver_code'),__('Day'),__('shift'),__('shift type'));
                    $export_table_field_select = array('driver_code','create_day','shift_start','shift_type');
                    $sub_heading = $start_date." to ".$end_date;
                    //$this->action_create_the_document_drivershift_logs($xls_output,$export_table_header,$export_table_field_select,$heading);
                }
        }
        //Post results for search 
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->driver_shift_logs_list($driver_id,trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date,$search_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->driver_shift_logs_list($driver_id,'','',$start_date,$end_date,$search_date,'',NULL,NULL,TRUE);  
        }

        //print_r($count_company_list);exit;
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
            if ($page_no == 0 || $page_no == 'index')
        ...
        if (isset($_REQUEST['keyword']))
        {
            $all_company_list = $this->manage_model->driver_shift_logs_list($driver_id,trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date,$search_date, $company, $offset, REC_PER_PAGE);
        }
        else
        {
            $all_company_list = $this->manage_model->driver_shift_logs_list($driver_id,'','',$start_date,$end_date,$search_date,'', $offset, REC_PER_PAGE);
        }
       
        $get_allcompany=array();
        //set data to view file 
        $view                    = View::factory('admin/driver_shifts_list')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list)->bind('driverlist', $driverlist);    
        $this->template->content = $view;
    }
    ...
    {   
        ...
        $zone_details = $manage_model->manage_zone_search($keyword,$status,'', REC_PER_PAGE,true);
		...
		$detail_zone = $manage_model->manage_zone_search($keyword,$status,$offset, REC_PER_PAGE);
        //****pagination ends here***//


        $this->page_title          = __( 'manage_zone' );
       
        //send data to view file 
        $view                       = View::factory( 'admin/manage_zone' )->bind( 'pag_data', $pag_data )->bind('zone_details',$detail_zone)->bind('Offset',$offset);
        $this->template->title      = SITENAME . " | " . __( 'manage_zone' );
        $this->template->page_title = __( 'manage_zone' );
        $this->template->content    = $view;
    }
    
    public function action_block_zone_request()
    {
        $user_createdby = $_SESSION['userid'];
        $usertype       = $_SESSION['user_type'];
        ...
        $this->is_login();
        $manage   = Model::factory( 'manage' );
        $status   = $manage->block_zone_request( $_REQUEST['uniqueId'] );
        $pagedata = explode( "/", $_SERVER["REQUEST_URI"] );
        $page     = isset( $pagedata[3] ) ? $pagedata[3] : '';
        //Flash message for Reject
        //==========================
    ...
    public function action_active_zone_request()
    {
        $user_createdby = $_SESSION['userid'];
        $usertype       = $_SESSION['user_type'];
        if ( $usertype != 'A' && $usertype != 'S' ) {
        ...
        $this->is_login();
        $manage   = Model::factory( 'manage' );
        $status   = $manage->active_zone_request( $_REQUEST['uniqueId'] );
        $pagedata = explode( "/", $_SERVER["REQUEST_URI"] );
        $page     = isset( $pagedata[3] ) ? $pagedata[3] : '';
        //Flash message for Reject
        //==========================
    ...
    public function action_trash_zone_request()
    {
        $user_createdby = $_SESSION['userid'];
        $usertype       = $_SESSION['user_type'];
        ...
        $this->is_login();
        $manage = Model::factory( 'manage' );
        $status = $manage->trash_zone_request( $_REQUEST['uniqueId'] );
        //Flash message for Reject
        //==========================
        Message::success( __( 'Checked requests has been deleted' ) );
        //redirects to job_feedback details page after deletion
    ...
    // 03 Feb 2020
    public function action_lost_customer_report() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('lost_customer_report');
        $this->selected_page_title = __('lost_customer_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        $search_post = arr::get($_REQUEST, 'search_user');
        $recharge_type = isset($_REQUEST['recharge_type'])?$_REQUEST['recharge_type']:'';
        $start_date = isset($_REQUEST['start_date'])?$_REQUEST['start_date']:date('Y-m-01 00:00:00');
        $end_date = isset($_REQUEST['end_date'])?$_REQUEST['end_date']:date('Y-m-d 23:59:59');
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->lost_customer_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$start_date,$end_date,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->lost_customer_report('','','','',NULL,NULL,TRUE);  
        }
			//echo $count_company_list; exit;
            //pagination loads here
            //-------------------------
            ...
            $pag_data    = Pagination::factory(array(
                ...
                'items_per_page' => REC_PER_PAGE,
                'total_items' => $count_company_list['count'],
                'view' => 'pagination/punbb'
            ));
        
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->lost_customer_report(trim(Html::chars($keyword)), trim(Html::chars($status)),$start_date,$end_date, $offset, REC_PER_PAGE,FALSE);
            //~ print "<prE>";
            ...
            if(isset($_SESSION['download_set']))
            {
                $all_company_list = $this->manage_model->lost_customer_report(trim(Html::chars($keyword)), trim(Html::chars($status)),$start_date,$end_date, $offset,3000);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {   
					$create_date =  (isset($val['created_date']) && !empty($val['created_date']) ) ? gettype($val['created_date']) : "string"; 
					if($create_date == 'object') {
						$date = commonfunction::convertphpdate('Y-m-d h:i:s',$val['created_date']);
					} else {
						$date = $val['created_date'];
					}                 
                    $xls_output[$key]['created_date'] = $date;                  
                }

                $export_table_header = array(__('country_code'),__('passenger_phone'),__('passenger_name'),__('passenger_email'),__('created_date'));
                $export_table_field_select = array('passenger_country_code','passenger_phone','passenger_name','passenger_email','created_date');

                $heading = __('lost_customer_report');
		...
		$country_codes = $count_company_list['country_code'];
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/lost_customers')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('all_company_list', $all_company_list)->bind('country_codes', $country_codes);
        $this->template->content = $view;
    }
    
    public function action_customer_accounts_report() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('customer_accounts_report');
        $this->selected_page_title = __('customer_accounts_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        $wallet_count    = $this->manage_model->customer_accounts_report($id, $start_date, $end_date, $keyword, REC_PER_PAGE, 0, TRUE);
		...
		$pag_data    = Pagination::factory(array(
			...
			'items_per_page' => REC_PER_PAGE,
			'total_items' => $wallet_count,
			'view' => 'pagination/punbb'
		));
	
		
		$customer_accounts_report_wallet    = $this->manage_model->customer_accounts_report($id, $start_date, $end_date, $keyword, REC_PER_PAGE, $offset, FALSE);	
        //~ print "<pre>";
        //~ print_r($customer_accounts_report_wallet);
        //~ exit;        
        //set data to view file 
        
        if(isset($_SESSION['download_set']))
		{
			$all_company_list = $this->manage_model->customer_accounts_report($id, $start_date, $end_date, $keyword, 3000, $offset, FALSE);	
			$xls_output = $all_company_list;
			foreach($all_company_list as $key => $val)
			{   
				$opening_amount = ( $val['wallet_amount'] + $val['total_trip_amt'] ) - $val['total_recharge_amt'];
				           
				$xls_output[$key]['opening_amount'] = $opening_amount;                  
			}

			$export_table_header = array(__('passenger_name'),__('passenger_email'),__('passenger_phone'),__('opening_balance'),__('recharge'),__('transactions'),__('closing_balance'));
			$export_table_field_select = array('name','email','phone','opening_amount','total_recharge_amt','total_trip_amt','wallet_amount');

			$heading = __('customer_accounts_report');
        ...
        $view                    = View::factory('admin/customer_accounts_report')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('all_company_list', $customer_accounts_report_wallet);
        $this->template->content = $view;
    }
    
    public function action_customer_accounts_detail() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('customer_accounts_detail');
        $this->selected_page_title = __('customer_accounts_detail');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        $wallet_count    = $this->manage_model->customer_accounts_report_wallet($id, $start_date, $end_date, $keyword, REC_PER_PAGE, 0, TRUE);
		...
		$customer_accounts_report_wallet    = $this->manage_model->customer_accounts_report_wallet($id, $start_date, $end_date, $keyword, REC_PER_PAGE, $offset, FALSE);	
         //~ print "<pre>";
         //~ echo $wallet_count;
        //~ print_r($customer_accounts_report_wallet);
        //~ exit;        
        //set data to view file 
        $view                    = View::factory('admin/customer_accounts_detail')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('all_company_list', $customer_accounts_report_wallet);
        $this->template->content = $view;
    }
    
    public function action_fleet_sales_report()
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('fleet_sales_report');
        $this->selected_page_title = __('fleet_sales_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->fleet_sales_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),trim(Html::chars($_REQUEST['taxi_model'])),$start_date,$end_date,$year,$month,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->fleet_sales_report('','','',$start_date,$end_date,$year,$month,'',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
        ...
        if (isset($_REQUEST['keyword']))
        {
            $all_company_list = $this->manage_model->fleet_sales_report(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),trim(Html::chars($_REQUEST['taxi_model'])),$start_date,$end_date,$year,$month, $company, $offset, 70);
        }
        else
        {
            $all_company_list = $this->manage_model->fleet_sales_report('','','',$start_date,$end_date,$year,$month,'', $offset, 70);
        }
          if(isset($_SESSION['download_set']))
            {
                $xls_output = $all_company_list;
                ...
                $export_table_header = array(__('taxi_no'),__('total_amount'),__('total_trips'));
                $export_table_field_select = array('taxi_no','total_amount','total_count');

                $heading = __('grand_limo_fleet_sales_report');
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/report/fleet_sales')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list)->bind('model_details_new', $model_details_new);
        $this->template->content = $view;
    }
    ...
    public function action_surge_pricing()
    {
        ...
        //Page Title
        $this->page_title          = __('manage_surge_pricing');
        $this->selected_page_title = __('manage_surge_pricing');
        
        $keyword = isset($_REQUEST['keyword']) ? trim(Html::chars($_REQUEST['keyword'])) : '';
		$status = isset($_REQUEST['status']) ? trim(Html::chars($_REQUEST['status'])) : '';
			
        $count_model_list        = $this->manage_model->all_surge_pricing_model_list($keyword, $status,'','',true);
        //pagination loads here
        //-------------------------
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
        ...
        $all_model_list           = $this->manage_model->all_surge_pricing_model_list($keyword, $status, $offset, REC_PER_PAGE, false);
        //****pagination ends here***//
        //$details = $siteusers->get_favorite_people($usrid);
        $details                    = '';
        //send data to view file 
        $view                       = View::factory('admin/manage_surge_pricing')->bind('all_model_list', $all_model_list)->bind('pag_data', $pag_data)->bind('ListCount', $count_model_list)->bind('srch', $_REQUEST)->bind('Offset', $offset);
        $this->template->title      = SITENAME . " | " . __('manage_surge_pricing');
        $this->template->page_title = __('manage_surge_pricing');
        $this->template->content    = $view;
    }
    
    public function action_block_surge_pricing_model_request()
    {
		$this->is_login();
        $user_createdby = $this->user_createdby;
        $usertype       = $this->usertype;
        ...
        $status   = $this->manage_model->block_surge_pricing_model_request($_REQUEST['uniqueId']);
		if($status==1){
			//Flash message for Reject
			//==========================
			Message::success(__('Checked requests have been changed to blocked status.'));
	...
	public function action_active_surge_pricing_model_request()
    {
		$this->is_login();
        $user_createdby = $this->user_createdby;
        $usertype       = $this->usertype;
        ...
        $status   = $this->manage_model->active_surge_pricing_model_request($_REQUEST['uniqueId']);
		if($status==1){
			//Flash message for Reject
			//==========================
			Message::success(__('Checked requests have been changed to activated status.'));
    ...
    public function action_send_notification()
    {
        ...
        if ($signup_submit) {
            ...
            $sub_title = $post_values['sub_title'];
            $template = $post_values['template'];
            $pushmessage = $post_values['content'];
            $title_arabic = $post_values['title_arabic'];
            $sub_title_arabic = $post_values['sub_title_arabic'];
            $pushmessageArabic = $post_values['content_arabic'];
            $start_date = $post_values['start_date'];
            $expire_date = $post_values['expire_date'];
            $image_only = isset($post_values['image_only'])?$post_values['image_only']:0;
            $form_values = Arr::extract($_POST, array(
    'content',
    'title',
    'sub_title',
    'template',
    ...
    'content_arabic'
            ...
            $values      = $form_values;
            $image_name = '';
            $validator   = $this->manage_model->validate_add_notification($values);
            if ($validator->check()) { 
                ...
                if (!empty($_FILES['notification_image']['name'])) {
                    ...
                    $path11           = DOCROOT . NOTIFY_IMG_IMGPATH.'/banner/';
                    $path1            = $image_name;                   
                }

                $notification_status = $this->manage_model->admin_add_notification($pushmessage,$template,$title,$sub_title,$title_arabic,$sub_title_arabic,$pushmessageArabic,$start_date,$expire_date,$image_only,$image_name);
                Message::success(__('Notification has been sent successfully'));
                $this->request->redirect("manage/admin_notification");
        ...
        $view                       = View::factory(ADMINVIEW . 'send_notification')->bind('errors', $errors)->bind('post_values', $post_values);
        $this->template->title      = __('Send Notification');
        $this->template->page_title = __('Send Notification');
        $this->template->content    = $view;
    }

    public function action_admin_notification()
    {
        $user_createdby = $this->user_createdby;
        $company_id     = $this->company_id;
        $usertype       = $this->usertype;
        ...
        //Page Title
        $this->page_title          = __('Manage Notifications');
        $this->selected_page_title = __('Manage Notifications');
        $count_push_list       = $this->manage_model->notification_list(NULL,NULL,TRUE);
        //pagination loads here
        //-------------------------
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
        ...
        $push_list             = $this->manage_model->notification_list($offset, REC_PER_PAGE,FALSE);

        $total_users                = count($push_list);
       
        //send data to view file 
        $view                       = View::factory('admin/manage_notification')->bind('push_list', $push_list)->bind('pag_data', $pag_data)->bind('total_users', $total_users)->bind('srch', $_REQUEST)->bind('Offset', $offset);
        $this->template->title      = SITENAME . " | " . __('Manage Notifications');
        $this->template->page_title = __('Manage Notifications');
        $this->template->content    = $view;
    }
    // 07 Aug 2020

    public function action_trash_notification_request()
    {
        $user_createdby = $this->user_createdby;
        $usertype       = $this->usertype;
        $this->is_login();
        
            $status = $this->manage_model->trash_notification_request($_REQUEST['uniqueId']);
            Message::success(__('Checked requests has been deleted'));
     ...
     public function action_advance_customer_transactions() 
    {
        $user_createdby            = $this->user_createdby;
        $company_id                = $this->company_id;
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('advance_customers_report');
        $this->selected_page_title = __('advance_customers_report');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->advance_customer_transactions(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->advance_customer_transactions('','','','','','',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
            ...
            $pag_data    = Pagination::factory(array(
                ...
                'items_per_page' => REC_PER_PAGE,
                'total_items' => count($count_company_list),
                'view' => 'pagination/punbb'
            ));
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->advance_customer_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
            ...
            if(isset($_SESSION['download_set']))
            {
                ...
                $offset = 0;
                $limit  = 1000;
                if(isset($_SESSION['range']) && !empty($_SESSION['range'])) {
                    $range = $_SESSION['range'];
                    // $range = "1-10";
                    $range = explode("-",$range);

                    $offset = (isset($range[0]) && !empty($range[0])) ? (int)$range[0] : 0; 
                    // $limit  = (isset($range[1]) && !empty($range[1])) ? (int)$range[1] : 3000; 
                }

                /* End */

                $all_company_list = $this->manage_model->advance_customer_transactions(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,$offset,$limit);
                $xls_output = $all_company_list;
                foreach($all_company_list as $key => $val)
                {                    
                    $xls_output[$key]['balance_wallet_amount'] = round($val['balance_wallet_amount'],3);                    
                ...
                $export_table_header = array(__('passenger_name'),__('passenger_email'),__('passenger_phone'),__('available_wallet'));
                $export_table_field_select = array('passenger_name','passenger_email','passenger_phone','balance_wallet_amount');

                $heading = __('advance_customers_report');
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/advance_customers')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }

public function action_gift_card_logs()
 
    {
        ...
        $usertype                  = $this->usertype;
        //Page Title
        $this->page_title          = __('gift_card_logs');
        $this->selected_page_title = __('gift_card_logs');
        //default empty list and offset
        $search_list               = '';
        $offset                    = '';
        //Find page action in view
        ...
        if (isset($_REQUEST['keyword'])) {
            ...
            $count_company_list = $this->manage_model->gift_card_logs(trim(Html::chars($_REQUEST['keyword'])), trim(Html::chars($_REQUEST['status'])),$recharge_type,$start_date,$end_date,$company,NULL,NULL,TRUE);
        }
        else
        {
           $count_company_list = $this->manage_model->gift_card_logs('','','','','','',NULL,NULL,TRUE);  
        }
            //pagination loads here
            //-------------------------
            $page_no = isset($_GET['page']) ? $_GET['page'] : 0;
        ...
        // {
            $keyword = isset($_REQUEST['keyword'])?$_REQUEST['keyword']:'';
            $status = isset($_REQUEST['status'])?$_REQUEST['status']:'';
            $all_company_list = $this->manage_model->gift_card_logs(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company, $offset, REC_PER_PAGE,FALSE);
        // }
        // else
        // {
        //     $all_company_list = $this->manage_model->passenger_wallet_logs('','','','','','', $offset, REC_PER_PAGE);
        // }
            // if(isset($_SESSION['download_set']))
            // {
            //     $all_company_list = $this->manage_model->gift_card_logs(trim(Html::chars($keyword)), trim(Html::chars($status)),$recharge_type,$start_date,$end_date, $company,0,3000);
            //     $xls_output = $all_company_list;
            //     foreach($all_company_list as $key => $val)
            //     {
        ...
        $get_allcompany          = $this->manage_model->get_allcompany();
        //set data to view file 
        $view                    = View::factory('admin/gift_card_logs')->bind('title', $title)->bind('Offset', $offset)->bind('action', $action)->bind('srch', $_REQUEST)->bind('pag_data', $pag_data)->bind('CompanyList', $CompanyList)->bind('availabilitycount', $availabilitycount)->bind('get_allcompany', $get_allcompany)->bind('all_company_list', $all_company_list);
        $this->template->content = $view;
    }

    /* Sasi */

    public function action_update_with_excel() {

        $this->page_title          = __('update_with_excel');
        $this->selected_page_title = __('update_with_excel');

        $view                       = View::factory('admin/update_wallet_with_excel');
        $this->template->title      = SITENAME . " | " . __('update_with_excel');
        $this->template->page_title = __('update_with_excel');
        $this->template->content    = $view;
    }

    public function action_update_wallet() {

        if(isset($_FILES['passenger_details'])) {
            $fileName = "warba_corporate_invoice.xlsx";
            $uploadFile = Upload::save($_FILES['passenger_details'], $fileName, DOCROOT.'public/');
        }
        ...
        try {
            ...
            $FileName = DOCROOT.'public/'."warba_corporate_invoice.xlsx";
            ...
            {
                try {
                    if ( $xlsx = SimpleXLSX::parse($FileName,false,true) ) {
                        $fields = $xlsx->rows();
                        for ($i=2; $i < count($fields) - 1; $i++) { 

                            if( isset($fields[$i][4]) && !empty($fields[$i][4])) {
				 $passenger_mobile = (string)$fields[$i][4];
                                $passengerDetails = $this->manage_model->getPassengerByPhoneNumber($passenger_mobile);
                                if(isset($passengerDetails['_id']) && !empty($passengerDetails['_id'])) {
				     //print_r($fields[$i]);
                                    $invoice_id = isset($fields[$i][11])  ? $fields[$i][11] : '';
                                    if(!empty($invoice_id)) {
                                    $amount = isset($fields[$i][9]) ? $fields[$i][9] : '0';
                                    $notes = "Warba - Invoice Update ";
                                    if($amount <= 0) {
                                        $amount = 0;
                                    }
                                 
                                    $passengerId = $passengerDetails['_id'];
                                    $last_invoice_id = isset($passengerDetails['last_invoice_id'])?$passengerDetails['last_invoice_id']:'';
                                    $update_wallet = 0;
                                    if($last_invoice_id != $invoice_id){
                                    $update_wallet = $this->manage_model->update_wallet_amount_invoice($passengerId,$amount,$negative,$notes,$invoice_id);
                                    $passenger = array(
                                        "phone_number" => $fields[$i][4], 
                                        "status" => __("active"),
                                        "update_status" => 0,
                                        "amount" => $fields[$i][9]
                                    );
                                    ...
                                    if($update_wallet > 0){
                                        $passenger['update_status'] = __("success_update_invoice");
                                    } else {
                                        $passenger['update_status'] = __("failed_to_update");
                                    }
                                    } else {
                                    $passenger = array(
                                        "phone_number" => $fields[$i][4], 
                                    ...
                                    $passenger['update_status'] = __("Already Updated");
                                    }
                                    ...
                                    $passenger['update_status'] = __("Empty Invoice ID");
                                    } 

                                    array_push($STATUS,$passenger);

                                } else {
                                    $passenger = array(
                                        "phone_number" => $fields[$i][4], 
                                        "status" => __('passenger_not_found'),
                                        "update_status" => "",
                                        "amount" => $fields[$i][9]
                                    );
                                    array_push($STATUS,$passenger);
            ...
            $response = $this->manage_model->generateXLSX($STATUS);
            echo $response;
            exit;
    ...
    public function action_corporate_promocode()
    {
        ...
        $count_promocode_list       = $this->manage_model->promocode_list_corporate(NULL,NULL,$search, $company_id,$corporate_id,TRUE);
        //pagination loads here
        //-------------------------
        $page_no                   = isset($_GET['page']) ? $_GET['page'] : 0;
        if ($page_no == 0 || $page_no == 'index')
        ...
        //  echo $offset;
        //echo REC_PER_PAGE;exit;
        $promocode_list             = $this->manage_model->promocode_list_corporate($offset, REC_PER_PAGE, $search, $company_id,$corporate_id,'');
       //echo '<pre>'; print_r($promocode_list);exit;
       
        $taxicompany_details        = $this->add_model->taxicompany_details();
        $total_users                = count($promocode_list);
        //send data to view file 
        $view                       = View::factory('admin/manage_corporate_promocode')->bind('promocode_list', $promocode_list)->bind('pag_data', $pag_data)->bind('total_users', $total_users)->bind('taxicompany_details', $taxicompany_details)->bind('srch', $_REQUEST)->bind('Offset', $offset);
        $this->template->title      = SITENAME . " | " . __('corporate_promocode');
        $this->template->page_title = __('corporate_promocode');
        $this->template->content    = $view;
    }

    public function action_delete_corporate_promocode()
    {
        $id          = $this->request->param('id');
        $promocode        = $this->manage_model->delete_promocode_corporate($id);
        if ($promocode) {
            Message::success(__('Promocode was deleted.'));
    ...
    public function action_add_promocode_with_excel() {
        if ($this->usertype == 'M') {
            $this->request->redirect("manager/login");
        }
        ...
        $this->page_title          = __('corporate_promocode');
        $this->selected_page_title = __('corporate_promocode');
        $view                       = View::factory('admin/add_promocode_with_excel')->bind('title', $title)->bind('details', $details)->bind('all_user_list', $count_user_list)->bind('pag_data', $pag_data)->bind('taxicompany_details', $taxicompany_details)->bind('srch', $_POST)->bind('company_id', $this->company_id)->bind('Offset', $offset)->bind('avail_package',$avail_package);
        $this->template->title      = SITENAME . ' ' . __('corporate_promocode');
        $this->template->page_title = __('corporate_promocode');
        $this->template->content    = $view;
    }

    public function action_add_promocode_with_excel_file() {
      ...
      if(isset($_FILES['promo_code_file'])) {
          $fileName = "add_promo_code.xlsx";
          $uploadFile = Upload::save($_FILES['promo_code_file'], $fileName, DOCROOT.'public/');
      }

      try {
          require_once "ReadExcel.php";
          $FileName = DOCROOT.'public/'."add_promo_code.xlsx";
          if (file_exists($FileName)) {
              try {
                  if ( $xlsx = SimpleXLSX::parse($FileName,false,true) ) {
                      $fields = $xlsx->rows();
                      for ($i=1; $i < count($fields); $i++) {
                          $ins_array      = array(
                              "company_id" => (int)$cid,
                              "promocode" => $fields[$i][0],
                              "package" => (int)$package,
                              "fixed_fare_amt"=>(int)$fixed_fare_amt,
                              "promo_type" => $mail_type,
                              "start_date" => $start_date,
                              "expire_date" => $expire_date,
                              "promo_limit" => (int)$promo_limit,
                              "createdate" => $currenttime,
                              "corporate_promocode" => (int)$corporate_promocode,
                              "total_airport_trips" => (int)$total_airport_trips,
                              "total_applied" => 0,
                              "total_used" => 0
                          );

                          $res = $this->manage_model->add_promocode_with_excel($fields[$i][0],$ins_array);
      ...
      if ($this->usertype == 'M') {
        ...
        $this->page_title          = __('corporate_promocode');
        $this->selected_page_title = __('corporate_promocode');
        $view                       = View::factory('admin/add_promocode_with_excel')->bind('title', $title)->bind('details', $details)->bind('all_user_list', $count_user_list)->bind('pag_data', $pag_data)->bind('taxicompany_details', $taxicompany_details)->bind('srch', $_POST)->bind('company_id', $this->company_id)->bind('Offset', $offset)->bind('avail_package',$avail_package)->bind('appliedStatus',$STATUS);
        $this->template->title      = SITENAME . ' ' . __('corporate_promocode');
        $this->template->page_title = __('corporate_promocode');
        $this->template->content    = $view;
    ...
    public function action_insert_promocode_new() {
        $startDate = "2024-01-01 00:00:00";
        $endDate = "2024-12-31 23:59:59";
        $package = 37;
        $company_id = $this->session->get('company_id');
        $cid = isset($company_id)? $company_id : 1;
        $currenttime = $this->commonmodel->getcompany_all_currenttimestamp(COMPANY_CID);

        /* GET the Promocode details from corporate_uniquecode_data collection */

        $res = $this->manage_model->getCodes(500);

        if(count($res) > 0) {
            foreach ($res as $key => $value) {
                if(isset($value['Unique ID']) && !empty($value['Unique ID'])) {
                    // $ins_array = array(
                    //     "company_id" => (int)$cid,
                    ...
                    //     "promo_type" => "1",
                    //     "promo_limit" => (int)100,
                    //     "corporate_promocode" => (int)1,
                    //     "total_airport_trips" => (int)2,
                    //     "total_applied" => 0,
                    //     "total_used" => 0
                    // );
                    ...
                    /*
                        {
                            "_id" : NumberLong(12753),
                            "company_id" : NumberLong(1),
                            "promocode" : "N380HA",
                            "package" : NumberLong(34),
                            "start_date" : ISODate("2021-12-31T21:00:00.000Z"),
                            "expire_date" : ISODate("2023-12-31T20:59:59.000Z"),
                            "createdate" : "2022-08-02 14:22:55",
                            "fixed_fare_amt" : NumberLong(0),
                            "promo_type" : "1",
                            "promo_limit" : NumberLong(100),
                            "corporate_promocode" : NumberLong(1),
                            "total_airport_trips" : NumberLong(2),
                            "register_promocode" : NumberLong(0),
                            "apply_user_limit" : NumberLong(1),
                            "maximum_allowed_limit" : NumberLong(2),
                            "total_applied" : NumberLong(0),
                            "total_used" : NumberLong(0),
                            "corporate_group" : NumberLong(1),
                            "kfh_promocode" : NumberLong(1)
                        }  
                    */
                    ...
                    $ins_array = array(
                        ...
                        "promo_limit" => (int)100,
                        "corporate_promocode" => (int)1,
                        "total_airport_trips" => (int)0,
                        "register_promocode" => (int)0,
                        "apply_user_limit" => (int)0,
                        "maximum_allowed_limit" => (int)0,
                        "total_applied" => 0,
                        "total_used" => 0,
                        "corporate_group" => 4,
                        "kfh_promocode" => 1
                    ...
                    $result = $this->manage_model->insertPromoCodeAndUpdateUniqueStatus($ins_array,$value['Unique ID']);
                    if($result == 2) {
                        echo $value['Unique ID']."------"."Already Exists";
                        echo "<br/>";
                    } else if($result == 0) {
                        echo $value['Unique ID']."------"."Failed";
                        echo "<br/>";
                    } else {
                        echo $value['Unique ID']."------"."Saved";
                        echo "<br/>";
                    }
                }
            }
    ...
    public function action_corporate_trips() {
        $startDate = isset($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-01 00:00:00');
        $endDate = isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-t 23:59:59');
        $corporate = (isset($_GET['corporate']) && !empty($_GET['corporate'])) ? $_GET['corporate'] : "";
        ...
        $corporateTrips = [];
        $corporateTripCount = 0;
        if($corporate != "") {
            $corporateTrips = $this->manage_model->getCorporateTrips($startDate, $endDate, $corporate, $offset, REC_PER_PAGE);
            $corporateTripCount = $this->manage_model->getCorporateTripsCount($startDate, $endDate, $corporate);
        }
        ...
        $pag_data = Pagination::factory(array(
            ...
            'items_per_page' => REC_PER_PAGE,
            'total_items' => $corporateTripCount,
            'view' => 'pagination/punbb'
        ));

        if(isset($_SESSION['download_set'])) {
            if($corporate != "") {
                $response = $this->manage_model->getCorporateTrips($startDate, $endDate, $corporate, 0, 4000);
                $corporateName = $this->manage_model->getCorporateName($corporate);

                $vars=explode('-',$_SESSION['download_set']);
                $start=$vars['0']-1;$end=$vars['1']-1;$type=$vars['2'];
                $this->session->delete('download_set');

                $table = '<table border="0" cellpadding="1" cellspacing="0"><tbody><tr><td width="100%" style="text-align:left;"><b>'.__('corporate_trips').'</b></td></tr><tr><td class="head_border" style="text-align:left;"><b>Generated Date : '.date("F j, Y").'</b></td></tr><tr><td class="head_border" style="text-align:left;"><b>Start Date : '.$startDate.'</b></td></tr><tr><td class="head_border" style="text-align:left;"><b>End Date : '.$endDate.'</b></td></tr><tr><td class="head_border" style="text-align:left;"><b>Corporate Name : '.$corporateName.'</b></td></tr></tbody></table>';

                $tableHeader = '<table><thead><tr>
                    <td><b>'.__('sno_label').'</b></td>
                    <td><b>'.__('passenger_name').'</b></td>
                    <td><b>'.__('passenger_phone').'</b></td>
                    <td><b>'.__('promocode').'</b></td>
                    <td><b>'.__('trip_id').'</b></td>
                    <td><b>'.__('actual_pickup_time').'</b></td>
                    <td><b>'.__('promo_discount').'</b></td>
                    <td><b>'.__('trip_total_fare').'</b></td>
                    <td><b>'.__('airport').'</b></td>
                    <td><b>'.__('status').'</b></td>
                    <td><b>'.__('actual_paid_amount').'</b></td>
                    <td><b>'.__('created_date').'</b></td></tr></thead>';
                ...
                foreach ($response as $key => $value) {
                    ...
                    $value['paid_status'] = isset($value['paid_status']) ? $value['paid_status'] : 0;
                    $value['airport_trip'] = isset($value['airport_trip']) ? $value['airport_trip'] : 0;
                    $value['total_fare_backup'] = isset($value['total_fare_backup']) ? $value['total_fare_backup'] : 0;
                    $value['actual_paid_amount'] = isset($value['actual_paid_amount']) ? $value['actual_paid_amount'] : 0;

                    $airport = $value['airport_trip'] == 1 ? $value['total_fare_backup'] != 0 ? "Airport (Free)" : "Airport" : "-";
                    $paidStatus = $value['paid_status'] == 1 ? 'Paid' : 'Unpaid';
                    $createdDate = Commonfunction::convertphpdate('Y-m-d h:i:s A',$value['createdAt']);
                    $pickupDate = Commonfunction::convertphpdate('Y-m-d h:i:s A',$value['actual_pickup_time']);

                    $sno += 1;
                    $tableBody = "";
                    $tableBody .= "<tr>";
                    $tableBody .= '<td>'.$sno.'</td>';
                    $tableBody .= '<td>'.$value['passenger_name'].'</td>';
                    $tableBody .= '<td>'.$value['country_code']." ".$value['phone'].'</td>';
                    $tableBody .= '<td>'.$value['promocode'].'</td>';
                    $tableBody .= '<td>'.$value['tripId'].'</td>';
                    $tableBody .= '<td>'.$pickupDate.'</td>';
                    $tableBody .= '<td>'.CURRENCY.' '.round($value['discount'],3).'</td>';
                    $tableBody .= '<td>'.CURRENCY.' '.round($value['trip_amount'],3).'</td>';
                    // $tableBody .= '<td>'.$airport.'</td>';
                    $tableBody .= '<td>'.$paidStatus.'</td>';
                    $tableBody .= '<td>'.$value['actual_paid_amount'].'</td>';
                ...
                $file = $table.$tableHeader.$tableBodyStart.$tableBodyContent.$tableBodyEnd.$tableClose;
                if($type == 2) {
                    $style = '<style>h1{color: navy;font-family: times;font-size: 24pt;font-weight:bold;}td {font:12pt arial; color:#000000;}</style>';
                    $file = $style.$file;
                    $filename = __('corporate_promocode_usage')."_".date("Y-m-d_H-i",time());
                    $html = preg_replace("<tbody>"," ",$file);
                    $html = preg_replace("</tbody>"," ",$html);
                    $html = trim($file);
                    ob_clean();
                    $generate_pdf = $this->manage_model->generate_pdf($html,$filename);
                } else {
                    $filename = __('corporate_trips')."_".date("Y-m-d_H-i",time());
        ...
        $corporateGroups = $this->manage_model->getCorporateGroups();

        $this->page_title          = __('corporate_trips');
        $this->selected_page_title = __('corporate_trips');
        $view = View::factory('admin/corporate_trips')->bind('title', $title)->bind('pag_data', $pag_data)->bind('srch', $_GET)->bind('Offset', $offset)->bind('corporateTrips',$corporateTrips)->bind('corporateGroups', $corporateGroups);
        $this->template->title      = SITENAME . ' ' . __('corporate_trips');
        $this->template->page_title = __('corporate_trips');
        $this->template->content    = $view;
    }

    public function action_update_corporate_trips() {
        $this->page_title          = __('corporate_trips');
        $this->selected_page_title = __('corporate_trips');
        $view                       = View::factory('admin/update_corporate_trips_excel');
        $this->template->title      = SITENAME . " | " . __('corporate_trips');
        $this->template->page_title = __('corporate_trips');
        $this->template->content    = $view;
    }

    public function action_update_corporate_trips_with_excel() {
        if(isset($_FILES['passenger_details'])) {
            $fileName = "warba_corporate_invoice.xlsx";
            $uploadFile = Upload::save($_FILES['passenger_details'], $fileName, DOCROOT.'public/');
        }
        ...
        try {
                ...
                try {
                    if ( $xlsx = SimpleXLSX::parse($FileName,false,true) ) {
                        ...
                        for ($i = 2; $i < count($fields) - 1; $i++) { 
                            $tripId = $fields[$i][1];
                            $amount = $fields[$i][4];
                            $passengerMobile = $fields[$i][3];
                                
                            if($tripId != "") {
                                $paymentInfo = $this->manage_model->getPaymentLogIngo($tripId);
                                if(isset($paymentInfo['tripId'])) {

                                    if(!isset($paymentInfo['paid_status']) || (isset($paymentInfo['paid_status']) && $paymentInfo['paid_status'] != 1)) {  
                                      $tripAmount = $paymentInfo['trip_amount'];
                                      ...
                                      if($tripAmount <= 0) {
                                          $updateArray['paid_status'] = 1;
                                      }

                                      $response = $this->manage_model->updateCorporatePaymentLog($tripId, $updateArray);

                                      $passenger = array(
                                          "phone_number" => $passengerMobile,
                                          "tripId" => $tripId, 
                                          "status" => __("success"),
                                          "update_status" => 1,
                                          "amount" => $amount
            ...
            $response = $this->manage_model->generateXLSXCorporate($STATUS);
            echo $response;
            exit;
    ...
    public function action_check_hesabe_transaction() {
        $this->page_title          = "Check Hesabe Transactions";
        $this->selected_page_title = "Check Hesabe Transactions";
        $view                       = View::factory('admin/check_hesabe_transaction');
        $this->template->title      = SITENAME . " | " . "Check Hesabe Transactions";
        $this->template->page_title = "Check Hesabe Transactions";
        $this->template->content    = $view;
    }

    public function action_check_hesabe_transactions() {

      if(isset($_FILES['passenger_details'])) {
            $fileName = "hesabe_transaction_report.xlsx";
            $uploadFile = Upload::save($_FILES['passenger_details'], $fileName, DOCROOT.'public/');
        }

      $hesabeTransactionReport = [];
      try {
        require_once "ReadExcel.php";
        $FileName = DOCROOT.'public/'."hesabe_transaction_report.xlsx";
        $STATUS = [];
        if (file_exists($FileName))
        {
          try {
            if ( $xlsx = SimpleXLSX::parse($FileName,false,true) ) {
              $fields = $xlsx->rows();
              for ($i = 2; $i < count($fields) - 1; $i++) { 
                if($fields[$i][11] == "Payment Gateway" && $fields[$i][10] == "success") {
                  $response = $this->manage_model->getHesabeTransactionReport($fields[$i][5],$fields[$i][7]);
                  $message = "Not Found";
                  if($response == 1) {
                  ...
                  $transaction = array(
                      "message" => $message,
                      "order_reference_number" => $fields[$i][5], 
                      "payment_id" => $fields[$i][7], 
                  );
                  array_push($hesabeTransactionReport,$transaction);
                }
              }
              $responseTable = $this->manage_model->getHesabeTransactionReportTable($hesabeTransactionReport);
              echo $responseTable;
            } else {
              echo SimpleXLSX::parseError();
            }
    ...
    public function action_promocode_usage_report() {
        ...
        if($promocode != "") {
            $totalTrips = $this->manage_model->getPromocodeUsageList($promocode, true);
            $tripDetails = $this->manage_model->getPromocodeUsageList($promocode, false, $offset, REC_PER_PAGE);
            $promocodeDetails = $this->manage_model->getPromocodeDetails($promocode);
        }
        ...
        $pag_data = Pagination::factory(array(
            ...
            'items_per_page' => REC_PER_PAGE,
            'total_items' => $totalTrips,
            'view' => 'pagination/punbb'
        ));

        $this->page_title = __('promocode_usage_report');
        $this->selected_page_title = __('promocode_usage_report');
        
        $view = View::factory('admin/promocode_usage_report')->bind('totalTrips', $totalTrips)->bind('pag_data', $pag_data)->bind('tripDetails', $tripDetails)->bind('promocodeDetails', $promocodeDetails)->bind('srch', $_REQUEST)->bind('Offset', $offset);

        $this->template->title      = SITENAME . " | " . __('promocode_usage_report');
        $this->template->page_title = __('promocode_usage_report');
        $this->template->content    = $view;
    }

    // Sasidharan Jan 17 2025
    public function action_passenger_coupons()
    {   
        $user_createdby = $_SESSION['userid'];
        $usertype       = $_SESSION['user_type'];
        $manage_model     = Model::factory('manage');
        ...
        $couponCount = $manage_model->getPassengerComments($startDate, true);
        ...
        $pag_data = Pagination::factory( 
            array(
                ...
                'items_per_page' => REC_PER_PAGE,
                'total_items' => $couponCount,
                'view' => 'pagination/punbb' 
            ) 
        );

		$couponDetails = $manage_model->getPassengerComments($startDate, false, $offset, REC_PER_PAGE);
        ...
        $this->page_title          = "Passenger Coupon Code";
        $view                       = View::factory( 'admin/manage_passenger_coupon' )->bind( 'pag_data', $pag_data )->bind('couponDetails',$couponDetails)->bind('Offset',$offset);
        $this->template->title      = SITENAME . " | " . "Passenger Coupon Code";
        $this->template->page_title = "Passenger Coupon Code";
        $this->template->content    = $view;
    }

    public function action_passenger_added_coupons()
    {   
        $user_createdby = $_SESSION['userid'];
        $usertype       = $_SESSION['user_type'];
        $manage_model     = Model::factory('manage');
    ...
    public function action_totalWalletCredit() {
        $startDate = "2024-01-01 00:00:00";
        $endDate = "2024-12-31 23:59:59";
        
        $manage_model     = Model::factory('manage');

        $response = $manage_model->getTotalPassengerCredits($startDate, $endDate);

        $export_table_header = array(__('passenger_name'),__('passenger_phone'),__('amount_credit') ,__('current_wallet_amt'));
        $export_table_field_select = array('passengerName','passengerPhone','totalCredit','walletAmount');

        $heading = "Total Credit In Passenger Wallet";

        $total_fare='';

        $sub_heading = $startDate." to ".$endDate;

        $var="1-20000-1";
        $this->session->set('download_set',$var);

        $this->action_create_the_document($response,$export_table_header,$export_table_field_select,$heading,$total_fare,$sub_heading);

    }

    public function action_promotion_push_notification() {

        $count = $this->manage_model->getPromotionalPushNotificationCount();
        ...
        $promotionPushNotificationDetails = $this->manage_model->getPromotionalPushNotifications($offset, REC_PER_PAGE);

        // echo "<pre>";
        // print_r($count);
        // echo "<br>";
        // print_r($promotionPushNotificationDetails);
        // exit;

        $this->page_title          = "Manage push notifications";
        $view                       = View::factory( 'admin/manage_promotional_push_notifications' )->bind( 'pag_data', $pag_data )->bind('promotionPushNotificationDetails',$promotionPushNotificationDetails)->bind('Offset',$offset);
        $this->template->title      = SITENAME . " | " . "Manage push notifications";
        $this->template->page_title = "Manage push notifications";
        $this->template->content    = $view;
    }
