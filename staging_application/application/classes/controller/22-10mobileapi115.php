...
****************************************************************/
Class Controller_Mobileapi115 extends Controller_Mobile101
{
        ...
        if ((count($apikey_result) > 0) || ($company_api_key == 'all')) {
            ...
            switch ($method) {
                ...
                case 'getcoreconfig':
					...
					$config_array['recent_location'] = $getrecentLocation;
                ...
                case 'driver_location_history':
                    ...
                    if (!empty($location_array)) {
						...
						//echo $check_driver_login_status;exit;
                        ...
                        if ($check_driver_login_status == 1) {
                            ...
                            if ($history_validator->check()) {
                                ...
                                if ($location_array['driver_id'] != "") {
									...
									//print_r($driver_request_details);exit;
                                    if (count($driver_request_details) > 0) {
                                        $msg                         = array(
                                            "message" => __('passenger_trip_cancelled'),
                                            "status" => 10
                                        );
                                        $update_driver_request_array = array(
                                            "notification_status" => 5
                                        ); // Passenger Cancelled
										if(isset($driver_request_details[0]['trip_id'])){
											$result                      = $api->update_table(MDB_PASSENGERS_LOGS, $update_driver_request_array, '_id', $driver_request_details[0]['trip_id']);											
										}                                        
                                ...
                                /*** Notification to driver for Dispatcher cancelled the trip *********************/
                                    ...
                                    if (count($driver_request_details)>0) {
										...
										if(isset($driver_request_details[0]['trip_id'])){
                                        $update_current_result = $api->update_table(MDB_PASSENGERS_LOGS, $update_driver_array, '_id', $driver_request_details[0]['trip_id']);
										}
                                        echo json_encode($msg);
                                        break;
                                    }
							...
							if ($check_new_request > 0) {
								...
								if (count($get_passenger_log_details) > 0) {
									...
									/** MULTIPLE TRIP FLOW 2 **/
									/* $get_passenger_log_details = $api->get_passenger_log_detail($trip_id);
                                     if (count($get_passenger_log_details) > 0) {
									 $my_current_location = array($latitude,$longitude);
									 $my_current_dlocation =  array($get_passenger_log_details[0]['drop_latitude'],$get_passenger_log_details[0]['drop_longitude']);
									 $my_current_taxi_id = isset($get_passenger_log_details[0]['taxi_id']) ? $get_passenger_log_details[0]['taxi_id'] :'';
									 $check_new_request_multitrip2            = $api->check_new_request_multitrip2($driver_id,$company_all_currenttimestamp,$my_current_location,$my_current_dlocation,$my_current_taxi_id);
									  if ($check_new_request_multitrip2 > 0) {
										  
											$passenger_name            = "";
											$get_passenger_log_details = $api->get_passenger_log_detail($check_new_request_multitrip2);
											$my_current_location = array($latitude,$longitude);
											$my_current_sdlocation =  array($get_passenger_log_details[0]['pickup_latitude'],$get_passenger_log_details[0]['pickup_longitude']);
											$my_current_staxi_id = isset($get_passenger_log_details[0]['taxi_id']) ? $get_passenger_log_details[0]['taxi_id'] :'';
											 $check_new_request_multitrip2_by2            = $api->check_new_request_multitrip2_by2($driver_id,$company_all_currenttimestamp,$my_current_location,$my_current_sdlocation,$my_current_staxi_id);
											 if ($check_new_request_multitrip2_by2 > 0) {
								$passenger_name            = "";
								$get_passenger_log_details = $api->get_passenger_log_detail($check_new_request_multitrip2_by2);
								//print_r($get_passenger_log_details);exit;
								if (count($get_passenger_log_details) > 0) {
									foreach ($get_passenger_log_details as $values) {
										$p_device_type        = isset($values['passenger_device_type']) ? $values['passenger_device_type'] :'';
									...
									$notification_seconds  = ($notification_seconds < 10) ? '0' . $notification_seconds : $notification_seconds;
                                                    ...
                                                    $trip_details          = array(
                                                        "message" => __('api_request_confirmed_passenger'),
                                                        "status" => "1",
                                                        "passengers_log_id" => $check_new_request_multitrip2_by2,
                                                        "booking_details" => array(
                                                            "pickupplace" => $pickupplace,
                                                            "dropplace" => $dropplace,
                                                            "pickup_time" => $pickup_time,
                                                    ...
                                                    $check_another_request = $api->check_new_request_bydriver($driver_id, $company_all_currenttimestamp, $check_new_request_multitrip2_by2);
                                                    if (count($check_another_request) > 0) {
                                                        foreach ($check_another_request as $cns) {
                                                            $api->change_driver_reqflow($cns['trip_id'], $cns['available_drivers'], $cns['rejected_timeout_drivers']);
                                                        }
                                                    ...
                                                    $result              = $api->update_table(MDB_REQUEST_HISTORY, $update_trip_array, 'trip_id', (int)$check_new_request_multitrip2_by2);
                                                    echo json_encode($msg);
													break;
                                                    $update_driver_array = array(
                                                        "status" => 'B'
									...
									/* MULTIPLE TRIP FLOW 1 */
									 ...
									 $interval = abs(strtotime($current_time) - strtotime($pickuptime));
									 if($interval >= 2 ){ 
									 $my_second_job            = $api->check_new_request($driver_id, $company_all_currenttimestamp);
									 $get_passenger_log_details = $api->get_passenger_log_detail($my_second_job);
									 $second_job_pickup_location = array($get_passenger_log_details[0]['pickup_latitude'],$get_passenger_log_details[0]['pickup_longitude']);
									 $second_job_drop_location =  array($get_passenger_log_details[0]['drop_latitude'],$get_passenger_log_details[0]['drop_longitude']);
									 $second_job_taxi_id = isset($get_passenger_log_details[0]['taxi_id']) ? $get_passenger_log_details[0]['taxi_id'] :'';
									 
									 $check_new_request_multitrip1            = $api->check_new_request_multitrip1($driver_id,$company_all_currenttimestamp,$second_job_pickup_location,$second_job_drop_location,$second_job_taxi_id);
									 if ($check_new_request_multitrip1 > 0) {
								$passenger_name            = "";
								$get_passenger_log_details = $api->get_passenger_log_detail($check_new_request_multitrip1);
								//print_r($get_passenger_log_details);exit;
								if (count($get_passenger_log_details) > 0) {
									foreach ($get_passenger_log_details as $values) {
										$p_device_type        = isset($values['passenger_device_type']) ? $values['passenger_device_type'] :'';
									...
									$notification_seconds  = ($notification_seconds < 10) ? '0' . $notification_seconds : $notification_seconds;
                                                    ...
                                                    $trip_details          = array(
                                                        "message" => __('api_request_confirmed_passenger'),
                                                        "status" => "1",
                                                        "passengers_log_id" => $check_new_request_multitrip1,
                                                        "booking_details" => array(
                                                            "pickupplace" => $pickupplace,
                                                            "dropplace" => $dropplace,
                                                            "pickup_time" => $pickup_time,
                                                    ...
                                                    $check_another_request = $api->check_new_request_bydriver($driver_id, $company_all_currenttimestamp, $check_new_request_multitrip1);
                                                    if (count($check_another_request) > 0) {
                                                        foreach ($check_another_request as $cns) {
                                                            $api->change_driver_reqflow($cns['trip_id'], $cns['available_drivers'], $cns['rejected_timeout_drivers']);
                                                        }
                                                    ...
                                                    $result              = $api->update_table(MDB_REQUEST_HISTORY, $update_trip_array, 'trip_id', (int)$check_new_request_multitrip1);
                                                    $update_driver_array = array(
                                                        "status" => 'B'
                                                    );
                                                    $result              = $api->update_table(MDB_DRIVER_INFO, $update_driver_array, '_id', (int)$driver_id);
				...
				*/
				case 'passenger_account_registration_step1':
					...
					$requireIndex = array('phone','devicetoken','deviceid','devicetype');
					sort($fromIndex);
					sort($requireIndex);
				...
				case 'passenger_account_registration_step3':
					...
					$requireIndex = array('phone','firstname','lastname','email','civilid','creditcard_no','expdatemonth','expdateyear','creditcard_cvv','cardholder_name','civilid_front','civilid_back');
					sort($fromIndex);
					sort($requireIndex);
                ...
                case 'passenger_profile':
					...
					if ($mobiledata['passengerid'] != null) {
						...
						if (count($result) > 0) {
							...
							$civilid_front = $result['civilid_front'];
							$civilid_back  = $result['civilid_back'];
							
							$civilid_front_thumb = $result['civilid_front_thumb'];
							$civilid_back_thumb  = $result['civilid_back_thumb'];
                ...
                case 'edit_passenger_profile':
                    ...
                    if (count($p_personal_array) > 0) {
                        if ($p_personal_array['email'] != null) {
                            ...
                            $p_firstname  = $p_personal_array['firstname'];
							$p_lastname   = $p_personal_array['lastname'];
                ...
                case 'add_favourite':
                    ...
                    if ($validator->check()) {
						...
						$fav_icon = $add_fav_array['fav_icon'];
                    ...
                    if ($trip_id != null) {
                        ...
                        if (count($get_passenger_log_details) > 0) {
                            foreach ($get_passenger_log_details as $journey) {
                                ...
                                $trip_details['booking_time']         = Commonfunction::convertphpdate('Y-m-d h:i:s A',$journey['pickup_time']);
                                ...
                                $trip_details['actual_distance']      = ($journey['actual_distance'] != 0)?$journey['actual_distance']:'';
                                $trip_details['metric']               = ($journey['metric'] != 0)?$journey['metric']:'';
                                $trip_details['amt']                  = round($journey['amt'], 2);
                                $trip_details['used_wallet_amount']   = isset($journey['used_wallet_amount']) ? round($journey['used_wallet_amount'], 2) : '';
                                $trip_details['job_ref']              = isset($journey['job_ref'])?$journey['job_ref']:'';
                                $trip_details['payment_type']         = isset($journey['payment_type'])?$journey['payment_type']:'';
                            ...
                            if (count($current_driver_status) > 0) {
								...
								$trip_details['driver_latitute']   = $current_driver_status['loc']['coordinates'][1];
								$trip_details['driver_longtitute'] = $current_driver_status['loc']['coordinates'][0];
                            }
                        ...
                        if ($validator->check()) {
                            ...
                            */
                                ...
                                foreach ($driver_pending_bookings as $key => $journey) {
                                    ...
                                    switch ($payment_type) {
                                        ...
                                        case 2:
                                            ...
                                            break;
                                        case 3:
                                            $driver_pending_bookings[$key]['payment_type'] = "Knet";
                                            break;
                                        default:
                                            $driver_pending_bookings[$key]['payment_type'] = "Cash";
                                            break;
                            ...
                            foreach ($pending_booking as $key => $journey) {
								...
								$pending_booking[$key]['metric'] = current($journey['metric']);
								
								$date1=date_create($act_pickup_time);
								$date2=date_create($drop_time);
								$diff=date_diff($date1,$date2);
								
								$pending_booking[$key]['waiting_hour'] = current($journey['waiting_hour']);
                                ...
                                switch ($payment_type) {
                                    ...
                                    case 2:
                                        ...
                                        break;
                                    case 3:
                                        $pending_booking[$key]['payment_type'] = "Knet";
                                        break;
                                    default:
                                        $pending_booking[$key]['payment_type'] = "Cash";
                                        break;
                ...
                case 'getpassenger_update':
                    ...
                    if ($validator->check()) {
                        ...
                        $request_type = $array['request_type'];
                         /* NEW CHNAGE */
                        $get_trip_detail_new = $api->get_trip_detail($trip_id, $passenger_id);
                        $get_taxi_min_speed = $get_trip_detail_new[0]['taxi_min_speed'];
                        
                        /* RUNING TIME UPDATE */
						$hours              = '';
						$minutes            = '';
						$seconds            = '';
						$pickuptime1 = commonfunction::convertphpdate('Y-m-d H:i:s',$get_trip_detail_new[0]['actual_pickup_time']);
						$pickuptime = strtotime($pickuptime1);
						$total_time =abs($pickuptime - time());
									$days       = floor($total_time /86400);        
									$hours      = floor($total_time /3600);     
                        ...
                        if ($request_type == 0) {
							...
							if (count($get_passenger_log_det) == 1) {
                                ...
                                if (($driver_reply == 'A') && ($travel_status == 9)) {
									...
									$liveDistance = isset($currentdistance['distance']) ? round($currentdistance['distance']) : 0;
                                    $message            = array(
                ...
                case 'complete_trip':
                    ...
                    if (!empty($array)) {
                        ...
                        if ($trip_id != null) {
							...
							$airport_parking_cost = $siteinfo_details['airport_parking_charge'];
                                   ...
                                   // $used_wallet_amount  = $get_passenger_log_details[0]['used_wallet_amount'];
                                    ...
                                    if (($travel_status == 2) || ($travel_status == 5)) {
                                       ...
                                       $interval = abs(strtotime($drop_time) - strtotime($actual_pickup_time));
                                        ...
                                        if($airport_pickup == 1 ){
                                          $total_fare           = $airport_parking_cost + $total_fare;
										}
                                      ...
                                      foreach($gateway_details as $k => $g){
										...
										*/
                                        ...
                                        $detail         = array(
											...
											"drop_latitude" => $get_passenger_log_details[0]['drop_latitude'],
											"drop_longitude" => $get_passenger_log_details[0]['drop_longitude'],
                                            "company_tax" => $tax,
                ...
                case 'tripfare_update':
                    ...
                    if ($validator->check()) {
						...
						if (count($get_passenger_log_details) > 0) {
                            $promocode            = (isset($get_passenger_log_details[0]['promocode']) ? $get_passenger_log_details[0]['promocode'] : '');
                           ...
                           $email = $forgot_result[0]['email'];
                            ...
                            if ($array_values['user_type'] == 'P') {
                                ...
                                $replace_variables = array(
                                    ...
                                    REPLACE_SITEURL => URL_BASE,
                                    SITE_DESCRIPTION => $this->app_description,
                                    RESET_LINK => URL_BASE . 'driver/resetpassword/?phone_no=' . $array_values['phone_no'],
                                    REPLACE_COMPANYDOMAIN => $this->domain_name,
                                    REPLACE_COPYRIGHTS => SITE_COPYRIGHT,
                                    REPLACE_COPYRIGHTYEAR => COPYRIGHT_YEAR
                                );
                                ...
                                if (count($get_driver_request) > 0) {
                                    ...
                                    if (!in_array($driver_id, $reject_driversArr)) {
                                        ...
                                        if ($status != '4') {
                                            ...
                                            $result = $api->update_table(MDB_REQUEST_HISTORY,$update_trip_array,'_id',$trip_id);
                                        }
                                    }
                                    $add_rejected_list      = $api->add_rejected_list($post, $rejection_type);
                                    // Driver Statistics ********************/
