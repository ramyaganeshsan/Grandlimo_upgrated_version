...
****************************************************************/
Class Controller_Mobileapi117 extends Controller_Mobile101
{
...
//print_r($company_model_details ); die();
                    ...
                    if(!isset($_REQUEST['latest'])) {
						$indexSpam = array_search(2, array_column($config_array['model_details'], '_id'));
						unset($config_array['model_details'][$indexSpam]);
						$config_array['model_details'] = array_values($config_array['model_details']);
					}
                ...
                */
                case 'passenger_account_registration_step1':
                    ...
                    if($fromIndex == $requireIndex) {
                        ...
                        if ($p_acc_validator->check()) {
                            echo $p_country_code     = isset($mobiledata['country_code'])?$mobiledata['country_code']:'';
...
$intq = array(
    ...
    $taxi_manufacturer  = (isset($get_passenger_log_details[0]['taxi_manufacturer']) ? $get_passenger_log_details[0]['taxi_manufacturer'] :0);
                             ...
                             foreach ($pending_bookings_zero as $key => $val) {
                                ...
                                $pending_bookings_zero[$key]['account_type_name']    = $acc;
                                  switch ($val['travel_status']) {
                                      case 0:
                                        $pending_bookings_zero[$key]['travel_msg'] = "Waiting for diver";
                                        break;
                             }
                            }
            ...
            $passenger_id = $array["passenger_id"];
                        ...
                        $get_taxi_min_speed = $get_passenger_log_det[0]['taxi_min_speed'];
			$app_ver = isset($get_passenger_log_det[0]['app_ver'])?$get_passenger_log_det[0]['app_ver']:'';
                        /* RUNING TIME UPDATE */
                        $hours              = '';
                        $minutes            = '';
                        $seconds            = '';
                        $pickuptime1 = commonfunction::convertphpdate('Y-m-d H:i:s',$get_passenger_log_det[0]['actual_pickup_time']);
                        $pickuptime = strtotime($pickuptime1);
                ...
                case 'tripfare_update':
                    ...
                    if ($validator->check()) {
                        ...
                        if (count($get_passenger_log_details) > 0) {
                            ...
                            if ($array['pay_mod_id'] == 1 || $array['pay_mod_id'] == 6) 
                            {
                                //Inserting to Transaction Table 
                                try {
