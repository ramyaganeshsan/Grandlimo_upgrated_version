...
//DEFsVmFwF8
class Controller_Taxidispatchnode extends Controller_Dispatchadminnode
{
    public function __construct(Request $request, Response $response)
    {
        ...
        $this->tdispatch_model              = Model::factory('taxidispatchnode');
        ...
        $this->api            = Model::factory('mobileapi120');
...
?>" id="no_passengers" />
					...
					<?php
                        ...
                        $this->request->redirect("taxidispatchnode/dashboard");
                    }
                } else {
                    Message::success(__('booking_added'));
                    $this->request->redirect("taxidispatchnode/dashboard");
            ...
            if ($validator->check() || ($dispatch_type == 1)) {
                ...
                if ($passenger_logid) {
                    if ($send_mail == 'S') {
                        ...
                        $from              = $this->siteemail;
                        $subject           = __('pass_account_details') . " - " . $this->app_name;
                        $redirect          = "taxidispatchnode/dashboard";
                        if (SMTP == 1) {
                            include($_SERVER['DOCUMENT_ROOT'] . "/modules/SMTP/smtp.php");
                        } else {
                            // To send HTML mail, the Content-type header must be set
                ...
                ?>
						<form action="<?php
                        echo URL_BASE;
?>taxidispatchnode/dashboard" method="post" id="form_showpopup">
						<input type="hidden" name="show_pass_logid" value="<?php
                        echo $passenger_logid;
?>" id="show_pass_logid" />
						<input type="hidden" name="pickup_latitude" value="<?php
...
?>" id="no_passengers" />
    ...
    //03 Feb 2020 
    public function action_surge_price() {
		
        $model_id = $_REQUEST['model_id'];
		$data = $_REQUEST;
		$update_surge_price = $this->commonmodel->update_surge_price($model_id, $data);
	}
	
	public function action_get_surge_price_model_item() {
		
        $model_id = $_REQUEST['model_id'];
		$data = $_REQUEST;
		$model_item = $this->commonmodel->get_surge_price_model_item($model_id);
		$view       = View::factory('admin/surge_pricing/list')->bind('surge_price_model_details', $model_item);
        $this->template->content    = $view;
        echo $view;
		exit;
	}
    ...
    {
        $view                       = View::factory(TAXI_DISPATCH_NODE . 'sample');
    ...
    public function action_driver_list_with_status()
    {
        ...
        if (count($all_company_map_list) > 0) {
            ...
            foreach ($all_company_map_list as $key => $val) {
                ...
                $driver_page = URL_BASE . "manage/driverinfo/" . $val['driver_id'];
                $output .= "<li class=" . $class . ">";
                $output .= "<span><a href='" . $driver_page . "' target='_blank' title='".$val['taxi_no']."'>" . ucfirst($val['name']). "</a> - ". (isset($val['driver_code']) ? $val['driver_code'] : "")."<p style='color:GoldenRod;'>".$val['model_name']."</p>"."</span>";
               ...
               // $output .= "<p>" . $val['model_name'] . "</p>";
                ...
                $drv_info .= '<p style="color:GoldenRod;">' .' GPS -' .$gps_enable . '</p>';
                $drv_info .= '<span class="info-content">' . $driver_info . '</span>';
                $drv_info .= '</br>';
	...
	$bk_type = isset($_GET['bk_t']) ? $_GET['bk_t'] : "";
		...
		$actpickup_time = ($listings['act_pickuptime'] != '')?commonfunction::convertphpdate('d-M-Y h:i:s A',$listings['act_pickuptime']):'';
                ...
                $tripfare = isset($listings['tripfare'])?$listings['tripfare']:'';
                $createdby_username = isset($listings['createdby_username'])?$listings['createdby_username']:'-';
                $fixedprice      = $tripfare + $wallet_amount_used;
		...
		if(isset($listings['dispatch_time']) && $listings['dispatch_time']!=''){
                ...
                if($listings['book_tag'] == "N") {
                    ...
                    $link = '<a href="'.URL_BASE.'taxidispatchnode/recurrent_booking?booking_key='.$listings['booking_key'].'">' . $listings['book_type'] . '</a>';
                }
		   ...
		   if(isset($listings['dispatch_time']) && $listings['dispatch_time']!=''){
               ...
               /* if ($this->usertype == "A") {
                ...
                if($listings['book_tag'] == "N") {
                    ...
                    $tag = __("recurrent_book");
                    $link = '<a href="'.URL_BASE.'taxidispatchnode/recurrent_booking?booking_key='.$listings['booking_key'].'">' . $listings['book_tag'] . '</a>';
                }
    ...
    public function action_updatebooking()
    {
        ...
        $booking_details = $this->tdispatch_model->get_bookingdetails($passenger_logid,'');
        ...
        if(count($booking_details) > 0)
        {
            ...
            if($performance_status != 1 && $performance_status != 2 && $performance_status != 4)
            {
             ...
             $this->tdispatch_model->update_performance_status($performance_status,$passenger_logid);
        }
    ?>
...
;
    ...
    public function action_manage_booking()
    {
        ...
        $booking_key                = '';
        $type                       = 'N';
        $view                       = View::factory(TAXI_DISPATCH_NODE . 'manage_booking')->bind('validator', $validator)->bind('errors', $errors)->bind('company_tax', $company_tax)->bind('company_timezone', $company_timezone)->bind('model_details', $model_details)->bind('postvalue', $post_values)->bind('booking_key', $booking_key)->bind('type',$type);
...
public function action_pay_details()
    ...
    public function action_get_promocodes(){
        ...
        if(count($res)){
            ...
            foreach($res as $val){
                ...
                $select_field .= '<option value="'.$val['promocode'].'">'.$val['promocode'].' ('.$val['passenger_commission'].'%)'.'</option>';
            }
        }
    ...
    public function action_manage_complete_booking()
    {
        ...
        $booking_key                = '';
        $type                       = 'N';
        $view                       = View::factory(TAXI_DISPATCH_NODE . 'manage_complete_booking')->bind('validator', $validator)->bind('errors', $errors)->bind('company_tax', $company_tax)->bind('company_timezone', $company_timezone)->bind('model_details', $model_details)->bind('postvalue', $post_values)->bind('booking_key', $booking_key)->bind('type',$type);
        $this->template->title      = SITENAME . " | " . __('manage_booking');
        $this->template->page_title = __('manage_booking');
        $this->template->content    = $view;
    }
