...
class Controller_Add extends Controller_Siteadmin
{
    ...
    public function action_passenger_coupon()
    {
        ...
        if ($signup_submit && Validation::factory($_POST)) {
            ...
            if (count($errors) == 0) {
                ...
                $passenger = $add_model->getPassengerCouponDetails($post_values['phone']);
                $isValid = true;
                if(!count($passenger)) {
                    $isValid = false;
                    $errors['phone'] = "Passenger not found.";   
                } else {
                    $passengerCouponAmount = isset($passenger[0]['couponAmount']) ? (int)$passenger[0]['couponAmount'] : 0;
                    if($passengerCouponAmount < COUPON_REDEEN_AMOUNT) {
                        $isValid = false;
                        $errors['phone'] = "Passenger is not eligible for this code.";   
                    }
                }

                $couponDetails = $add_model->getCouponDetails($post_values['coupon_code']);
                if(count($couponDetails) > 0) {
                    $isValid = false;
                    $errors['coupon_code'] = "This coupon already issued.";   
                }           

                if($isValid) {

                    $isDeduducted = $add_model->deductCouponPoints((int)$passenger[0]['_id'], -COUPON_REDEEN_AMOUNT);
                    if($isDeduducted) {
                        ...
                        $isAdded = $add_model->addPassengerCoupon($couponDetails);
                        if($isAdded) {
                            Message::success("Coupon code added successfully.");
                            $this->request->redirect("add/passenger_coupon");
                        } else {
                            $isDeduducted = $add_model->deductCouponPoints((int)$passenger[0]['_id'], COUPON_REDEEN_AMOUNT);
                            Message::success("Failed to add coupon code.");
                            $this->request->redirect("add/passenger_coupon");
                        }
                    }
                }
        ...
        $view = View::factory('admin/add_passenger_coupon_package')->bind('validator', $validator)->bind('errors', $errors)->bind('postvalue', $post_values);
        $this->template->content    = $view;
        $this->page_title = __('Passenger Coupon Code');
        $this->template->title      = SITENAME . " | " . __('Passenger Coupon Code');
        $this->template->page_title = __('Passenger Coupon Code');
        $this->template->content    = $view;
    }

    public function action_add_passenger_coupon()
    {   
        $response = [
            "status" => 0,
        ...
        if (Validation::factory($_POST)) {
            ...
            if (count($errors) == 0) {
                ...
                if(!count($passenger)) {
                    ...
                    if($passengerCouponAmount < COUPON_REDEEN_AMOUNT) {
                        $isValid = false;
                        $response['message'] = "Passenger is not eligible for this code.";   
                    }
                }
                ...
                if(count($couponDetails) > 0) {
                    $isValid = false;
                    $response['message'] = "This coupon already issued.";   
                }           
                ...
                if($isValid) {
                    ...
                    if($isDeduducted) {
                        ...
                        $couponDetails = [
                            ...
                            "passengerId" => (int)$passenger[0]['_id'],
                            "deductedPoints" => COUPON_REDEEN_AMOUNT,
                            "tripId" => (int)$post_values['trip_id'],
                            "dispatcherAdded" => 1
                        ];
                        $isAdded = $add_model->addPassengerCoupon($couponDetails);
                        if($isAdded) {
                            ...
                            $isDeduducted = $add_model->deductCouponPoints((int)$passenger[0]['_id'], COUPON_REDEEN_AMOUNT);
                            $response['status'] = 0;
                            $response['message'] = "Something went worng. pleae try again later.";
                        }
                    }
                }
            }
    ...
    public function action_promotion_push_notification() {
        ...
        $errors = [];
        $post_values = $_POST;
        $is_submit       = arr::get($_REQUEST, 'promotional_push_notification');

        if($is_submit && $_POST) {
            $isValid = true;
            if(!$post_values['title']) {
                $errors['title'] = "Title should not empty";
                $isValid = false;
            } else if(strlen($post_values['title']) <= 4 || strlen($post_values['title']) >= 100) {
                $errors['title'] = "Title should be greater then 4 charecter and less then 100 charecter's";
                $isValid = false;
            }

            if(!$post_values['description']) {
                $errors['description'] = "Description should not empty";
                $isValid = false;
            } else if(strlen($post_values['description']) <= 4 || strlen($post_values['description']) >= 250) {
                $errors['description'] = "description should be greater then 4 charecter and less then 250 charecter's";
                $isValid = false;
            }

            if($isValid) {
                // Process the data and save it in DB.
                $current_date   = date('Y-m-d H:i:s', time());
                $data = [
                ...
                $res = $this->add_model->addPromotionalPushNotification($data);
                if($res) {
                    Message::success("Notification has added into the queue. Soon it will be delivered to customers.");
                    $this->request->redirect("manage/promotion_push_notification");
                }
            }
        }

        $view = View::factory('admin/add_promotional_push_notification')->bind('errors', $errors)->bind('postvalue', $post_values);
        $this->template->content    = $view;
        $this->page_title = "Send promotional push notification.";
        $this->template->title      = SITENAME . " | " . "Send promotional push notification.";
        $this->template->page_title = "Send promotional push notification.";
        $this->template->content    = $view;
    }
}
