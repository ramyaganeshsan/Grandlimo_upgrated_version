...
Class Controller_Users extends Controller_Website
{
    ...
    public function action_index()
    {
        ...
        */
        ...
        $this->template->title =$this->title;
            ...
            $home_page_certificate = $this->cms->get_website_certificate();
    ...
    public function action_booking_request(){
        ...
        if($_GET){
            //print_r($_GET);exit;
//booking_request?pickup=testplace&drop=chennai&phone=7777777&email=raj@test.com&car_type=grand&p_date=24-07-2018&p_time=09:22&passenger=12
            ...
            if($pickup_loc!='' && $drop_loc!='' && $phone!='' && $car_type!='' && $p_date!='' && $p_time!='' && $passenger!=''){
                ...
                if($booking_type==2){
                    ...
                    $pickup_type = 'Airport Pickup/Drop off';
                }

            $booking_date = date('d-m-Y H:i a', time());

               $replace_variables =  ['##PICKUP##'=> $pickup_loc,'##DROP##'=> $drop_loc,'##PHONE##'=> $phone,'##EMAIL##'=> $email,'##CAR_TYPE##'=> $car_type,'##DATE##'=> $p_date,'##TIME##'=> $p_time,'##PASSENGER##'=> $passenger,'##BAGGAGE##'=>$baggage,'##FLIGHT_NO##'=> $flight_no,REPLACE_SITEURL => URL_BASE,  REPLACE_COPYRIGHTS => SITE_COPYRIGHT, REPLACE_COPYRIGHTYEAR => COPYRIGHT_YEAR,REPLACE_SITENAME => $this->app_name,'##BOOKING_TIME##'=>$booking_date,'##PICKUP_TYPE##'=>$pickup_type];

               $message = $this->emailtemplate->emailtemplate(DOCROOT . TEMPLATEPATH . 'booking_request.html', $replace_variables);

                    $from     = $this->siteemail;
                    $to = 'rajasekar.m@ndot.in';//SITE_EMAIL_CONTACT;//
                ...
                Message::success(__('booking_request_sent_successfully'));    
                //$this->request->redirect(URL_BASE);
            }else{
    ...
    public function action_booking_success(){

        //echo 4444;exit;
        $booking_confirm_content = $this->cms->get_website_page_content('booking_confirm_page');
        
        //$_SESSION['booking_confirm'] = 1;$_SESSION['booking_model'] = 3;
        if(isset($_SESSION['booking_confirm']) && $_SESSION['booking_confirm'] ==1 ){

            unset($_SESSION['booking_confirm']);
            $model_id = isset($_SESSION['booking_model'])?$_SESSION['booking_model']:'';

            $model_detail = $this->cms->get_model_detail($model_id);

            $website_model_image = isset($model_detail[0]['website_model_image']) ? $model_detail[0]['website_model_image']:'';
            //print_r($model_detail[0]['website_model_image']);exit;
            if(file_exists($_SERVER["DOCUMENT_ROOT"].'/public/uploads/model_image/'.$website_model_image) && !empty($website_model_image) ){

                $booking_image = URL_BASE.'public/uploads/model_image/'.$website_model_image;
            }else{
                $booking_image = URL_BASE.'public/images/booked_car.png';//URL_BASE.'public/images/pricing_block_img1.png';
            }
            ...
            $booking_confirm_content = isset($booking_confirm_content[0]) ? $booking_confirm_content[0]: [];

            $view= View::factory(USERVIEW.'booking_success')->bind('lang',$this->lang)->bind('booking_image',$booking_image)->bind('booking_confirm_content',$booking_confirm_content)->bind('booking_id',$booking_id);
            $this->template->content = $view;
    ...
    public function action_save_booking_request()
    {
        //echo $this->lang; exit;
        /**To Set Errors Null to avoid error if not set in view**/
        $errors        = [];
        ...
        /**Send entered values to model for validation**/
        $validator  = $this->cms->validate_save_booking_request(arr::extract($_POST, [
                        'pickup',
                        'drop',
                        'phone',
                        'email',
        ...
        if ($validator->check()) {        
                ...
                $postvalues['firstname'] = isset($_POST['pass_name']) ? $_POST['pass_name'] : "";

                $req_result = $this->cms->save_booking_request($postvalues);
                $model_id = isset($postvalues['car_model'])?$postvalues['car_model']:'';
                $_SESSION['booking_id'] = $req_result;
                $_SESSION['booking_model'] = $model_id;
                $_SESSION['booking_confirm'] = 1; 

                $response = ['status'=>200,'message'=>'Booking request sent successfully'];
    ...
    public function action_get_passenger_details() {
        if(isset($_POST['phone']) && !empty($_POST['phone'])) {
            $result = $this->cms->getPassengerDetails($_POST['phone'], $_POST['country_code']);
