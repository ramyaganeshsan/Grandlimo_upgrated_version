...
class Controller_Taxidispatch extends Controller_Dispatchadmin
{
     ...
     public function action_dashboard()
    {
        ...
        $corporate_list = $this->tdispatch_model->get_corporate_list();
      ...
      //  echo $create_submit;exit();
        if ($create_submit || $dispatch_submit) {
            ...
            if ($validator->check()) {
                ...
                /* Sasidharan Feb 07 2023 And Add $surgePriceStatus == 1 */
                // $surgePriceStatus = commonfunction::isSurgePriceActiveToday();
                $surgePriceStatus = commonfunction::isSurgePriceActiveToday($post_values['pickup_time']);

                if(SURGE_PRICING == 1 && $surgePriceStatus == 1) {
                	//26 feb 2021
                	$admin_companyid = 1;
                	$request = [];
                    ...
                    if(!$booking_details[0]['applied_corporate_promocode']) {

                        /* Sasi */
                        $passengerId     = $booking_details[0]["passengers_id"];
                        ...
                        // echo $surgePriceStatus;
                        // echo SURGE_PRICING;
                        // exit;

    		            $update_surge_price = $this->common_model->update_trip_data($post_values['taxi_model'],$passenger_logid,$free_drivers,$post_values['pickup_date'],$now_after,$passengerId,true);
                ...
                if (($passenger_logid)) {
					...
					if($post_values['passenger_id'] != "") { 
						...
						if(isset($det['result']) && count($det['result']) > 0){
                            ...
                            $notify_msg = "Booking created by dispatcher on " . $post_values['pickup_date'] . " - تم إنشاء الحجز بواسطة المنسق في " . $post_values['pickup_date'];
							$push_message = ['message'=>$notify_msg,'trip_id'=>$passenger_logid,'status'=>11];
							$title='';
        ...
        if ($p_device_type == 1) {
                    ...
                    $this->api->send_ios_push_via_node($p_device_token, $push_message);
                     }
...
?>" id="no_passengers" />
            ...
            if ($validator->check() || ($dispatch_type == 1)) {
                ...
                /* Sasidharan Feb 07 2023 And Add $surgePriceStatus == 1 */
                // $surgePriceStatus = commonfunction::isSurgePriceActiveToday();
                $surgePriceStatus = commonfunction::isSurgePriceActiveToday($post_values['edit_pickup_date']);
...
?>" id="no_passengers" />
        ...
        /* Sasidharan Sep 07 2022 also bind this in view */
        $airportTerminals = $this->manage_model->getAirportTerminals();
        $formattedAirportTerminals = [];
        foreach ($airportTerminals as $airport) {
            $formattedAirportTerminals[$airport["name"]."@-@".$airport["latitude"]."@-@".$airport["longitude"]] = $airport["name"];
        }

        $view                       = View::factory(TAXI_DISPATCH . 'dashboard')->bind('validator', $validator)->bind('errors', $errors)->bind('company_tax', $company_tax)->bind('company_timezone', $company_timezone)->bind('model_details', $model_details)->bind('postvalue', $post_values)->bind('all_company_map_list', $all_company_map_list)->bind('show_popup', $_REQUEST)->bind('current_latitude', $current_latitude)->bind('current_longitude', $current_longitude)->bind('get_active_company_details', $get_active_company_details)->bind('total_sum', $total_sum)->bind('promo_res_edit',$promo_res_edit)->bind('total_sum_day',$total_sum_day)->bind('corporate_list',$corporate_list)->bind("airportTerminals",$formattedAirportTerminals);
    ...
    public function action_all_booking_list_manage()
    {
	  ...
	  $bk_type = isset($_GET['bk_t']) ? $_GET['bk_t'] : "";
		...
		$actpickup_time = ($listings['act_pickuptime'] != '')?commonfunction::convertphpdate('d-M-Y h:i:s A',$listings['act_pickuptime']):'';
                ...
                $corporate_promocode = (isset($listings['corporate_promocode']) && !empty($listings['corporate_promocode'])) ? $listings['corporate_promocode'] : 0;
                // if($corporate_promocode) {
                    /* Sasidharan aug 4 2022 */
                    $promocode = isset($listings['promocode']) ? $listings['promocode'] : "";
                    $passenger_phone = $passenger_phone." (".$promocode.")";
                    // $passenger_phone = $passenger_phone."("."CORP".")";
                // }
                ...
                /* Sasi */
                $discount_wallet_amount_used  = (!empty($listings['discount_wallet_amount_used'])) ? $listings['discount_wallet_amount_used'] : 0;
                $wallet_amount_used += $discount_wallet_amount_used;
                ...
                $passenger_corporate_name  = (!empty($listings['corporate_name'])) ? $listings['corporate_name'] : '';

                // Sasidharan Jan 17 2025
                $isCouponStillAvailable = false;
                $couponAmount  = (!empty($listings['couponAmount'])) ? $listings['couponAmount'] : 0;
                $startDate = date('Y-m-d', explode(" ",COUPON_START_DATE)[1]);
                $endDate = date('Y-m-d', explode(" ",COUPON_END_DATE)[1]);
                $currentDate = new DateTime();
                $start = new DateTime($startDate);
                $end = new DateTime($endDate);
                $isValidDate = $currentDate >= $start && $currentDate <= $end;
                if(COUPON_STATUS && $isValidDate && $couponAmount >= COUPON_REDEEN_AMOUNT) {
		...
		if(isset($listings['dispatch_time']) && $listings['dispatch_time']!=''){
                ...
                if ($listings['travel_status'] == 0) {
                  ...
                  //  $edit          = '<a href="javascript:;" class="edit-ico status ' . $trcolor . '" name="edit" id="addtr_' . $pass_logid . '" value="' . __('dispatch') . '" ><i class="glyphicon glyphicon-edit">&nbsp;</i></a>';
                    ...
                    $cancelation_time_string = isset($listings['cancelation_time_string']) ? $listings['cancelation_time_string'] : "-";

                    $travel_status = '<div style="color:' . $status_color . ';">' . __('cancel_by_passenger') . '</div>';
                    $status_button = '<button type="submit" class="btn btn-outline btn-primary btn-xs cancelled" name="update_dispatch" id="update_dispatch_' . $pass_logid . '" value="' . __('dispatch') . '" ' .'title="' . $cancelation_time_string. '" >'. __('cancelledd') . '</button>';
                ...
                $passenger_corporate_name_text = '';
                if($passenger_corporate_name != ''){
                    $passenger_corporate_name_text = "<br/>(".$passenger_corporate_name.")";
                }

                $passCouponSymbol = "";
                if($isCouponStillAvailable) {
                    // $passCouponSymbol = "(<span style='font-size:16px;padding:0px 4px'>🎟️</span>)";
                    $passCouponSymbol = "<img onclick='handleCouponPopup(".$passenger_phone.",".$pass_logid.",".$driver_id.")' src='https://grandlimo.indiprotechnologies.com/public/images/passengerCoupon.png' alt='coupon' style='width:20px;height:20px;cursor:pointer;' />";
                }



                /* Sasidharan july 28 2022 */
                $bookedBy = $createdby_username != "-" ? "W" : "A";
                ...
                if ($listings['travel_status'] == 1) {
                    $op[] .= '<td width="5%" style="color:blue;text-decoration:none;"><a style="text-decoration:none;" href="' . URL_BASE . 'transaction/transaction_details/' . $pass_logid . '">' . $pass_logid .'('.$bookedBy.')'. '</a></td>';
                } else {
                    $op[] .= '<td width="5%" style="color:blue;">' . $pass_logid .'('.$bookedBy.')'. '</td>';
                }
                $op[] .= '<td width="7%"><span>' . ucfirst($passenger_name).'<br/>'.$pass_notes_img.'</span>'.$passenger_pending_amt_txt.$passenger_corporate_name_text.$passCouponSymbol.'</td>';
               /* if ($this->usertype == "A") {
    ...
    public function action_all_booking_list_manage_all()
    {
        ...
        if (count($get_all_booking_list) > 0) {
            foreach ($get_all_booking_list as $listings) {
                ...
                /* Sasi */
                $discount_wallet_amount_used  = (!empty($listings['discount_wallet_amount_used'][0])) ? $listings['discount_wallet_amount_used'][0] : 0;
                $wallet_amount_used += $discount_wallet_amount_used;
    ...
    public function action_updatebooking()
    {
        ...
        if (isset($_REQUEST['driver_id'])) {
            $current_trips = $this->tdispatch_model->check_trip_requests_exits($_REQUEST['driver_id']);
            if(count($current_trips) == 0){
                $update_booking = $this->tdispatch_model->updatebooking_logid($_REQUEST);
            } else{
                Message::error(__('Driver Already In Trip'));
                echo 0;exit;
            }
        } else {
...
;
            ...
            $p_device_id     = isset($dets['p_device_id'])?$dets['p_device_id']:'';
            $p_device_type     =  isset($dets['p_device_type'])?$dets['p_device_type']:1;
             $title 		 ="Your driver is on the way - سائقك في الطريق إليك";
...
if ($p_device_token != '') {
     ...
     if ($p_device_type == 1) {
            $p_send_notification = $this->api->send_passenger_mobile_pushnotification($p_device_token,$p_device_type,$pushmessage,$this->customer_google_api,$title);
     }else if ($p_device_type == 2) {
        // IOS
        $this->api->send_ios_push_via_node($p_device_token, $pushmessage);
    }
...
public function action_pay_details()
    ...
    public function action_validate_promocode(){
         ...
         $check_promo = $this->tdispatch_model->checkpromocode($promo_code, $passenger_number, $default_companyid='',$passenger_id,$passenger_info);
            //echo $check_promo;
            //exit;
            if ($check_promo == 0) {
                $msg = [
    ...
    /* Sasidharan apr 16 - 2022 */
    public function action_get_corporate_promocode() {
        $name             = [];
        $like_q           = arr::get($_REQUEST, 'query');
        $like_q           = urlencode($like_q);

        // $ee = explode("-","TES333-(10)(12)(0)");
        // echo "<pre>";
        // print_r($ee);

        $user_details = $this->tdispatch_model->getCorporatePromocode($like_q, 3);
        foreach ($user_details as $details) {

            $promocode = isset($details['promocode']) ? $details['promocode'] : "0";
            $total_used = isset($details['total_used']) ? $details['total_used'] : "0";
            $total_airport_trips = isset($details['total_airport_trips']) ? $details['total_airport_trips'] : "-";
            $promo_limit = isset($details['promo_limit']) ? $details['promo_limit'] : "0";

            $name[] = $promocode."-(".$promo_limit.")"."(".$total_airport_trips.")"."(".$total_used.")";
        }
        echo json_encode($name);
        exit;
    }
    ...
    public function action_check_dispatcher_password() {
        ...
        if($username == "" || $password == "") {
            ...
            $response = $this->tdispatch_model->checkUsernameAndPassword($username, $password);
    ...
    public function action_get_on_going_driver_list() {
        $response = $this->tdispatch_model->getOnGoingDriverList();
        echo json_encode($response);
        exit;
    }

    public function action_check_push_notification() {
        $push_message = ['message'=>"Hello this is a test message",'trip_id'=>1234,'status'=>11];
        $p_send_notification = $this->api->send_passenger_mobile_pushnotification("8c8e0a8224348056d520e99a6cec88c843b4f271bff4ebc5fee589995e1ecc3d",2,$push_message,"","This is a title");
        echo "<pre>";
        echo "Before message";
