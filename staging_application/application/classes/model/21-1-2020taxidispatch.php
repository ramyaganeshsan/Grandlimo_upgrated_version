   ...
   /* public function get_all_booking_list_all($array)
    ...
    {   //echo $this->session->get('name');
        ...
        $update_array = array(
            ...
            'zone_fare_applicable' => isset($post['edit_zone_fare_applicable'])?(int)$post['edit_zone_fare_applicable']:(int)0,
            'zone_fare_type' => isset($post['edit_zone_fare_type'])?(int)$post['edit_zone_fare_type']:(int)0,
    ...
    public function dispatcher_booking_list($array)
    {
       ...
       /* $current_time_hour            = (int)date('H', strtotime($array['current_time']));
        ...
        if( isset($array['book_type']) && $array['book_type'] != 'undefined')
        {
            ...
            else if($array['book_type'] == __('zone'))
            {
                $match_array['zone_fare_applicable'] = 1;
            }
            else
            {
                $match_array['book_type'] = $array['book_type'];
                $match_array['airport_pickup'] = '';
                $match_array['zone_fare_applicable'] = '';
  ...
  $arguments = array(array(
   ...
   /* }
    ...
    public function get_promocodes(){
        ...
        $current_time = convert_timezone('now', TIMEZONE);
         $match_arg = array('auto_generated_promocode'=>array('$ne'=>(int)1),'start_date'=>array('$lte' => New MongoDate(strtotime($current_time))),'expire_date'=>array('$gte' => New MongoDate(strtotime($current_time))));
   ...
   public function get_last_four_drivers_list($passenger_id,$trip_id,$model_id)
   {
        ...
        $common_arguments = array(            
            ...
            $field_arguments = array(
                ...
                array(
                    '$project' => array(
                        ...
                        "driver_model"=>'$model.model_name',
                        "driver_model_id"=>'$model._id',
                        "driver_shift_status"=>'$driver.shift_status',
                        "driver_status"=>'$driver.status',
                        "driver_taxi_id"=>'$taxi._id',
                        "driver_company_id"=>'$taxi.company_id'
                    )
                ),
                array('$skip'   => (int)0 ),
                array('$limit'  => (int)4 ),
            ...
            if(count($res)){
                foreach($res as $k=>$val){
                    ...
                    $driver_model_id = isset($val['driver_model_id'][0])?$val['driver_model_id'][0]:'';
                    $driver_taxi_id = isset($val['driver_taxi_id'])?$val['driver_taxi_id']:'';
                    $driver_company_id = isset($val['driver_company_id'])?$val['driver_company_id']:1;
                    $driver_id = isset($val['driver_id'])?$val['driver_id']:0;

                    $driver_shift_status = isset($val['driver_shift_status'])?$val['driver_shift_status']:'';
                    $driver_status = isset($val['driver_status'])?$val['driver_status']:'';

                    $status_text = '';
                    if($driver_status == 'F') { $status_text = "Free ".$driver_shift_status;};
                    if($driver_status == 'B') { $status_text = "Busy";};
                    if($driver_status == 'A') { $status_text = "Active";};

                    $status_button = "<button type='submit' onclick='assign_driver_from_last_four(".$trip_id.",".$driver_id.",".$driver_taxi_id.",".$driver_company_id.")' class='btn btn-outline btn-primary btn-xs update_dispatch' name='update_dispatch'  value='Assign' >Assign</button>";


                    if($val['driver_name'] != '' && $val['driver_code']){
                        if($driver_status == 'F' && $driver_model_id == $model_id ){
                            $drivers_list .= "<li style='margin:2px;'>".ucfirst($val['driver_name'])."(".$val['driver_code'].")"."  - ".$driver_model." - ".$status_text." ".$status_button."</li>";
                        }
                        else {
                            $drivers_list .= "<li style='margin:2px;'>".ucfirst($val['driver_name'])."(".$val['driver_code'].")"."  - ".$driver_model." - ".$status_text."</li>";
                        }
                    }
                }
            }
            ?> <script>
            function assign_driver_from_last_four(trip_id,driver_id,taxi_id,company_id) {
                var dataS = "pass_logid="+trip_id+"&driver_id="+driver_id+"&taxi_id="+taxi_id+"&driver_away_in_km=0&company_id="+company_id;                
                ...
                $.ajax({
                    ...
                    {
                         socket.emit('dispatcher_new_booking',emitData);    
