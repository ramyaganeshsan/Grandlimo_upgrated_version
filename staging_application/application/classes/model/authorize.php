...
class Model_Authorize extends Model
{
    ...
    //print_r($post);exit;	
        ...
        $startdate = $post['lateral_start_date'];
        $enddate = $post['lateral_end_date'];
		...
		if($post['pay_by'] != 3) {
			$data = array(
                ...
                'wallet_max_limit_check' => isset($post['wallet_max_limit_check'])?(int)$post['wallet_max_limit_check']:0,
                'surge_price_disable' => isset($post['surge_price_disable'])?(int)$post['surge_price_disable']:0,
    ...
    public function login_details_by_passengerid($userid)
    {
        ...
        //MongoDB
        $result = $this->mongo_db->find_one(MDB_PASSENGERS,array('_id'=>(int)$userid),array('_id','name','lastname','email','address','phone','discount','pay_by','trip_amt_limit','vip_user','notes','country_code','lateral_start_date','lateral_end_date','wallet_max_limit_check','surge_price_disable'));
        return (!empty($result)) ? $result : array();
    }
