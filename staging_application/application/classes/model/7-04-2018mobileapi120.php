    ...
    public function passenger_trip_list($ids = "", $status = "", $driver_reply = "", $createdate = "", $start = null, $limit = null, $company_id)
    {
       ...
       // $match_query['passengers_id'] = array('$in' =>$ids);
        ...
        $arguments = array(
            ...
            array(
                '$unwind' => array('path'=>'$transaction','preserveNullAndEmptyArrays'=>true)
            ),
            array('$lookup' =>
                array(
                    'from'=>MDB_PASSENGERS,
        ...
        if(!empty($result['result']) && !empty($completed_result['result']))
        array_merge($result,$completed_result);

        if(!empty($result['result']) && !empty($cancelled_result['result']))
        array_merge($result,$cancelled_result);

        if(!empty($result['result']) && !empty($missed_result['result']))
        array_merge($result,$missed_result);

        if(!empty($result['result']) && !empty($rejected_result['result']))
        array_merge($result,$rejected_result);
