...
Class Model_Edit extends Model
{
   ...
   /* public static function checkassigntaxi($enddate, $post, $uid)
       ...
       public function coupon_package_details($uid)
    {
		...
		//MongoDB
		$res = $this->mongo_db->find_one(MDB_COUPON_PACKAGE,['_id'=>(int)$uid],['_id','coupon_package_name','coupon_package_description','corporate_commission','passenger_commission','coupon_package_status','fixed_fare_apply']);
		return (!empty($res))?$res:[];
    }
 ...
 public function update_corporateimage($image,$userid)
{
    ...
    public function surge_pricing_details($uid)
    {
        //MongoDB
        $arguments = [
            ['$match'  => ['_id' => (int)$uid, 'model_status' => 'A']],
            [
                '$project' => [
                    'id' => '$_id',
