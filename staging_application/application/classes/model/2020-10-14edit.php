...
Class Model_Edit extends Model
{
   ...
   /* public static function checkassigntaxi($enddate, $post, $uid)
       ...
       public function coupon_package_details($uid)
    {
		...
		//MongoDB
		$res = $this->mongo_db->find_one(MDB_COUPON_PACKAGE,array('_id'=>(int)$uid),array('_id','coupon_package_name','coupon_package_description','corporate_commission','passenger_commission','coupon_package_status','fixed_fare_apply'));
		return (!empty($res))?$res:array();
    }
 ...
 public function update_corporateimage($image,$userid)
{
    ...
    public function surge_pricing_details($uid)
    {
        //MongoDB
        $arguments = array(
            array('$match'  => array('_id' => (int)$uid, 'model_status' => 'A')),
            array(
                '$project' => array(
                    'id' => '$_id',
		...
		return (!empty($res))?$res['result'][0]:array();
    }
