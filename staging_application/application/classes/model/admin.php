...
class Model_Admin extends Model
{
    ...
    public function get_all_searchpassenger_list($keyword = "", $status = "", $company = "", $customer_type="",$offset = "", $val = "", $find_count = false)
    {
        ...
        if($customer_type == 1)
        {
            $match_query = ['vip_user' => (int)$customer_type ];
            $srch_query = array_merge($srch_query,$match_query);
        }
        if($customer_type == 0)
        {
           $match_query = ['vip_user' => ['$ne'=>1]];
           $srch_query = array_merge($srch_query,$match_query);
        }
...
'dispatcher_notification_time' => (int) $post_value_array['dispatcher_notification_time'],
   ...
   /* public function get_availabletaxi_list_count()
    ...
    public function all_passenger_wallet_list($keyword='',$customer_type='',$offset, $val, $find_count = FALSE)
    {
        ...
        if(!empty($keyword)){
            $match_query = [ "\$and" => [["\$or"=>[[ 'name' => new \MongoDB\BSON\Regex($keyword, 'i')] , [ 'lastname' => new \MongoDB\BSON\Regex($keyword, 'i') ],[ 'email' => new \MongoDB\BSON\Regex($keyword, 'i') ], [ 'phone' => new \MongoDB\BSON\Regex($keyword, 'i') ] ] ] ] ];
        }else{
            $match_query['name'] = ['$ne' => ''];
        }
    ...
    public function get_admin_dashboard_data_new() {
        ...
        $filter = [
            "travel_status" => 1,
            "actual_pickup_time" => [
                '$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($firstDayOfThisMonth) * 1000),
                '$lte' => new \MongoDB\BSON\UTCDateTime(strtotime($currentDate) * 1000)
            ]
        ];
