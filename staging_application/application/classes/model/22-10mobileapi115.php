...
facebook_secretkey,facebook_share,twitter_share,site_logo FROM " . SITEINFO . " limit 0,1 ";*/
    ...
    {
		$result = $this->mongo_db->find_one(MDB_PASSENGERS,['_id'=>(int)$id,'user_status'=>'A'],['_id','salutation','name','lastname','email','phone','deviceid','devicetoken','devicetype','child_exist','allow_creditcard','login_status']);
		return $result;
    }
	
	public function passenger_login_details_with_phone($phone)
    {
		$result = $this->mongo_db->find_one(MDB_PASSENGERS,['phone'=>$phone,'user_status'=>'A'],['_id','salutation','name','lastname','email','phone','deviceid','devicetoken','devicetype','child_exist','allow_creditcard','login_status']);
		return $result;
    }
    ...
    {        
		...
		$res = $this->mongo_db->find(MDB_PASSENGERS,$match_query,[]);
        ...
        if (count($result1) == 0) {
            ...
            if ($devicetoken != "") {
				...
				$update_device_token_result = $this->mongo_db->Update(MDB_PASSENGERS,['_id'=>(int)$inc_id],['$set'=>$update_array],['upsert'=>false]);                
            }
            return 2;
			...
			$ress = $this->mongo_db->find_one(MDB_PASSENGERS,$match_query,['_id']);
            ...
            if ($counts != 0) {
                ...
                if ($company_id != '' && $company_id != 0) {
					 $match = [ "\$and" => [['passenger_cid' => (int)$company_id ],
													  ['email' => $email ],
													  ["\$or"=>[[ 'name' => ""] , [ 'lastname' => "" ] ] ] ] ];
                } else {
				...
				$update_device_token_result = $this->mongo_db->update(MDB_PASSENGERS,$match_query,[$set=>$update_array],['upsert'=>false]);
    ...
    {
		$result = $this->mongo_db->find_one(MDB_PASSENGERS,['_id'=> (int)$passid],['_id','name','lastname','salutation','email','profile_image','phone','civilid','civilid_front','civilid_back','civilid_front_thumb','civilid_back_thumb','user_status','login_from']);
       return (!empty($result)) ? $result: [];		
    }
       ...
       /* $company_id  = $this->get_company_id($val['driver_id']);
        ...
        if (count($driver_availability_result) == 0) {
            ...
            if ((int)$now_after == 0) {
				...
				$rs = $this->mongo_db->find(MDB_PASSENGERS_LOGS,[],['_id'])->sort(['_id'=>-1])->limit(1);
				$res = (!empty($rs))?iterator_to_array($rs):[1];
				reset($res);
				$first_key = key($res);
				$inc_id = $first_key+1;
    ...
    {
		$update = ['comments' => $comments,'rating' => (int)$ratings];
		$result = $this->mongo_db->Update(MDB_PASSENGERS_LOGS,['_id'=>(int)$log_id],
...
WHERE  " . PASSENGERS_LOG . ".`passengers_log_id` =  '$passengerlog_id' $company_condition";
    ...
    *
    ...
    public function siteinfo_details()
    {
        ...
        $result = $this->mongo_db->find_one(MDB_SITEINFO,[],["admin_commission","referral_discount","currency_format","referral_amount","referral_settings","wallet_amount1","wallet_amount2","wallet_amount3","wallet_amount_range","airport_parking_charge"]);
        return (isset($result)?$result:[]);
    }
...
JOIN  " . PEOPLE . " ON (  " . PEOPLE . ".`id` =  " . PASSENGERS_LOG . ".`driver_id` ) JOIN  " . TAXI . " ON (  " . TAXI . ".`taxi_id` =  " . PASSENGERS_LOG . ".`Taxi_id` )  WHERE driver_id = '$userid' AND " . PASSENGERS_LOG . ".bookby = '" . BOOK_BY_CONTROLLER . "' AND (travel_status = '9' or travel_status = '3') AND driver_reply = '" . $driver_reply . "'  $condition  $orderby";
    ...
    public function get_driver_current_status($id, $company_id = '')
    {
        ...
        //print_r($result);              
        return $result;*/
		$res = $this->mongo_db->find(MDB_DRIVER_INFO,['_id'=>(int)$id],['status', 'loc.coordinates'])->sort(['_id'=>1]);
		$result = iterator_to_array($res);
		return (isset($result)? reset($result) :[]);  
    }
   ...
   public function edit_check_phone_passengers($phone = "", $passenger_id = '', $company_id = '')
    ...
    public function get_passenger_phone_by_id($id)
    {
		...
		$res = $this->mongo_db->find_one(MDB_PASSENGERS,['_id'=>(int)$id],['phone']);
    ...
    public function getpromodetails($promo_code = "", $passenger_id = "",$passeger_number="")
    {
        ...
        $promo_fetch = $this->mongo_db->find_one(MDB_PASSENGERS_PROMO,['promocode'=>$promo_code,'customer_number'=>$passeger_number],["promocode","package","promo_used","promo_limit"]);
        if (count($promo_fetch) > 0) {
            ...
            $promo_user_count = $this->mongo_db->find_one(MDB_PASSENGERS_LOGS,['promocode'=>$promo_code,'passenger_id'=>(int)$passenger_id]);
      ...
      public function checkpromocode($promo_code = "", $customer_number = "", $company_id = "",$passenger_id="")
    {
		...
		$project = ['promocode','promo_discount','promo_used','start_date','expire_date','promo_limit'];		
     ...
     public function check_new_request_tripid($taxi_id = null, $company_id = null, $trip_id, $driver_id, $company_all_currenttimestamp, $driver_reply, $operator_id = 0)
    {
        ...
        if (count($result) > 0) {
            if ($driver_reply != 'C') {
                ...
                $update_result     = $this->mongo_db->update(MDB_REQUEST_HISTORY,['_id'=>(int)$trip_id],['$set'=>$update_trip_array],['upsert'=>false]);
                ...
                if ($temp_driver == '') {
                    ...
                    $update_result         = $this->mongo_db->update(MDB_REQUEST_HISTORY, ['_id'=>(int)$trip_id],['$set'=>$update_trip_array_one],['upsert'=>false]);
                ...
                if (count($comp_result) == 0) {
                    ...
                    $update_result         = $this->mongo_db->update(MDB_REQUEST_HISTORY, ['_id'=>(int)$trip_id],['$set'=>$update_trip_array_one],['$upsert'=>true]);
                    if ($operator_id != 0 && $user_type_dets['user_type'] == 'A') {
                        $update_log_array_driver = [
                            "driver_id" => 0,
                            "taxi_id" => 0,
   ...
   public function get_trip_detail_only($passengerlog_id = "")
    ...
    public function get_driver_cancelled_trips($driver_id, $company_id)
    {
        ...
        $start_time               = $get_company_time_details['start_time']; //Start time
        $end_time                 = $get_company_time_details['end_time']; //end time
        $result = $this->mongo_db->count(MDB_PASSENGERS_LOGS,['travel_status'=>9,'driver_reply'=>"C",'createdate'=>['$gte'=>new \MongoDB\BSON\UTCDateTime(strtotime($start_time) * 1000),'$lte'=>new \MongoDB\BSON\UTCDateTime(strtotime($end_time) * 1000)]]);
        return (isset($result))?$result:0;
    }
	...
	public function get_child_trip_count($passId) {
		$common_arguments = [
			...
			[
				'$match' =>['parent_id'=>(int)$passId, 'plog.travel_status' => (int)2],
			],
			[
					'$group' => [
						'_id' => NULL,
