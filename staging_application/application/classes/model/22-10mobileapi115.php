...
facebook_secretkey,facebook_share,twitter_share,site_logo FROM " . SITEINFO . " limit 0,1 ";*/
    ...
    {        
		...
		$res = $this->mongo_db->find(MDB_PASSENGERS,$match_query,array());
		$result1 = iterator_to_array($res);
        ...
        if (count($result1) == 0) {
			...
			$result = $this->mongo_db->insert(MDB_PASSENGERS,$insert_array);
            if ($devicetoken != "") {
                $update_array               = array(
                    "device_token" => $devicetoken,
                    "device_id" => $device_id,
				...
				$update_device_token_result = $this->mongo_db->Update(MDB_PASSENGERS,array('_id'=>(int)$inc_id),array('$set'=>$update_array),array('upsert'=>false));                
            }
            return 2;
			...
			$ress = $this->mongo_db->find_one(MDB_PASSENGERS,$match_query,array('_id'));
            $counts = (isset($ress)?count($ress):0);
            if ($counts != 0) {
				...
				$update_device_token_result = $this->mongo_db->update(MDB_PASSENGERS,$match_query,array($set=>$update_array),array('upsert'=>false));
    ...
    public function register_facebook_user_old($accessToken = "", $uid = "", $otp = "", $referral_code = "", $fname = "", $lname = "", $email = "", $image_name = "", $devicetoken = "", $device_id = "", $devicetype = "", $company_id = "")
    {
...
WHERE  " . PASSENGERS_LOG . ".`passengers_log_id` =  '$passengerlog_id' $company_condition";
    ...
    *
    ...
    public function siteinfo_details()
    {
        ...
        $result = $this->mongo_db->find_one(MDB_SITEINFO,array(),array("admin_commission","referral_discount","currency_format","referral_amount","referral_settings","wallet_amount1","wallet_amount2","wallet_amount3","wallet_amount_range","airport_parking_charge"));
        return (isset($result)?$result:array());
    }
...
WHERE  " . PASSENGERS_LOG . ".`passengers_log_id` =  '$passengerlog_id'";
    ...
    public function get_trip_detail($passengerlog_id = "", $passenger_id = "")
    {
		...
		$arguments = array(
			...
			/*array(
				'$project' => array(
					'amt' => array('$add' => array('$amt','$wallet_amount')),
					...
					//'metric'=>'$metric'
					'actual_distance' => array('$cond' => array(array('$eq' => array('$actual_distance',array())),0,'$actual_distance')),
					'metric' => array('$cond' => array(array('$eq' => array('$metric',array())),0,'$metric')),
					'job_ref' => array('$cond' => array(array('$eq' => array('$job_ref',array())),0,'$job_ref')),
					'payment_type' => array('$cond' => array(array('$eq' => array('$payment_type',array())),0,'$payment_type')),
				)
			),*/
			array(
				'$match' => $match_query
...
JOIN  " . PEOPLE . " ON (  " . PEOPLE . ".`id` =  " . PASSENGERS_LOG . ".`driver_id` ) JOIN  " . TAXI . " ON (  " . TAXI . ".`taxi_id` =  " . PASSENGERS_LOG . ".`Taxi_id` )  WHERE driver_id = '$userid' AND " . PASSENGERS_LOG . ".bookby = '" . BOOK_BY_CONTROLLER . "' AND (travel_status = '9' or travel_status = '3') AND driver_reply = '" . $driver_reply . "'  $condition  $orderby";
   ...
   public function edit_check_phone_passengers($phone = "", $passenger_id = '', $company_id = '')
    ...
    public function get_passenger_trips_bymonth($pagination, $booktype, $ids, $status = "", $driver_reply = "", $createdate = "", $start = null, $limit = null, $fdate, $tdate)
    {
		...
		$args = array(//array('$unwind' => '$passenger'),
						array(
							'$lookup' => array(
								'from' => MDB_PASSENGERS,
								'localField' => "passengers_id",
    ...
    public function send_passenger_mobile_pushnotification($d_device_token = "", $device_type = "", $pushmessage = null, $android_api = "",$android_title="")
    {
        ...
        if ($device_type == 1) {
            ...
            if (!empty($deviceToken)) {
                ...
                ////////////////////////////////////////////////////////////////////////////////
                //$root       = $_SERVER['DOCUMENT_ROOT'] . '/application/classes/controller/ck.pem';
                $root       = $_SERVER['DOCUMENT_ROOT'] . '/pushcert.pem';
                $ctx        = stream_context_create();
                stream_context_set_option($ctx, 'ssl', 'local_cert', $root);
                stream_context_set_option($ctx, 'ssl', 'passphrase', $passphrase);
                // Open a connection to the APNS server
    ...
    public function get_passenger_phone_by_id($id)
    {
		...
		$res = $this->mongo_db->find_one(MDB_PASSENGERS,array('_id'=>(int)$id),array('phone'));
		return isset($res['phone']) ? $res['phone'] : '';
    }
     ...
     public function check_new_request_tripid($taxi_id = null, $company_id = null, $trip_id, $driver_id, $company_all_currenttimestamp, $driver_reply, $operator_id = 0)
    {
        ...
        if (count($result) > 0) {
            if ($driver_reply != 'C') {
                ...
                $update_result     = $this->mongo_db->update(MDB_REQUEST_HISTORY,array('_id'=>(int)$trip_id),array('$set'=>$update_trip_array),array('upsert'=>false));
                ...
                if ($temp_driver == '') {
                    ...
                    $update_result         = $this->mongo_db->update(MDB_REQUEST_HISTORY, array('_id'=>(int)$trip_id),array('$set'=>$update_trip_array_one),array('upsert'=>false));
                ...
                if (count($comp_result) == 0) {
                    ...
                    $update_result         = $this->mongo_db->update(MDB_REQUEST_HISTORY, array('_id'=>(int)$trip_id),array('$set'=>$update_trip_array_one),array('$upsert'=>true));
                    if ($operator_id != 0 && $user_type_dets['user_type'] == 'A') {
                        $update_log_array_driver = array(
                            "driver_id" => 0,
                            "taxi_id" => 0,
   ...
   public function get_trip_detail_only($passengerlog_id = "")
	...
	public function get_child_trip_count($passId) {
		$common_arguments = array(
			...
			array(
				'$match' =>array('parent_id'=>(int)$passId, 'plog.travel_status' => (int)2),
			),
			array(
					'$group' => array(
						'_id' => NULL,
