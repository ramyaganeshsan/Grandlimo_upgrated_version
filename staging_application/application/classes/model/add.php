...
Class Model_Add extends Model
{
    ...
    public function validate_addcompany($arr, $files_value_array)
    {
        ...
        ->rule('email', 'not_empty')->rule('email', 'email')->rule('email', 'max_length', [
            ':value',
            '50'
        ])->rule('email', 'Model_Add::checkemail', [
            ':value'
        ...
        ->rule('phone', 'contact_phone', [
            ':value'
        ])->rule('phone', 'Model_Add::checkphone', [
            ':value'
            ...
            ':value',
            '30'
        ])->rule('company_name', 'Model_Add::checkcompany', [
            ':value',
            $arr['country'],
            $arr['state'],
            $arr['city']
		...
		->rule('taxi_image', 'Upload::not_empty', [
            $files_value_array['taxi_image']
        ])->rule('taxi_image', 'Upload::valid', [
            $files_value_array['taxi_image']
    ...
    public function validate_addmoderator($arr)
    {
        return Validation::factory($arr)->rule('name', 'not_empty')->rule('name', 'min_length', [
            ...
            ':value'
        ])->rule('sales_person_email', 'not_empty')->rule('sales_person_email', 'email')->rule('sales_person_email', 'max_length', [
            ':value',
            '50'
        ])->rule('phone', 'not_empty')
        //->rule('phone', 'numeric')
    ...
    public function validate_addtaxi($arr, $files_value_array = "")
    {
        ...
        //->rule('taxi_no', 'alpha_numeric', array(':value','/^[0-9]{1,}/'))
            ...
            ':value',
            '/^[a-z0-9A-Z -]++$/iD'
        ])->rule('taxi_no', 'Model_Add::check_taxino', [
            ':value'
			...
			->rule('taxi_insurance_number', 'Model_Add::check_taxinsurance_number', [
            ':value'
        ])->rule('taxi_insurance_expire_date', 'not_empty')
			->rule('taxi_pco_licence_number', 'not_empty')
			->rule('taxi_pco_licence_number', 'Model_Add::check_taxipco_number', [
            ':value'
        ...
        ->rule('taxi_capacity', 'digit', array(':value','/^[0-9]{1,}/'))*/
			...
			->rule('taxi_fare_km', 'min_length', [
            ':value',
            '1'
        ])->rule('taxi_fare_km', 'max_length', [
            ':value',
            '20'
        ])->rule('taxi_fare_km', 'digit', [
            ':value',
            '/^[0-9]{1,}/'
        ]);
 ...
 return $rule;
    ...
    public function validate_addfield($arr)
    {
        $rule = Validation::factory($arr)->rule('field_labelname', 'not_empty')->rule('field_labelname', 'min_length', [
            ...
            ':value'
        ])->rule('field_name', 'Model_Add::checkfieldname', [
            ':value'
    ...
    public function validate_addmotor($arr)
    {
        ...
        //->rule('country_name', 'alpha_dash')
            ...
            ':value',
            '30'
        ])->rule('companyname', 'Model_Add::checkmotorname', [
            ':value'
        ]);
    }
    /**Validating for Add Motor**/
    public function validate_addmodel($arr)
    {
        ...
        //->rule('model_name', 'alpha_dash')
            ...
            ':value',
            '30'
        ])->rule('model_name', 'Model_Add::checkmodelname', [
            ':value',
            $arr['companyname']
        ])->rule('model_size', 'not_empty')->rule('model_size', 'Model_Edit::check_fare_zero', [
            ':value',
            $arr['model_size']
        ])->rule('companyname', 'not_empty')->rule('waiting_time', 'not_empty')->rule('waiting_time', 'Model_Add::check_waiting_time', [
            ':value',
            $arr['waiting_time']
        ])->rule('base_fare', 'not_empty')->rule('base_fare', 'Model_Add::check_base_fare', [
            ':value',
            $arr['base_fare']
        ])->rule('min_km', 'not_empty')->rule('min_km', 'Model_Add::check_min_km', [
            ':value',
            $arr['min_km']
        ])->rule('min_fare', 'not_empty')->rule('min_fare', 'Model_Add::check_min_fare', [
            ':value',
            $arr['min_fare']
        ])->rule('cancellation_fare', 'not_empty')->rule('cancellation_fare', 'Model_Add::check_cancellation_fare', [
            ':value',
            ...
            ':value',
            $arr['min_km']
        ])->rule('below_km', 'not_empty')->rule('below_km', 'Model_Add::check_below_km', [
            ':value',
            $arr['below_km']
        ])->rule('above_km', 'not_empty')->rule('above_km', 'Model_Add::check_above_km', [
            ':value',
            $arr['above_km']
        ])->rule('night_charge', 'not_empty');
        if (Arr::get($arr, 'night_charge') == 1) {
            //echo "dsf";exit;
            $validation->rule('night_timing_from', 'not_empty')->rule('night_timing_to', 'not_empty')->rule('night_fare', 'not_empty')->rule('night_fare', 'Model_Add::check_night_fare', [
                ':value',
                $arr['night_fare']
            ]);
        }
    ...
    {
        $validation = Validation::factory($arr)->rule('model_name', 'not_empty')->rule('model_size', 'not_empty')->rule('model_size', 'Model_Edit::check_fare_zero', [
            ':value',
            $arr['model_size']
        ])->rule('waiting_time', 'not_empty')->rule('waiting_time', 'Model_Add::check_waiting_time', [
            ':value',
            $arr['waiting_time']
        ])->rule('base_fare', 'not_empty')->rule('base_fare', 'Model_Add::check_base_fare', [
            ':value',
            ...
            ':value',
            $arr['min_km']
        ])->rule('below_and_above_km', 'Model_Add::check_value_zero', [
            ':value',
            ...
            ':value',
            $arr['above_km']
        ])->rule('minutes_fare', 'not_empty')->rule('minutes_fare', 'Model_Add::check_minute_fare', [
            ':value',
            $arr['minutes_fare']
    ...
    public function validate_adddriver($arr)
    {
        ...
        ->rule('email', 'not_empty')->rule('email', 'email')->rule('email', 'max_length', [
			...
			->rule('driver_license_id', 'max_length', [
            ':value',
            '30'
        ])->rule('driver_license_id', 'Model_Add::checklicenceId', [
            ':value'
        ])->rule('driver_license_expire_date', 'not_empty')
			->rule('driver_pco_license_number', 'not_empty')
			->rule('driver_pco_license_number', 'max_length', [
            ':value',
            '30'
        ])->rule('driver_pco_license_number', 'Model_Add::checkpcolicenceNo', [
            ':value'
        ])->rule('driver_pco_license_expire_date', 'not_empty')
			->rule('driver_insurance_number', 'not_empty')
			->rule('driver_insurance_number', 'max_length', [
            ':value',
            '30'
        ])->rule('driver_insurance_number', 'Model_Add::checkinsuranceNo', [
            ':value'
        ])->rule('driver_insurance_expire_date', 'not_empty')
		->rule('driver_national_insurance_number', 'not_empty')
		->rule('driver_national_insurance_number', 'max_length', [
            ':value',
            '30'
        ])->rule('driver_national_insurance_number', 'Model_Add::checkNationalinsuranceNo', [
            ':value'
		...
		->rule('booking_limit', 'Model_Add::check_booking_limit', [
            ':value',
            $arr['booking_limit']
        ])
        // Client Request
    ...
    public function addcompany($post, $path)
    {
        ...
        if (isset($image_data['mime'])) {
			...
			//Get the last object id
			$company_rs = $this->mongo_db->find(MDB_COMPANY,[],['_id'])->sort(['_id'=>-1])->limit(1);
			...
			$company_insert = ['_id' => $reg_companyid,
			...
			//update company id with People Collection
			 $update_people = $this->mongo_db->update(MDB_PEOPLE,['_id'=>$company_userid],['$set'=>(['company_id' => $reg_companyid])],['upsert'=>true]);
            ...
            if ($upgrade_package == 'D') {
				...
				//Get the last object id
				$package_rs = $this->mongo_db->find(MDB_PACKAGE_REPORT,[],['_id'])->sort(['_id'=>-1])->limit(1);
				$package_res = iterator_to_array($package_rs);
				reset($package_res);
				$package_first_key = key($package_res);
				$package_id = $package_first_key+1;
				//Package Report Data
				$package_report_data = ['_id'=> $package_id,
    ...
    public function addtaxi($post, $image, $files,$encodeString,$filename)
    {
		...
		//MongoDB
		$rs = $this->mongo_db->find(MDB_TAXI,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$res = iterator_to_array($rs);
		reset($res);
		$first_key = key($res);
		$inc_id = $taxi_id = $first_key+1;
		$taxi_data = ['_id' => (int)$inc_id,
    ...
    public function check_fare_exist($comny_id, $modelid)
    {
		...
		//MongoDB
		$result = $this->mongo_db->count(MDB_COMPANY,['_id'=>(int)$comny_id,'model_fare.model_id'=>(int)$modelid],['_id','model_fare.model_id']);
		//echo '<pre>';print_r($result);exit;
		return $result;
    }
    ...
    public function addfare($post)
    {
		...
		*/
		...
		$rs = $this->mongo_db->find(MDB_MOTOR_MODEL,['model_status'=>['$eq'=>'A'], "_id"=>(int)$model_id], ['model_name']);
		...
		if (!empty($result)) {
			...
			$model_fare_array = ["model_fare" =>$query ];
			$result = $this->mongo_db->update(MDB_COMPANY,['_id'=>(int)$company_id],['$push'=>$model_fare_array],['upsert'=>false]);
			return (empty($result['err']))?1:0;
		} else {
            return 0;
        }
    ...
    public function exist_models($model_id)
    {
		...
		$result = $this->mongo_db->find(MDB_COMPANY,["_id"=>(int)$id, "model_fare.model_id"=>(int)$model_id ],["model_fare"]);
    ...
    public function add_driver($post, $filename,$web_image_name='')
    {
		...
		//Get the last object id
		$rs = $this->mongo_db->find(MDB_PEOPLE,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$res = iterator_to_array($rs);
		reset($res);
		$first_key = key($res);
		$inc_id = $first_key+1;
		...
		//print_r($result);exit;
        $driver_id      = $inc_id;
		$query = $this->mongo_db->find(MDB_DRIVER_INFO,['_id'=> (int)$driver_id],['_id']);
        ...
        if ($result) {
			...
			$result = $this->mongo_db->update(MDB_DRIVER_INFO,['_id'=>(int)$driver_id],['$set'=>$driver_insert_data],['upsert'=>false]);
            return $driver_id;
        } else {
            return 0;
        }
    ...
    public static function check_taxino($name)
    {
		...
		$result = $mongodb->count(MDB_TAXI,['taxi_no'=>$name],['taxi_no']);
    ...
    public static function checkcompany($companyname, $country, $state, $city)
    {
       /* $result = DB::select('company_name')->from(COMPANY)->where('company_name', '=', $companyname)->where('company_country', '=', $country)->where('company_state', '=', $state)->where('company_city', '=', $city)->execute()->as_array();
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_COMPANY,['company_name'=>$companyname,'company_country'=>(int)$country,'company_state'=>(int)$state,'company_city'=>(int)$city]);
		return ($result>0)?false:true;
    }
    ...
    public static function checkdomain($domainname)
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_COMPANY,['company_domain'=>$domainname]);
		return ($result>0)?false:true;
    }
    // To Check Company Domain is Already Available or Not
    ...
    public static function checkemail($email = "")
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PEOPLE,['email'=>$email]);
		//print_r($result);exit;
		return ($result > 0)?false:true;
    }
    // Check Whether Email is Already Exist or Not
    public static function checkphone($phone = "")
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PEOPLE,['phone'=>$phone]);
		//print_r($result);exit;
		return ($result > 0)?false:true;
    }
    public static function checkphone_autocreate($phone = "")
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PEOPLE,['phone'=>(int)$phone]);
		//print_r($result);exit;
		return ($result > 0)?false:true;
    }
    ...
    {	
		$result = $this->mongo_db->find(MDB_MOTOR_MODEL,["model_status"=>['$eq'=> "A"]],["_id","model_name"])->sort(['model_name'=>1]);
		$res = iterator_to_array($result);
        return $res;
    }
    ...
    public function validate_addpackage($arr)
    {
        return Validation::factory($arr)->rule('package_name', 'not_empty')->rule('package_name', 'min_length', [
            ...
            ':value',
            '100'
        ])->rule('package_name', 'Model_Add::checkpackagename', [
            ':value',
            $arr['package_name']
    ...
    public function add_package($post)
    {
       ...
       /* $result = DB::insert(PACKAGE, array(
		...
		//MongoDB
		//Get the last object id
		$rs = $this->mongo_db->find(MDB_PACKAGE,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$res = iterator_to_array($rs);
		reset($res);
		$first_key = key($res);
		$inc_id = $first_key+1;
    ...
    public function validate_addcountry($arr)
    {
        ...
        //->rule('country_name', 'alpha_dash')
            ->rule('country_name', 'Model_Add::check_reg_countryname', [
            ':value'
            ...
            ':value',
            '30'
        ])->rule('country_name', 'Model_Add::checkcountryname', [
            ':value'
            ...
            ':value',
            '5'
        ])->rule('iso_country_code', 'Model_Add::checkisocountrycode', [
            ':value'
    ...
    public static function checkcountryname($name)
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$res = $mongodb->count(MDB_CSC,[ "country_name" => new \MongoDB\BSON\Regex('^' . $name, 'i')],['country_name']);
    ...
    public static function checkisocountrycode($iso_country_code)
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$res = $mongodb->count(MDB_CSC,['iso_country_code'=>new \MongoDB\BSON\Regex('^' . $iso_country_code, 'i')],['iso_country_code']);
		return ($res > 0)?false:true;
    }
    ...
    public function addcountry($post)
    {
		...
		//MongoDB
		//Get the last object id
		$rs = $this->mongo_db->find(MDB_CSC,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$res = iterator_to_array($rs);
		reset($res);
		$first_key = key($res);
		$inc_id = $first_key+1;
    ...
    public function country_details_new()
    {
        ...
        return $result;*/
		//MongoDB
		$result = $this->mongo_db->find(MDB_CSC,['country_status'=>'A','stateinfo.state_status'=>'A'],['_id','country_name','default'])->sort(['country_name'=>1]);
    ...
    public function get_city_state_details($countryid)
    {
		...
		$ops = [
			['$unwind' => '$stateinfo'],
			['$match' => ['stateinfo.state_status'=>'A','stateinfo.state_countryid'=>(int)$state_countryid,'stateinfo.default' => 1]],
			['$project' => ['_id' => 0,
				'state_id' => '$stateinfo.state_id', 
				'state_name' => '$stateinfo.state_name',
	...
	public static function getstate_details($country_id)
    {
		...
		$ops = [
			['$unwind' => '$stateinfo'],
			['$match' => ['stateinfo.state_status'=>'A','stateinfo.state_countryid'=>(int)$country_id]],
			['$project' => ['_id' => 0,
				'state_id' => '$stateinfo.state_id', 
				'state_name' => '$stateinfo.state_name',
				'state_default' => '$stateinfo.default',
	...
	public static function getcity_details($country_id, $state_id)
    {
		...
		$ops = [
			['$unwind' => '$stateinfo'],
			['$unwind' => '$stateinfo.cityinfo'],
			['$match' => ['stateinfo.cityinfo.city_status'=>'A','stateinfo.state_id'=>(int)$state_id,'_id'=>(int)$country_id]],
			['$project' => ['_id' => 0,
				'city_id' => '$stateinfo.cityinfo.city_id', 
				'city_name' => '$stateinfo.cityinfo.city_name',
				]
    ...
    public function validate_addcity($arr)
    {
        ...
        //->rule('city_name', 'alpha_dash')
            ->rule('city_name', 'Model_Add::check_reg_city_name', [
            ':value'
            ...
            ':value',
            '30'
        ])->rule('city_name', 'Model_Add::checkcityname', [
            ':value',
    ...
    public function validate_addstate($arr)
    {
        ...
        //->rule('state_name', 'alpha_dash')
            ->rule('state_name', 'Model_Add::check_reg_state_name', [
            ':value'
            ...
            ':value',
            '30'
        ])->rule('state_name', 'Model_Add::checkstatename', [
            ':value',
            $arr['country_name']
    ...
    public static function checkcityname($name, $stateid, $id)
    {
		...
		$mongodb = MangoDB::instance('default');
		$res = $mongodb->find(MDB_CSC,[ "\$and" => [[ "stateinfo.cityinfo.city_name" => new \MongoDB\BSON\Regex('^' . $name, 'i')] , ['_id'=>['$eq'=>$country_id]],['stateinfo.state_id'=>['$eq'=>$state_id]]]],['stateinfo.cityinfo.city_name'])->sort(['stateinfo.cityinfo.city_id'=>-1]);
    ...
    public static function checkstatename($name, $id)
    {
		...
		$cid = (int)$id;
		$mongodb = MangoDB::instance('default');
		$res = $mongodb->find(MDB_CSC,[ "\$and" => [[ "stateinfo.state_name" => new \MongoDB\BSON\Regex('^' . $name, 'i')] , ['_id'=>['$eq'=>$cid]]]],['stateinfo.state_name'])->sort(['stateinfo.state_id'=>-1]);
		$result = (!empty($res))?iterator_to_array($res):[];
		//echo '<pre>';print_r($result);
		return (count($result) > 0)?false:true;
    }
    public function addcity($post)
    {
		...
		//Get the last object id
		$rs = $this->mongo_db->find(MDB_CSC,['_id'=>$country_id,'stateinfo.state_id'=>$state_id],['stateinfo.cityinfo.$'=>1])->sort(['stateinfo.cityinfo.city_id'=>-1])->limit(1);
		...
		//echo $index_key;
		$city_array = [$index_key => $query ];
		$mresult = $this->mongo_db->update(MDB_CSC,['_id'=>$country_id,'stateinfo.state_id'=>$state_id],['$push'=>$city_array],['upsert'=>true]);
    ...
    public function addstate($post)
    {
		...
		$country_id = (int)$post['country_name'];
		//Get the last object id
		$rs = $this->mongo_db->find(MDB_CSC,['_id'=>$country_id],['stateinfo.state_id'])->sort(['stateinfo.state_id'=>-1])->limit(1);		
		$result = (!empty($rs))?iterator_to_array($rs):[];
		//echo '<pre>';print_r($result);
		if (!empty($result)){
		...
		$state_array = ["stateinfo" =>$query ];
		$mresult = $this->mongo_db->update(MDB_CSC,['_id'=>$country_id],['$push'=>$state_array],['upsert'=>true]);
		//echo '<pre>';print_r($mresult);exit;
    ...
    public function getcompany_details($country_id, $state_id, $city_id)
    {
		...
		$ops = [
			['$unwind' => '$companydetails'],
			['$match' => ['companydetails.company_status' => 'A','companydetails.company_country' => (int)$country_id, 'companydetails.company_state' => (int)$state_id, 'companydetails.company_city' => (int)$city_id]],
			['$project' => ['cid' => '$_id','company_name' => '$companydetails.company_name']
			],
			[
				'$sort' => [
   ...
   /* public function taxi_details()
    ...
    {
        return Validation::factory($arr)->rule('company_name', 'not_empty')->rule('country', 'not_empty')->rule('state', 'not_empty')->rule('city', 'not_empty')->rule('driver', 'not_empty')->rule('startdate', 'not_empty')->rule('enddate', 'not_empty')->rule('enddate', 'Model_Add::checkassigntaxi', [
            ':value',
            $arr
        ])->rule('taxi', 'not_empty');
    }
  ...
  /*  public function addassigntaxi($post)
    ...
    public function package_details()
    {
        ...
        return $result;*/
		//MongoDB
		$result = $this->mongo_db->find(MDB_PACKAGE,['package_status'=>'A','package_type'=>'N'])->sort(['package_name'=>1]);
		return (!empty($result))?iterator_to_array($result):[];
    }
    public function payment_packagedetails($packid = 0)
    {
		...
		//MongoDB
		$result = $this->mongo_db->find_one(MDB_PACKAGE,['_id'=>(int)$packid]);
		//echo '<pre>';print_r($result);exit;
    ...
    {
		$current_time = convert_timezone('now', TIMEZONE);
		$arguments = [['$match'=> ['upgrade_companyid' => (int)$cid, 'upgrade_expirydate' =>['$gte' => $current_time ]]],
			['$project' => ['no_driver'=> '$upgrade_no_driver','check_package_type' => '$check_package_type']],
			['$sort' => ['upgrade_id' => -1]],
			['$limit' => 1],
		];
		$result = $this->mongo_db->aggregate(MDB_PACKAGE_REPORT,$arguments);
		$driver_count = $this->mongo_db->count(MDB_PEOPLE,['company_id' => (int)$cid, 'user_type' => 'D', 'availability_status' => 'A']);
		if (isset($result['result']) && count($result['result']) > 0) {
			if ($result['result'][0]['check_package_type'] != 'T') {
				$driver_add = 1;
			} else {
    ...
    {
        return Validation::factory($arr)->rule('meta_title', 'not_empty')->rule('meta_keyword', 'not_empty')->rule('meta_description', 'not_empty')->rule('menu_name', 'not_empty')->rule('menu_name', 'Model_Add::menu_content_exits', [':value']);
    }
    ...
    public static function menu_content_exits($menu_name)
    {
        ...
        $mongodb = MangoDB::instance('default');
		$res = $mongodb->find(MDB_CMS,['_id'=>(int)$menu_name],['menu'=>1]);
    ...
    public function check_companyid($cid)
    {
		...
		$res = $this->mongo_db->count(MDB_COMPANY,['_id'=>(int)$cid,'companydetails.company_status'=>'A'],['_id']);
		return $res;
    }
    ...
    public function change_imagecount($taxi_id = '', $image_id = '')
    {
		...
		//MongoDB
		$result = $this->mongo_db->find_one(MDB_TAXI,['_id'=>(int)$taxi_id],['taxi_sliderimage']);
		$count_image = (!empty($result))?$result['taxi_sliderimage']:0;
		if($count_image > 0) {
			$count_image = $count_image-1 ;
            $res = $this->mongo_db->update(MDB_TAXI,['_id'=>($taxi_id)],['$set'=>['taxi_sliderimage'=>(int)$count_image]],['upsert'=>true]);
        }
		return $count_image;
    }
    ...
    public function validate_addmenu($arr)
    {
        ...
        return Validation::factory($arr)->rule('menu_name', 'not_empty')->rule('menu_name', 'min_length', [
            ...
            ':value',
            '30'
        ])->rule('slug', 'not_empty')->rule('menu_name','Model_Add::menu_name_exits',[':value']);
    }
    ...
    public function addmenu($post)
    {
        ...
        $cms_rs = $this->mongo_db->find(MDB_CMS,[],['_id'])->sort(['_id'=>-1])->limit(1);
    ...
    public static function menu_name_exits($menu_name="")
    {
        ...
        $mongodb = MangoDB::instance('default');
        $result = $mongodb->count(MDB_CMS,["menu_name"=>$menu_name]);
    ...
    {
        return Validation::factory($arr)->rule('tags', 'not_empty')->rule('image_tag', 'not_empty')->rule('file', 'Upload::not_empty', [
    ...
    {
        $validation = Validation::factory($arr)->rule('faq_title', 'not_empty')->rule('faq_title', 'Model_Add::checkfaqtitle', [
            ':value'
    ...
    {
        $result = DB::insert(PASSENGERS_FAQ, [
            'faq_title',
            'faq_details',
            'status'
        ])->values([
    ...
    {
        return Validation::factory($arr)->rule('firstname', 'not_empty')->rule('firstname', 'Model_Add::checkname', [
            ':value'
        ])->rule('firstname', 'min_length', [
            ':value',
            '4'
            ...
            ':value',
            '30'
        ])->rule('lastname', 'not_empty')->rule('no_of_login', 'not_empty')->rule('phone', 'numeric')->rule('phone', 'not_empty')->rule('phone', 'contact_phone', [
            ':value'
        ])->rule('phone', 'Model_Add::checkphone_autocreate', [
            ':value'
        ]);
    }
    ...
    {
        $mongodb = MangoDB::instance('default');
        $result = $mongodb->find(PEOPLE,['name'=>$firstname."1"],[]);
    ...
    public function create_login($post)
    {
        ...
        for ($i = 1; $i <= $no_of_login; $i++) {
            ...
            $result            = DB::insert(PASSENGERS_CARD_DETAILS, [
                'passenger_id',
                'passenger_email',
                'card_type',
                'creditcard_no',
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PEOPLE,['driver_license_id'=>$value],['driver_license_id']);
		return ($result>0)?false:true;		
    }
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_DRIVER_INFO,['driverinfo.driver_pco_license_number'=>$value],['_id']);
		return ($result>0)?false:true;	
    }
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_DRIVER_INFO,['driverinfo.driver_insurance_number'=>$value],['_id']);
		return ($result>0)?false:true;		
    }
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_DRIVER_INFO,['driverinfo.driver_national_insurance_number'=>$value],['_id']);
		return ($result>0)?false:true;	
    }
    ...
    public static function check_taxinsurance_number($number)
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_TAXI,['taxi_insurance_number'=>$number],['taxi_insurance_number']);
		return ($result>0)?false:true;
    }
    ...
    public static function check_taxipco_number($number)
    {
		...
		//MongoDB
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_TAXI,['taxi_pco_licence_number'=>$number],['taxi_pco_licence_number']);
		return ($result>0)?false:true;
    }
    ...
    {        
        return Validation::factory($arr)->rule('account_type', 'not_empty')->rule('account_type', 'min_length', [
            ...
            ':value',
            '30'
        ])->rule('account_type','Model_Add::account_type_exits',[':value'])->rule('account_type_arabic', 'not_empty');
    }
    ...
    public function addaccounttype($post)
    {
        ...
        */
        $cat_rs = $this->mongo_db->find(MDB_CHILD_ACCOUNT_TYPE,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$cat_rs = iterator_to_array($cat_rs);
		reset($cat_rs);
		$cat_first_key = key($cat_rs);
		$cat_id = $cat_first_key + 1;
		$cat_account_type = ['_id'=>$cat_id,
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_CHILD_ACCOUNT_TYPE,['$text' => ['$search' => "\"$account_type\""]]);
		return ($result ==1)?false:true;
    }
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_CHILD_ACCOUNT_TYPE,['_id' => (int)$id]);
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PASSENGERS,['_id' => (int)$id]);		
		return ($result ==1)?true:false;
    }
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PASSENGERS,['civilid' => $id]);//exit;
		return ($result >= 1)?false:true;
    }
    ...
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PASSENGERS,['phone' => $phone]);	
		return ($result ==1)?true:false;
    }
...
public static function driver_id_isValid($id="")
    {
		$mongodb = MangoDB::instance('default');
		$result = $mongodb->count(MDB_PEOPLE,['_id' => (int)$id,'user_type' =>'D']);		
		return ($result ==1)?true:false;
    }
     ...
     public function validate_addcoupon_package($arr)
    {
        return Validation::factory($arr)->rule('coupon_package_name', 'not_empty')->rule('coupon_package_name', 'min_length', [
            ...
            ':value',
            '100'
        ])->rule('coupon_package_name', 'Model_Add::checkcoupon_packagename', [
            ':value',
        ...
        ->rule('coupon_package_name_ar', 'min_length', [':value', '4'])
        ->rule('coupon_package_name_ar', 'max_length', [':value', '100'])
        ->rule('coupon_package_description_ar', 'not_empty')
        ->rule('coupon_package_description_ar', 'min_length', [':value', '20']);
    ...
    public function add_coupon_package($post)
    {
		...
		$this->mongo_db = MangoDB::instance('default');
		//Get the last object id
		$rs = $this->mongo_db->find(MDB_COUPON_PACKAGE,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$res = iterator_to_array($rs);
		reset($res);
		$first_key = key($res);
		$inc_id = $first_key+1;
      ...
      public function validate_addcorporate($arr)
    {
        ...
        //->rule('username', 'alpha_dash')
            ...
            ':value',
            '30'
        ])->rule('lastname', 'not_empty')->rule('phone', 'Model_Add::checkphone', [
            ':value'
        ])->rule('email', 'not_empty')->rule('email', 'email')->rule('email', 'max_length', [
            ':value',
            '50'
		...
		->rule('photo', 'Upload::not_empty', [
			...
			->rule('phone', 'contact_phone', [
            ':value'
        ])->rule('phone', 'not_empty') ->rule('phone', 'min_length', [
            ':value',
            '7'
        ])->rule('phone', 'max_length', [
            ':value',
    ...
    public function update_corporateimage($image,$userid)
{
      ...
      public static function check_ratingsno($ratings_no = "",$edit_id="")
    {
        ...
        if($flag == 1){
            $mongodb = MangoDB::instance('default');
            $result = $mongodb->count('ratings',['ratings_no'=>(int)$ratings_no,'status'=>'A'] );
            //print_r($result);exit;
            return ($result > 0)?false:true;    
        }
    ...
    /**Validating for Add ratings**/
      ...
      public function add_ratings($postval){
        $rs = $this->mongo_db->find('ratings',[],['_id'])->sort(['_id'=>-1])->limit(1);
        $res = iterator_to_array($rs);
        reset($res);
        $first_key = key($res);
        $inc_id = $first_key+1;
    ...
    public function add_testimonial($post,$image){
            ...
            $rs = $this->mongo_db->find(MDB_TESTIMONIAL,[],['_id'])->sort(['_id'=>-1])->limit(1);
    ...
    public function validate_add_zone($arr)
    {
       ...
       $validate = Validation::factory($arr)
            ...
            ->rule('zone_name', 'min_length', [':value', '2'])
            ->rule('zone_name', 'max_length', [':value', '30'])
    ...
    public function add_zone($post,$sourceloc){
        //echo"<pre>"; print_r($post);exit();
        $user_createdby = $this->user_createdby;
        $rs = $this->mongo_db->find(MDB_ZONES,[],['_id'])->sort(['_id'=>-1])->limit(1);
		$rs = iterator_to_array($rs);
		reset($rs);
		$rs_first_key = key($rs);
        ...
        $source_polygon['coordinates'] =[$sourceloc];
        
        $source_location = isset($post['source_location']) ? $post['source_location'] : '';
        $source_latlng[] = isset($post['source_lng']) && isset($post['source_lat']) ? ['type'=>'Point','coordinates'=>[(double)$post['source_lng'],(double)$post['source_lat']]] : '';
    ...
    {
        $rs = $this->mongo_db->find(MDB_CORPORATE_GROUP_PROMOCODE,[],['_id'])->sort(['_id'=>-1])->limit(1);
        $res = iterator_to_array($rs);
        reset($res);
        $first_key = key($res);
        $inc_id = $first_key+1;
    ...
    public function getPassengerCouponDetails($passengerPhone) {
        ...
        return (!empty($passenger))?iterator_to_array($passenger, false):[];
    }
    ...
    public function getCouponDetails($code) {
        ...
        return (!empty($coupon))?iterator_to_array($coupon, false):[];
    }
    ...
    public function addPromotionalPushNotification($pushNotificationDetails) {
        ...
        $rs = $this->mongo_db->find("promotional_push_notifications",[],['_id'])
        ->sort(['_id'=>-1])
        ->limit(1);
        $res = iterator_to_array($rs);
        reset($res);
