...
facebook_secretkey,facebook_share,twitter_share,site_logo FROM " . SITEINFO . " limit 0,1 ";*/
    ...
    {
        $driver_reply = '';
        $data = $this->mongo_db->find_one(MDB_PASSENGERS_LOGS,['_id'=>(int)$id],['driver_reply','taxi_edit_modelid']);
            ...
            $update_query = [
             ...
             if($taxi_model !=''){
                if(isset($data['taxi_edit_modelid']) && $data['taxi_edit_modelid']!=''){
                    $taxi_query = ['taxi_edit_modelid'=>(int)$taxi_model];
                }else{
...
//print_r($result['result']);exit;
    ...
    public function get_passenger_notifications(){
        ...
        $arguments = [ ['$match'=>['sent_status'=>['$ne' =>0]]] ,['$project'=> ['title'=>['$ifNull'=>['$title','']],'sub_title'=>['$ifNull'=>['$sub_title','']],'message'=>'$message','image'=>'$image','created_date'=>'$created_date' ] ],['$sort'=>['_id' => -1]], ];
