...
Class Model_Edit extends Model
{
   ...
   /* public static function checkassigntaxi($enddate, $post, $uid)
    ...
    public function get_promocodedetails($id)
    {
        ...
        /* Sasidharan july 16 2022 added apply_user_limit, maximum_allowed_limit */
        /* Sasidharan Nov 15 2022 added corporate_group */
        /* Sasidharan June 12 2024 added skipCard */
		$query = $this->mongo_db->find_one(MDB_PASSENGERS_PROMO,['_id' => (int)$id],['_id', 'promocode', 'promo_discount', 'start_date', 'expire_date', 'promo_limit','corporate_promocode','total_airport_trips','register_promocode','apply_user_limit','maximum_allowed_limit', 'corporate_group', "skipCard"]);
    ...
    public function editpromocode($post, $promocode)
    {
		...
		$data     = [
            ...
            */
            'corporate_promocode'=> (isset($post['corporate_promocode']) && !empty($post['corporate_promocode'])) ? (int)$post['corporate_promocode'] : 0,
            'total_airport_trips'=> (isset($post['total_airport_trips']) && !empty($post['total_airport_trips'])) ? (int)$post['total_airport_trips'] : 0,

            /* Sasidharan june 20 - 2022 */
            'register_promocode'=> (isset($post['register_promocode']) && !empty($post['register_promocode'])) ? (int)$post['register_promocode'] : 0,

            /* Sasidharan June 12 2024 */
            'skipCard'=> (isset($post['skipCard']) && !empty($post['skipCard'])) ? (int)$post['skipCard'] : 0,

            /* Sasidharan june 16 2022 */
            'maximum_allowed_limit'=> (isset($post['maximum_allowed_limit']) && !empty($post['maximum_allowed_limit'])) ? (int)$post['maximum_allowed_limit'] : 0,
            'apply_user_limit'=> (isset($post['apply_user_limit']) && !empty($post['apply_user_limit'])) ? (int)$post['apply_user_limit'] : 0,

            /* Sasidharan nov 15 2022 */
            'corporate_group'=> (isset($post['corporate_group']) && !empty($post['corporate_group'])) ? (int)$post['corporate_group'] : "",
       ...
       public function coupon_package_details($uid)
    {
		...
		//MongoDB
		$res = $this->mongo_db->find_one(MDB_COUPON_PACKAGE,['_id'=>(int)$uid],['_id','coupon_package_name','coupon_package_description','coupon_package_name_ar','coupon_package_description_ar','corporate_commission','passenger_commission','coupon_package_status','fixed_fare_apply']);
		return (!empty($res))?$res:[];
    }
    public function validate_editcoupon_package($arr, $uid)
    {
        ...
        ->rule('coupon_package_description_ar', 'min_length', [':value', '10']);
 ...
 public function update_corporateimage($image,$userid)
{
    ...
    {
        $query = $this->mongo_db->find(MDB_ZONES,['_id' => (int)$id]);
        $result = iterator_to_array($query);
        $finrest = array_shift($result);
        //echo "<pre>";print_r($finrest);exit();
        return $finrest;
    }
    public function validate_editzone($arr,$id)
    {
    ...
    public function edit_zone($postvalues,$id,$sourceloc)
    {
        $user_createdby = $_SESSION['userid'];
        
        $source_polygon['coordinates'] =[$sourceloc];
        $source_polygon['type'] ="Polygon";
        $source_location = isset($postvalues['source_location']) ? $postvalues['source_location'] : '';
        $source_latlng[] = isset($postvalues['source_lng']) && isset($postvalues['source_lat']) ? ['type'=>'Point','coordinates'=>[(double)$postvalues['source_lng'],(double)$postvalues['source_lat']]] : '';

		$param = [
                    'zone_name'=>$postvalues['zone_name'],
                    ...
                    'status' => 'A',          
                    'is_airport' => isset($postvalues['is_airport']) ? (int)1 : (int)0,           
                    'created_by'=>$user_createdby
                ];
        $result = $this->mongo_db->update(MDB_ZONES,['_id'=>(int)$id],['$set'=>$param],['upsert'=>false]);
        return (empty($result['err']))?1:$result['err'];
    }
    
    public function surge_pricing_details($uid)
    {
        //MongoDB
        $arguments = [
            ['$match'  => ['_id' => (int)$uid]], 
            ...
            [
                '$sort' => ['surge_pricing_fare.default'=> 1,'surge_pricing_manual_interval.default'=> 1]
            ],
            ['$group'=>['_id' => '$_id',
                'id' => ['$first' => '$_id' ],
                ...
                'surge_pricing_interval' => ['$addToSet' => '$surge_pricing_interval' ],//26 feb 2021
                'surge_pricing_status' => ['$addToSet' => '$surge_pricing_status' ],//26 feb 2021
    ...
    public function edit_surge_pricing($time , $price , $id){
        $data = [
            'surge_pricing_fare'=>$price,
            'surge_pricing_manual_interval'=>$time
        ];
        $result = $this->mongo_db->update(MDB_MOTOR_MODEL,['_id'=>(int)$id],['$set'=>$data]);
    ...
    //26 feb 2021
    public function update_surge_pricing($id,$arr){
        $array = [];
        foreach($arr['package'] as $key => $value) {
            $array[$key]['from'] = (int)$value['from'];
            $array[$key]['fare'] = !empty($value['fare']) ? (float)$value['fare'] : (float)0;
            $array[$key]['display_fare'] = $value['display_fare'];
        }

        $data = [
            'book_later_time_interval'=>(int)$arr['book_later_time_interval'],//26 feb 2021
            'book_later_fare'=>(float)$arr['book_later_fare'],//26 feb 2021
            'book_later_display_fare'=>$arr['book_later_display_fare'],//26 feb 2021
            'surge_pricing_interval'=>$array,
            'surge_pricing_status'=>$arr['surge_pricing_status']
        ];
        $result = $this->mongo_db->update(MDB_MOTOR_MODEL,['_id'=>(int)$id],['$set'=>$data]);        
        return (empty($result['err']))?1:$result['err'];
    }
    ...
    /* Sasidharan */
    public function get_corporate_promocodedetails($id)
    {
        /*$query  = "SELECT distinct * ,count(passenger_id) as passenger_count  FROM " . PASSENGER_PROMO . " group by promocode  HAVING `passenger_promoid` = $id";
        $result = Db::query(Database::SELECT, $query)->execute()->as_array();
        return $result;*/
        
        $query = $this->mongo_db->find_one(MDB_CORPORATE_GROUP_PROMOCODE,['_id' => (int)$id],['_id', 'promocode', 'promo_discount', 'start_date', 'expire_date', 'promo_limit']);
        //echo "<pre>"; print_r($query); exit;
        return $query;
    }

    public function editcorporatepromocode($post, $promocode)
    {   
        $data     = [
            'start_date' => new \MongoDB\BSON\UTCDateTime(strtotime($post['start_date']) * 1000),
            'expire_date' => new \MongoDB\BSON\UTCDateTime(strtotime( $post['expire_date']) * 1000),
        ...
        $result = $this->mongo_db->update(MDB_CORPORATE_GROUP_PROMOCODE,['promocode'=>$promocode],['$set'=>$data],['upsert'=>false,'multiple'=>true]);
        return (empty($result['err']))?1:$result['err'];
    }
