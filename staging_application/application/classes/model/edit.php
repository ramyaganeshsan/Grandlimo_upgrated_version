...
Class Model_Edit extends Model
{
   ...
   /* public static function checkassigntaxi($enddate, $post, $uid)
    ...
    public function get_promocodedetails($id)
    {
		...
		$query = $this->mongo_db->find_one(MDB_PASSENGERS_PROMO,array('_id' => (int)$id),array('_id', 'promocode', 'promo_discount', 'start_date', 'expire_date', 'promo_limit','corporate_promocode','total_airport_trips','register_promocode','apply_user_limit','maximum_allowed_limit', 'corporate_group', "skipCard"));
       ...
       public function coupon_package_details($uid)
    {
		...
		//MongoDB
		$res = $this->mongo_db->find_one(MDB_COUPON_PACKAGE,array('_id'=>(int)$uid),array('_id','coupon_package_name','coupon_package_description','coupon_package_name_ar','coupon_package_description_ar','corporate_commission','passenger_commission','coupon_package_status','fixed_fare_apply'));
		return (!empty($res))?$res:array();
    }
    public function validate_editcoupon_package($arr, $uid)
    {
        ...
        ->rule('coupon_package_description_ar', 'min_length', array(':value', '10'));
 ...
 public function update_corporateimage($image,$userid)
{
    ...
    {
        $query = $this->mongo_db->find(MDB_ZONES,array('_id' => (int)$id));
        $result = iterator_to_array($query);
        $finrest = array_shift($result);
        //echo "<pre>";print_r($finrest);exit();
        return $finrest;
    ...
    public function edit_zone($postvalues,$id,$sourceloc)
    {
        ...
        $source_latlng[] = isset($postvalues['source_lng']) && isset($postvalues['source_lat']) ? array('type'=>'Point','coordinates'=>array((double)$postvalues['source_lng'],(double)$postvalues['source_lat'])) : '';
        ...
        $result = $this->mongo_db->update(MDB_ZONES,array('_id'=>(int)$id),array('$set'=>$param),array('upsert'=>false));
        return (empty($result['err']))?1:$result['err'];
    }
    ...
    public function surge_pricing_details($uid)
    {
        ...
        $arguments = array(
            ...
            array(
                '$sort' => array('surge_pricing_fare.default'=> 1,'surge_pricing_manual_interval.default'=> 1)
            ),
            array('$group'=>array('_id' => '$_id',
                ...
                'surge_pricing_interval' => array('$addToSet' => '$surge_pricing_interval' ),//26 feb 2021
                'surge_pricing_status' => array('$addToSet' => '$surge_pricing_status' ),//26 feb 2021
    ...
    public function edit_surge_pricing($time , $price , $id){
        ...
        $result = $this->mongo_db->update(MDB_MOTOR_MODEL,array('_id'=>(int)$id),array('$set'=>$data));
    ...
    public function get_corporate_promocodedetails($id)
    {
        ...
        $query = $this->mongo_db->find_one(MDB_CORPORATE_GROUP_PROMOCODE,array('_id' => (int)$id),array('_id', 'promocode', 'promo_discount', 'start_date', 'expire_date', 'promo_limit'));
        //echo "<pre>"; print_r($query); exit;
        return $query;
    }
    ...
    {   
        ...
        $result = $this->mongo_db->update(MDB_CORPORATE_GROUP_PROMOCODE,array('promocode'=>$promocode),array('$set'=>$data),array('upsert'=>false,'multiple'=>true));
        return (empty($result['err']))?1:$result['err'];
    }
