  ...
  /*  public function free_driver_list_count($cid = 0)
	...
	public function promocode_list($offset, $val, $search, $company_id = 0, $corporate_id = 0, $find_count = FALSE)
    {
		...
		if($find_count == TRUE){
			...
			$arguments = array(
					...
					array('$match'=> $match_query),
					array('$unwind' => '$_id'),
					array('$project' => array('promocode' 	=> 1,'promo_discount'=> 1,'passenger_id'	=> 1,'start_date'	=> 1,'expire_date'	=> 1,'promo_limit'	=> 1,'company_name'	=> '$company.companydetails.company_name','package_name'	=> '$package.coupon_package_name')),
					array('$group' =>array('_id' => array('passenger_promoid'=> '$_id','promocode'=> '$promocode','promo_discount'=>'$promo_discount','package' => '$package','passenger_id' => '$passenger_id','start_date'=>'$start_date','expire_date'=>'$expire_date','promo_limit'=>'$promo_limit','company_name'=>'$company_name','package_name' => '$package_name'),'passenger_count' => array('$sum' => 1))),
					array( '$group' =>array('_id' => '$_id.promocode','promodetails' =>array( '$first' =>array('passenger_promoid'=> '$_id.passenger_promoid','package' => '$_id.package','promocode'=> '$_id.promocode','promo_discount'=>'$_id.promo_discount','start_date'=>'$_id.start_date','expire_date'=>'$_id.expire_date','promo_limit'=>'$_id.promo_limit','company_name'=>'$_id.company_name','package_name' => '$_id.package_name')),'count' => array('$sum' => '$passenger_count'))),
					array('$skip' => (int)$offset),
					array('$limit' => (int)$val),
				);
			//	echo REC_PER_PAGE;
	...
	*/
	  ...
	  public function all_coupon_package_list($offset, $val,$condition=FALSE)
    {
	  ...
	  //MongoDB
		if($condition == TRUE){
			...
			//MongoDB
			$res = $this->mongo_db->find(MDB_COUPON_PACKAGE,array('coupon_package_status'=>array('$ne'=>'T')),array('_id','coupon_package_name','coupon_package_description','corporate_commission','passenger_commission','coupon_package_status'))->sort(array('coupon_package_name'=>1))->skip($offset)->limit($val);
			//echo '<pre>'; print_r(iterator_to_array($res));exit;
			return (!empty($res))?iterator_to_array($res):array();	
		}
	...
	public function avail_package(){
		$res = $this->mongo_db->find(MDB_COUPON_PACKAGE,array('coupon_package_status'=>array('$ne'=>'T')),array('_id','coupon_package_name','coupon_package_description','corporate_commission','passenger_commission','coupon_package_status'))->sort(array('coupon_package_name'=>1));
