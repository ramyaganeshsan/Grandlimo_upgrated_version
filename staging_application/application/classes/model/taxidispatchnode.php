        ...
        $arguments = [
            ...
            [
                '$project' => [
                    ...
                    'performance_status'=>['$ifNull'=>['$performance_status',0]],
                ]
            ],
        ];
        //echo "<pre>"; print_r($arguments);
...
$mat['updatetime_difference'] = [ '$lte' => (int)LOCATIONUPDATESECONDS];
    ...
    public function cancelbooking_logid($data)
    {
        ...
        if (!empty($selectresult) && count($selectresult)>0) {
            if ($selectresult['travel_status'] != '5' || $selectresult['travel_status'] != '2') {
                $updateresult = $this->mongo_db->update(MDB_PASSENGERS_LOGS,['_id'=>(int)$log_id],['$set'=>['travel_status'=>8,'cancel_reason'=>$cancel_reason,'performance_status'=>(int)5,'dispatcher_id'=>(int)$this->session->get('userid')]],['upsert'=>true]);
 ...
 public function search_upcoming_trips()
  ...
  $arguments = [[
    ...
    $update_array = ["fare_detail" => [ ["key"=>"1","value" => (double)$post["cash_pay"]], ["key"=>"6","value" => (double)$post["card_pay"]], ["key"=>"3","value" => (double)$post["knet_pay"]], ["key"=>"additional amount","value" => (double)$post["add_amt"]],["key"=>"wallet","value"=>$post["wallet_pay"]],["key"=>"pending","value"=>$post["pending_pay"]],["key"=>"fare_note","value"=>$post["fare_note"]],["key" => "discount_wallet","value"=>$discount_wallet],["key" => "corporate_amount","value"=>(double)$corporate_amount]],"createdby_username"=>$this->session->get('name')];
   ...
   /* }
    ...
    {
        $updatequery = ['performance_status'=>$performance_status,'dispatcher_id'=>(int)$this->userid];
             $updateresult = $this->mongo_db->update(MDB_PASSENGERS_LOGS,['_id'=>(int)$passenger_logid],['$set'=>$updatequery],['upsert'=>true]);
             return $updateresult;
    }
