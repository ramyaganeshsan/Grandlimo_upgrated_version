...
class Model_Taxidispatchnode extends Model
{
    ...
    public function addbooking($post, $random_key, $password, $company_tax)
    {
        ...
        if ($post['recurrent'] == 1) {
            ...
            $insert_array   = array('_id' => (int)$log_id,
                ...
                'approx_duration' => (int)$post['total_duration_secs'],
                ...
                'airport_pickup' => isset($post['airport_pickup'])? (int)$post['airport_pickup']:0,
                'airport_type' => isset($post['airport_type'])? (int)$post['airport_type'] : 0,
                // 'zone_fare_applicable' => (int)isset($post['zone_fare_applicable'])?$post['zone_fare_applicable']:0,
                // 'zone_fare_type' => (int)isset($post['zone_fare_type'])?$post['zone_fare_type']:0,

                /* Sasidharan Aug 10 2023 */
                'zone_fare_applicable' => isset($post['zone_fare_applicable'])?(int)$post['zone_fare_applicable']:(int)0,
                'zone_fare_type' => isset($post['zone_fare_type'])?(int)$post['zone_fare_type']:(int)0,
   ...
   /* public function get_all_booking_list_all($array)
    ...
    {   //echo $this->session->get('name');
        ...
        $update_array = array(
            ...
            // 'airport_pickup' => (int)isset($post['edit_airport_pickup'])?$post['edit_airport_pickup']:0,
            // 'airport_type' => (int)isset($post['edit_airport_type'])?$post['edit_airport_type']:0,
            
            'zone_fare_applicable' => isset($post['edit_zone_fare_applicable'])? (int)$post['edit_zone_fare_applicable']:0,
            'zone_fare_type' => isset($post['edit_zone_fare_type']) ? (int)$post['edit_zone_fare_type']:0,
            ...
            if (count($companydispatch) > 0) {
                ...
                if ($tdispatch_type == (int)1 && $pass_logid != '') {
                   // echo $tdispatch_type;exit();
                    ...
                    $performance_status        = $booking_details[0]["performance_status"];
                    $cityname          = '';
                    $search_driver     = '';
                    ...
                    if (count($driver_details) > 0) {
                        ...
                        if($performance_status != 1 && $performance_status != 2 && $performance_status != 4)
                        {
        ...
        $arguments = array(
            ...
            array(
                '$project' => array(
                    ...
                    'performance_status'=>array('$ifNull'=>array('$performance_status',0)),
                )
            ),
        );
        //echo "<pre>"; print_r($arguments);
...
$mat['updatetime_difference'] = array( '$lte' => (int)LOCATIONUPDATESECONDS);
    ...
    public function cancelbooking_logid($data)
    {
        ...
        if (!empty($selectresult) && count($selectresult)>0) {
            if ($selectresult['travel_status'] != '5' || $selectresult['travel_status'] != '2') {
                $updateresult = $this->mongo_db->update(MDB_PASSENGERS_LOGS,array('_id'=>(int)$log_id),array('$set'=>array('travel_status'=>8,'cancel_reason'=>$cancel_reason,'performance_status'=>(int)5,'dispatcher_id'=>(int)$this->session->get('userid'))),array('upsert'=>true));
    ...
    public function get_current_driver($passengerlog_id = "")
    {        
        $ops = array(
            array('$match' => array('_id'=>(int)$passengerlog_id)),
            array(
        ...
        $results = $this->mongo_db->aggregate(MDB_PASSENGERS_LOGS,$ops);
        return (!empty($results['result'][0]['driver_id']))?$results['result'][0]['driver_id']:0;
    }
    ...
    public function updatebooking_logid($data)
    {
        ...
        $current_driver_id   = $this->get_current_driver($data['pass_logid']);
        ...
        $updatequery = array(
            ...
            'msg_status'=>'U',
            'comments' => '',
            'previous_driver_id'=>(int)$current_driver_id,
            "createdby_username"=>$this->session->get('name'),
            "operator_id"=>$userid,
            'dispatch_time'=>new MongoDate(strtotime($current_datetime))
        );
        ...
        if ($pass_logid != '') {
            ...
            if(count($booking_details)>0)
            {
             ...
             //  print_r($driver_details); die();
                ...
                //             $updatequery['surge_price_percentage'] = $surge_price_percentage;
                //         }
                //     }
                // }
 ...
 public function search_upcoming_trips()
  ...
  $arguments = array(array(
   ...
   /* }
    ...
    public function update_performance_status($performance_status,$passenger_logid)
    {
        $updatequery = array('performance_status'=>$performance_status,'dispatcher_id'=>(int)$this->userid);
             $updateresult = $this->mongo_db->update(MDB_PASSENGERS_LOGS,array('_id'=>(int)$passenger_logid),array('$set'=>$updatequery),array('upsert'=>true));
             return $updateresult;
    }
