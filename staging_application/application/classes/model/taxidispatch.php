...
class Model_Taxidispatch extends Model
{
 ...
 if($company_id!=0){
    ...
    public function addbooking($post, $random_key, $password, $company_tax)
    {
        ...
        $giftcard = isset($post['giftcard'])?$post['giftcard']:'';
        $corporate_name = isset($post['corporate_name'])?$post['corporate_name']:'';
        ...
        $applied_corporate_promocode = 0;

        /* Sasidharan apr 16 - 2022 */
        $corporate_promocode = (isset($post['corporate_promocode']) && !empty($post['corporate_promocode'])) ? $post['corporate_promocode'] : "";
        if($corporate_promocode != "") {
            $splittedPromocode = explode("-",$corporate_promocode);
            if(isset($splittedPromocode[0]) && !empty($splittedPromocode[0])) {
                $current_time = convert_timezone('now', TIMEZONE);
                $condition =array(
                    'promocode' => $splittedPromocode[0],
                    'corporate_promocode' => 1,
                    'promo_limit' => array( '$gt' => 0 ),
                    'start_date'=>array('$lte' => New MongoDate(strtotime($current_time))),
                    'expire_date'=>array('$gte' => New MongoDate(strtotime($current_time))),
                ); 
                $promocodeDetails = $this->mongo_db->find_one(
                    MDB_PASSENGERS_PROMO,
                    $condition,
                    array('_id','promocode','total_used','total_applied','total_airport_trips','promo_limit')
                );
                if(isset($promocodeDetails['_id']) && isset($promocodeDetails['promo_limit']) && $promocodeDetails['promo_limit'] > 0 && isset($promocodeDetails['total_applied']) && $promocodeDetails['total_applied'] <= $promocodeDetails['promo_limit']) {
                    $promocode = $splittedPromocode[0];
        ...
        if ($post['recurrent'] == 1) {
            ...
            if($post['current_location'] !='' && $post['drop_location'] !='' && $post['pickup_lat'] != '' && $post['pickup_lng'] != '' && $post['drop_lat'] !='' && $post['drop_lng']!=''){
                if($post['total_fare'] >0 ){
                    ...
                    $fixed_estimate_result = array('estimate_fare'=>(float)$post['total_fare'],'estimate_km'=>(float)$post['distance_km'],'estimate_duration'=>0,'estimate_model'=>(int)$post['taxi_model']);
                }
            }
            ...
            $insert_array   = array('_id' => (int)$log_id,
                ...
                'promocode' => $promocode,
                "pickupdrop" => "",
                'airport_pickup' => isset($post['airport_pickup'])?(int)$post['airport_pickup']:(int)0,
                'airport_type' => isset($post['airport_type'])?(int)$post['airport_type']:(int)0,
                ...
                "fixed_estimate_result"=> (object)$fixed_estimate_result,
                "gift_card_status"=>(int)$gift_card_status,
                "corporate_name"=>$corporate_name,
                "applied_corporate_promocode"=>(int)$applied_corporate_promocode,
            );
   ...
   /* public function get_all_booking_list_all($array)
    ...
    public function get_all_booking_list_all($array)
    {
       ...
       //print "<pre>"; print_r($matchquery); exit;
        $arguments = array(
           ...
           array(
            ...
            array(
                '$unwind' =>array('path'=>'$motormodel_edit','preserveNullAndEmptyArrays'=>true)
            ),
            array(
                  '$lookup' => array(
      ...
      $project_query = array(      array(
                '$project' => array(
                    ...
                    'edit_model_name' => array('$ifNull'=>array('$motormodel_edit.model_name','')),
                    ...
                    /* Sasi */
                    'discount_wallet_amount_used'=>array('$ifNull'=>array('$trans.discount_wallet_amount_used',0)),
                )
            ));
        ...
        if (FARE_SETTINGS == 2 && !empty($company_id)) {
            $arguments = array(
               ...
               /* array(
                ...
                array(
                    '$lookup' => array(
                        'from' => MDB_CORPORATE_PAYMENT_LOG,
                        'localField' => '_id',
                        'foreignField' => "tripId",
                        'as' => "corporatePayment"
                    )
                ...
                array(
                    '$unwind' => array('path'=>'$corporatePayment','preserveNullAndEmptyArrays'=>true)
                ),
                ...
                array(
                    '$project' => array(
                        ...
                        'applied_corporate_promocode' => array('$ifNull'=>array('$applied_corporate_promocode',0)),
                        'pending_amt'=>array('$ifNull'=>array('$trans.pending_amt','')),
                        'corporate_name'=>array('$ifNull'=>array('$corporate_name','')),

                        /* Sasidharan dec 18 2022 */
                        'airport_pickup'=>array('$ifNull'=>array('$airport_pickup',0)),
                        'airport_type'=>array('$ifNull'=>array('$airport_type',0)),
                        'zone_fare_applicable'=>array('$ifNull'=>array('$zone_fare_applicable',0)),
                        'zone_fare_type'=>array('$ifNull'=>array('$zone_fare_type',0)),

                        /* Sasidharan Nov 26 2022 */
                        'corporate_amount' => array('$ifNull' => array('$corporatePayment.trip_amount',0))
            ...
            if(isset($result['result'][0]['pay']) && $result['result'][0]['pay'] != '') {
                foreach($result['result'][0]['pay'] as $key => $value) {
                    ...
                    $result['result'][0]['pay'][$key] = ((string)$result['result'][0]['pay'][$key] == "NAN") ? 0 : $result['result'][0]['pay'][$key];
                }
            }
    ...
    {   //echo $this->session->get('name');
        ...
        /* Sasidharan aug 7 2022 */
        $applied_corporate_promocode = 0;
        $corporate_promocode = (isset($post['edit_corporate_promocode']) && !empty($post['edit_corporate_promocode'])) ? $post['edit_corporate_promocode'] : "";
        if($corporate_promocode != "") {
            $splittedPromocode = explode("-",$corporate_promocode);
            if(isset($splittedPromocode[0]) && !empty($splittedPromocode[0])) {
                $current_time = convert_timezone('now', TIMEZONE);
                ...
                /*  
                  ...
                  Added total_applied param in $promocodeDetails
                */
                $promocodeDetails = $this->mongo_db->find_one(
                    MDB_PASSENGERS_PROMO,
                    $condition,
        ...
        $giftcard = isset($post['edit_giftcard'])?$post['edit_giftcard']:'';
        $corporate_name = isset($post['edit_corporate_name'])?$post['edit_corporate_name']:'';
        ...
        if($post['edit_current_location'] !='' && $post['edit_drop_location'] !='' && $post['edit_pickup_lat'] != '' && $post['edit_pickup_lng'] != '' && $post['edit_drop_lat'] !='' && $post['edit_drop_lng']!=''){
            if($post['edit_total_fare'] >0 ){
                $fixed_estimate_fare_type = 1;
                $fixed_estimate_result = array('estimate_fare'=>(float)$post['edit_total_fare'],'estimate_km'=>(float)$post['edit_distance_km'],'estimate_duration'=>0,'estimate_model'=>(int)$post['edit_taxi_model']);
            }
        }
        ...
        $update_array = array(
            ...
            /* Sasidharan dec 18 2022 */
            // "airport_pickup" => isset($post['edit_airport_pickup'])?(int)$post['edit_airport_pickup']:(int)0,
            // "airport_type" => isset($post['edit_airport_type'])?(int)$post['edit_airport_type']:(int)0,
        ...
        /* Sasi */
        // $surge_item = $this->commonmodel->get_surge_price_model_item((int)$post['edit_taxi_model']);
        // if(isset($surge_item[0]['surge_pricing_status']) && $surge_item[0]['surge_pricing_status'] == 'A') {
        //     if(isset($passenger_id)) {
        //         $passengerGroup = $this->commonmodel->getPassengerGroupInfo($passenger_id);
        ...
        //             $update_array['surge_price_status'] = 1;
        ...
        $arguments = array(
            ...
            array(
                '$project' => array(
                    ...
                    'applied_corporate_promocode' => array('$ifNull'=>array('$applied_corporate_promocode',0)),
                    'edit_model_min_fare' => array('$ifNull'=>array('$motormodel_edit.min_fare','')),
                    'taxi_edit_modelid'=>array('$ifNull'=>array('$motormodel_edit._id','')),
                )
            ),
        );
        //echo "<pre>"; print_r($arguments);
...
$mat['updatetime_difference'] = array( '$lte' => (int)LOCATIONUPDATESECONDS);
    ...
    public function test_sms() {
        $to = "96566324783";
        $message = "Test Message Sep 6 2020";

        $this->api_model->sendSMS($to, $message);
        echo "Done";
        exit;
    }
    public function cancelbooking_logid($data)
    {
        ...
        if (!empty($selectresult) && count($selectresult)>0) {
            if ($selectresult['travel_status'] != '5' || $selectresult['travel_status'] != '2') {
                ...
                $title 			   ="Dispatcher cancelled the trip - ألغى المنسق الرحلة";
								
				$pushmessage = array(
    "message" => $title, 
    "trip_id" => isset($data['pass_logid']) ? $data['pass_logid'] : "", 
    "app_ver" => $app_ver,
    "status" => 17
); 
                if ($p_device_token != '') {
                ...
                } else if ($p_device_type == 2) {
        $this->api_model->send_ios_push_via_node($p_device_token, $pushmessage);
    }
            ...
            array('$unwind' => '$model'),
                array(
                    '$project' => array(
                        ...
                        'loc' => '$loc.coordinates',
                        'shift_status' => '$shift_status',
                        'gps_enable' => array('$ifNull'=>array('$gps_enable',1)),
                        'update_date' => '$update_date',
            ...
            array('$unwind' => '$model'),
                array(
                    '$project' => array(
                        ...
                        'gps_enable' => array('$ifNull'=>array('$driver_driverinfo.gps_enable',1)),
                        'update_date' => '$driver_driverinfo.update_date',
                        'model_name'=>'$model.model_name',
                        'trip_remaining_time'=>array('$ifNull'=>array('$driver_driverinfo.trip_remaining_time',0)),
                        'updatetime_difference' =>  array( '$divide' => array( array('$subtract' =>array(new \MongoDB\BSON\UTCDateTime(strtotime($company_current_time) * 1000),'$driver_driverinfo.update_date')), 1000)),
                        'model_id'=>'$model._id',//26 feb 2021
                    )
                ),
               //array('$match' => array('updatetime_difference'=>array('$lte'=>(int)LOCATIONUPDATESECONDS ))),
       ...
       /* $current_time_hour            = (int)date('H', strtotime($array['current_time']));
        ...
        $arguments = array(array(
            ...
            array(
              ...
              '$unwind' =>array('path'=>'$promocodeDetails','preserveNullAndEmptyArrays'=>true)
            ),
            array(
                '$lookup' => array(
                    'from' => MDB_MOTOR_MODEL,
            ...
            array(
                '$project' => array(
                    ...
                    'hourly_booking'=>array('$ifNull'=>array('$hourly_booking',0)),
                    'hourly_booking_hrs'=>array('$ifNull'=>array('$hourly_booking_hrs',0)),
                    'tripDistance'=>array('$ifNull'=>array('$trans.distance',0)),
                    'distanceUnit'=>array('$ifNull'=>array('$trans.distance_unit',0)),
                    'apply_estimate_fare'=>array('$ifNull'=>array('$trans.apply_estimate_fare',0)),
                    'corporate_name'=>array('$ifNull'=>array('$corporate_name','')),
                    /* Sasi */
                    'discount_wallet_amount_used'=>array('$ifNull'=>array('$trans.discount_wallet_amount_used',0)),

                    /* Sasidharan apr 16 - 2022 */
                    'corporate_promocode' => array('$ifNull' => array('$promocodeDetails.corporate_promocode',0)),

                    /* Sasidharan aug 4 2022 */
                    'promocode'=>array('$ifNull'=>array('$promocode',"-")),

                    /* Sasidharan jan 17 2025 */
                    'couponAmount'=>array('$ifNull'=>array('$passengers.couponAmount',0)),

                    /* Sasidharan july 19 2023 */
                    'cancelation_time_string' => array('$ifNull'=>array('$cancelation_time_string',"-")),
                    // New Field //
                )
            ),
            array(
        ...
        if ($pass_logid != '') {
            ...
            if(count($booking_details)>0)
            {
             ...
             //  print_r($driver_details); die();
                ...
                /* Sasi */
                // $surge_item = $this->commonmodel->get_surge_price_model_item((int)$taxi_model);
                // if(isset($surge_item[0]['surge_pricing_status']) && $surge_item[0]['surge_pricing_status'] == 'A') {
                //     if(isset($booking_details[0]['passengers_id'])) {
                //         $passengerGroup = $this->commonmodel->getPassengerGroupInfo($booking_details[0]['passengers_id']);
                //         if(isset($passengerGroup[0]) && isset($passengerGroup[0]['corporate_trip_discount']) && $passengerGroup[0]['corporate_trip_discount'] == 1) {
                //             $discount = isset($passengerGroup[0]['corporate_discount']) ? $passengerGroup[0]['corporate_discount'] : 0;
                //             $surge_price_percentage = -$discount;

                //             $updatequery['surge_price_status'] = 1;
                //             $updatequery['surge_price_percentage'] = (float)$surge_price_percentage;
                //         }
                //     }
                // }
  ...
  /***** Insert the druiver details to driver request table ************/
         ...
         public function confirmbooking_logid($data)
    {
        ...
        if (isset($get_passenger_log_det[0]) && !empty($get_passenger_log_det[0])) {
			...
			$app_ver         = isset($get_passenger_log_det[0]['app_ver']) ? $get_passenger_log_det[0]['app_ver'] : "";
			$detail        = array(
						"confirm_message" => "Booking created by dispatcher on - تم إنشاء الحجز بواسطة المنسق في",
 ...
 public function search_upcoming_trips()
  ...
  $arguments = array(array(
    ...
    /* Sasi */
    $discount_wallet = isset($post['discount_wallet']) ? (double)$post['discount_wallet'] : 0;

    /* Sasidharan nov 26 2022 */
    $corporate_amount = isset($post['corporate_amount']) ? (double)$post['corporate_amount'] : 0;
    $ifCorporateLogExists = $this->mongo_db->find_one(MDB_CORPORATE_PAYMENT_LOG, ['tripId' => (int)$id], ['tripId', 'trip_amount']);

    if(isset($ifCorporateLogExists['tripId'])) {
        // $previousAmount = isset($ifCorporateLogExists['trip_amount']) ? $ifCorporateLogExists['trip_amount'] : 0;
        // if($previousAmount != $corporate_amount) {
            $current_date   = date('Y-m-d H:i:s', time());
            $updateArray = [
                'trip_amount' => (double)$corporate_amount,
                'dispatcher_altered' => 1,
                'updatedAt' => new \MongoDB\BSON\UTCDateTime(strtotime($current_date) * 1000)
            ];
            $this->mongo_db->update(MDB_CORPORATE_PAYMENT_LOG, ['tripId' => (int)$id], ['$set' => $updateArray]);
        // }
    } else if($corporate_amount > 0){
        $filter = [
            [
                '$match' => [
                    '_id' => (int)$id
            ...
            [
                '$project' => [
                    'passengers_id' => array('$ifNull' => array('$passengers_id',0)),
                    'promocode' => array('$ifNull' => array('$promocode',"")),
                    'corporate_group' => array('$ifNull' => array('$promocodeDetails.corporate_group',0))
                ]
            ]
        ];
        $tripInfo = $this->mongo_db->aggregate(MDB_PASSENGERSLOGS_COMPLETED,$filter);
        $tripInfo = isset($tripInfo['result'][0]) ? $tripInfo['result'][0] : [];
        ...
        if(count($tripInfo) > 0) {
            ...
            $insertArray = [
                "tripId" => (int)$id,
                "passengerId" => (int)$tripInfo['passengers_id'],
                "promocode" => $tripInfo['promocode'],
                "discount" => 0,
                "trip_amount" => (float)$corporate_amount,
                "airport_trip" => 0,
                "createdAt" => new \MongoDB\BSON\UTCDateTime(strtotime($current_date) * 1000),
                "corporate_id" => $tripInfo['corporate_group'],
                "total_fare_backup" => 0,
                "is_airport_discount_applied" => 0,
                "paid_status" => 0,
                'dispatcher_altered' => 1,
            ];
            $this->mongo_db->insert(MDB_CORPORATE_PAYMENT_LOG,$insertArray);
        }
    }

    $update_array = array(
        "fare_detail" => array( 
            array("key"=>"1","value" => (double)$post["cash_pay"]), 
            array("key"=>"6","value" => (double)$post["card_pay"]), 
            array("key"=>"3","value" => (double)$post["knet_pay"]), 
            array("key"=>"additional amount","value" => (double)$post["add_amt"]),
            array("key"=>"wallet","value"=>(double)$post["wallet_pay"]),
            array("key"=>"pending","value"=>(double)$post["pending_pay"]),
            array("key"=>"fare_note","value"=>$post["fare_note"]),
            array("key" => "discount_wallet","value"=>(double)$discount_wallet),
            array("key" => "corporate_amount","value"=>(double)$corporate_amount))
            ,"createdby_username"=>$this->session->get('name')
    );

    /* Sasidharan Jan 31 2023 */
    $oldFareDetails = $this->mongo_db->find_one(MDB_PASSENGERS_LOGS,array('_id'=>(int)$id),array('fare_detail' => 1, "_id" => 0 ));

    $alteredUserName = isset($post['username']) ? $post['username'] : "";
    $alteredUserEmail = isset($post['email']) ? $post['email'] : "";
    $alteredUserId = isset($post['id']) ? $post['id'] : "";

    $mailResponse = $this->commonmodel->sendNotificationEmail($id, $alteredUserName,$update_array['fare_detail'],$oldFareDetails['fare_detail']);

    $current_date   = date('Y-m-d H:i:s', time());
    $logData = array(
        "old_fare" => $oldFareDetails['fare_detail'],
        "updated_fare" => $update_array['fare_detail'],
        "alteredUserName" => $alteredUserName,
        "alteredUserEmail" => $alteredUserEmail,
        "alteredUserId" => (int)$alteredUserId,
        "trip_id" => (int)$id,
        "mail_status" => $mailResponse ? 1 : 0,
        'createdAt' => new \MongoDB\BSON\UTCDateTime(strtotime($current_date) * 1000)
    );
    $emailLog = $this->mongo_db->insert(MDB_TRIP_FARE_EDIT_LOG, $logData);

    $updateresult = $this->mongo_db->update(MDB_PASSENGERS_LOGS, array('_id'=>(int)$id),array('$set'=>$update_array),array( 'upsert' => true));  
    $updateresult = $this->mongo_db->update(MDB_PASSENGERSLOGS_COMPLETED, array('_id'=>(int)$id),array('$set'=>$update_array),array( 'upsert' => true));

    $pending_pay =$post["pending_pay"];
    $prev_pending_pay =$post["prev_pending_pay"];


    /* Sasidharan aug 9 2022 */
    ...
    if($post["cash_pay"] > 0 && $post["card_pay"] > 0)
    {
        $actual_paid_amt = $post["cash_pay"] + $post["card_pay"];
        $payment_type = 8;
    }
   ...
   /* }
    ...
    if (count($trans_result_data) > 0) {
       $updateresult = $this->mongo_db->update(MDB_TRANSACTION, array('passengers_log_id'=>(int)$id),array('$set'=>$update_trans_array));
    } else {

        $rs = $this->mongo_db->find(MDB_TRANSACTION,array(),array('_id'))->sort(array('_id'=>-1))->limit(1);
    ...
    public function insert_wallet_log($data)
    {
        ...
        if($insert_array['recharge_type'] == 7) {
            $insert_array['notes'] = "Fare altered by ".$this->session->get('name');
            $insert_array['edited_by'] = (int)$this->session->get('userid');
        }
    ...
    public function get_total_sum($operator_id)
    {
        ...
        $match_array = array( "\$and" => array( array('pickup_time' => array('$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($currentdate) * 1000))), array('pickup_time' =>array('$lte' => new \MongoDB\BSON\UTCDateTime(strtotime($current_date_last) * 1000)) ), array('travel_status'=>(int)1 )) );
    ...
    public function get_promocodes(){
        ...
        $current_time = convert_timezone('now', TIMEZONE);
         $match_arg =array('$and'=>array(array('$or'=>array(
         array('auto_generated_promocode'=>array('$ne'=>(int)1)),
         array('corporate_dispatcher_enable'=>array('$eq'=>(int)1))
         ),
         'start_date'=>array('$lte' => New MongoDate(strtotime($current_time))),
         'expire_date'=>array('$gte' => New MongoDate(strtotime($current_time))),
         /* Sasidharan apr 16 - 2022 */
         'corporate_promocode' => array('$ne' => 1)
         )));
    ...
    public function checkpromocode($promo_code = "", $customer_number = "", $company_id = "",$passenger_id="",$passenger_info = array())
    {
       // $match = array('promocode' => $promo_code, array( "\$or"=> array( array( 'promo_type' => "1"),array('promo_type' => 1) ) ) );
        ...
        if (count($promo_fetch) > 0) {
            ...
            /* Check if it is register promocode */
            $register_promocode = isset($promo_fetch['register_promocode']) ? $promo_fetch['register_promocode'] : 0;
            $passenger_registered_date = isset($passenger_info[0]['created_date']) ? Commonfunction::convertphpdate('Y-m-d H:i:s',$passenger_info[0]['created_date']) : "";

            if($register_promocode && $passenger_registered_date != "") {
              if(strtotime($passenger_registered_date) < strtotime(REGISTER_PROMOCODE)) {
                return 4;
              } else {
    ...
    $updateresult = $this->mongo_db->update(MDB_PASSENGERS_LOGS, array('_id'=>(int)$id),array('$set'=>$update_array));
    $updateresult = $this->mongo_db->update(MDB_PASSENGERSLOGS_COMPLETED, array('_id'=>(int)$id),array('$set'=>$update_array),array( 'upsert' => true));
   ...
   public function get_past_places_old($passenger_id = "")
   {
         //echo $passenger_id.'--id--'.$current_lat.'-lat lng-'.$current_lng;exit;
        $match_query = array('passengers_id'=>(int)$passenger_id,'current_location'=>array('$ne'=>''),'drop_location'=>array('$ne'=>''),'pickup_latitude'=>array('$ne'=>''),'pickup_longitude'=>array('$ne'=>''),'drop_latitude'=>array('$ne'=>''),'drop_longitude'=>array('$ne'=>''));
   ...
   {
         //echo $passenger_id.'--id--'.$current_lat.'-lat lng-'.$current_lng;exit;
        $match_query = array('passenger_id'=>(int)$passenger_id,'p_favourite_place'=>array('$ne'=>''),'p_fav_latitude'=>array('$ne'=>''),'p_fav_longtitute'=>array('$ne'=>''));
        ...
        $common_arguments = array(            
            ...
            //$merge_arguments = array_merge($common_arguments, $field_arguments);
            $result    = $this->mongo_db->aggregate(PASSENGERS_FAV, $field_arguments);
            //echo "<pre>";print_r($result['result']); exit;
            $res = (!empty($result['result'])) ? $result['result'] : array();
            ...
            if(count($res)){
                ...
                foreach($res as $k=>$val){
                    ...
                    $p_fav_latitude = $val['p_fav_latitude'];
                    $p_fav_longtitute = $val['p_fav_longtitute'];
                    $p_favourite_place = $val['p_favourite_place'];
                    $fav_loction_type = isset($val['fav_loction_type'])?ucfirst($val['fav_loction_type']):'';

                    if(!in_array($p_favourite_place,$total_places) ){

                        $total_places[] = $p_favourite_place;                        
                        $total_place_lat_long[] = array('place'=>$p_favourite_place,'latitude'=>$p_fav_latitude,'longitude'=>$p_fav_longtitute,'type'=>$fav_loction_type);
                    }                
                }
            }
            //echo 555;exit;
   ...
   public function get_last_four_drivers_list($passenger_id = "")
   {
        ...
        $common_arguments = array(            
            ...
            $field_arguments = array(
                ...
                array(
                    '$project' => array(
                        ...
                        'driver_code'=>'$people.driver_code',
                        'driver_name'=>'$people.name',
                        'driver_last_name'=>'$people.last_name',
                        "driver_model"=>'$model.model_name',
                    )
                ),
                array('$skip'   => (int)0 ),
            ...
            if(count($res)){
                ...
                foreach($res as $k=>$val){
                    $driver_model = isset($val['driver_model'][0])?$val['driver_model'][0]:'';
                    if($val['driver_name'] != '' && $val['driver_code'])
                    $drivers_list .= "<li>".ucfirst($val['driver_name'])."(".$val['driver_code'].")"."  - ".$driver_model."</li>";
                }
            }
   ...
   //26 feb 2021
   public function get_corporate_list(){

     $results = $this->mongo_db->find(MDB_CORPORATE,array('status'=>'A'),array('id','corporate_name'));
     //print_r($result);exit;
     return (!empty($results))?iterator_to_array($results):array();
    }

    public function check_trip_requests_exits($driver_id)
    {
        $company_current_time = $this->company_current_time;
        //$condition =array('phone' => $phone_no,'user_status'=>'A');
        $up_time_milli = 150 * 1000;
        $arguments = array(
            array(
                '$match' => array(
                    "selected_driver" => (int)$driver_id,
                    "status" => array('$in' => array(1, 3, 0)),
                )
            ),
            array(
                '$project' => array(
                    '_id' => 1,
                    'updatetime_difference' => array('$subtract' => array(new \MongoDB\BSON\UTCDateTime(strtotime($company_current_time) * 1000), '$createdate'))
                )
            ),
            array(
                '$match' => array(
    ...
    /* Sasidharan apr 16 2022 */
    public function getCorporatePromocode($like) {
        $current_time = convert_timezone('now', TIMEZONE);
        $condition =array(
            'promocode' => new MongoRegex("/$like/i"),
            'corporate_promocode' => 1,
            'promo_limit' => array( '$gt' => 0 ),
            'total_airport_trips' => array( '$gt' => 0 ),
            'start_date'=>array('$lte' => New MongoDate(strtotime($current_time))),
            'expire_date'=>array('$gte' => New MongoDate(strtotime($current_time))),
        );
        $results = $this->mongo_db->find(MDB_PASSENGERS_PROMO,$condition,array('_id','promocode','total_used','total_airport_trips','promo_limit'));
        return (!empty($results))?iterator_to_array($results):array();
    }
    ...
    public function checkUsernameAndPassword($username, $password) {
        ...
        $result  = $this->mongo_db->find_one("dispatcher_fare_edit_passwords", $filter,['password' => 0, "org_password" => 0]);
        return $result;
    }

    public function getOnGoingDriverList() {
        $date                       = date('Y-m-d');
        $currentdate                = $date . ' 00:00:00';
        $enddate                    = $date . ' 23:59:59';
        $aggregate = [
            ...
            [
                '$match' => [
                    "driver_info.status" => "B",
                    "driver_info.shift_status" => "IN",
                ]
            ],
            [
                '$project' => [
                    ...
                    'trip_id' => '$_id',
                    'gps_enable' => '$driver_info.gps_enable',   
                    'travel_status' => '$travel_status',   
                ]
            ],
            [
