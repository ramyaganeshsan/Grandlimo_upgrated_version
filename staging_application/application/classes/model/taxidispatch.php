        ...
        if($corporate_promocode != "") {
            ...
            if(isset($splittedPromocode[0]) && !empty($splittedPromocode[0])) {
                ...
                $condition =[
                    ...
                    'promo_limit' => [ '$gt' => 0 ],
                    'start_date'=>['$lte' => new \MongoDB\BSON\UTCDateTime(strtotime($current_time) * 1000)],
                    'expire_date'=>['$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($current_time) * 1000)],
                ]; 
                $promocodeDetails = $this->mongo_db->find_one(
                    MDB_PASSENGERS_PROMO,
                    $condition,
                    ['_id','promocode','total_used','total_applied','total_airport_trips','promo_limit']
                );
    ...
    public function get_all_booking_list_all($array)
    {
      ...
      $project_query = [      [
                '$project' => [
                    ...
                    'pass_secondary_phone' => ['$ifNull'=>['$passengers.secondary_phone','']],
                    'pass_secondary_name' => ['$ifNull'=>['$passengers.secondary_name','']],
                    ...
                    /* Sasi */
                    'discount_wallet_amount_used'=>['$ifNull'=>['$trans.discount_wallet_amount_used',0]],
                ]
            ]];
        ...
        if (FARE_SETTINGS == 2 && !empty($company_id)) {
            $arguments = [
               ...
               /* array(
                ...
                [
                    '$unwind' => ['path'=>'$corporatePayment','preserveNullAndEmptyArrays'=>true]
                ],
                ...
                [
                    '$project' => [
                        ...
                        'applied_corporate_promocode' => ['$ifNull'=>['$applied_corporate_promocode',0]],
                        'pending_amt'=>['$ifNull'=>['$trans.pending_amt','']],
                        'corporate_name'=>['$ifNull'=>['$corporate_name','']],

                        /* Sasidharan dec 18 2022 */
                        'airport_pickup'=>['$ifNull'=>['$airport_pickup',0]],
                        ...
                        /* Sasidharan Nov 26 2022 */
                        'corporate_amount' => ['$ifNull' => ['$corporatePayment.trip_amount',0]]
  ...
  $arguments = [[
    ...
    if(isset($ifCorporateLogExists['tripId'])) {
        ...
        $filter = [
            ...
            [
                '$project' => [
                    'passengers_id' => ['$ifNull' => ['$passengers_id',0]],
                    'promocode' => ['$ifNull' => ['$promocode',""]],
                    'corporate_group' => ['$ifNull' => ['$promocodeDetails.corporate_group',0]]
                ]
            ]
        ];
    ...
    $update_array = [
        "fare_detail" => [ 
            ["key"=>"1","value" => (double)$post["cash_pay"]], 
            ["key"=>"6","value" => (double)$post["card_pay"]], 
            ["key"=>"3","value" => (double)$post["knet_pay"]], 
            ["key"=>"additional amount","value" => (double)$post["add_amt"]],
            ["key"=>"wallet","value"=>(double)$post["wallet_pay"]],
            ["key"=>"pending","value"=>(double)$post["pending_pay"]],
            ["key"=>"fare_note","value"=>$post["fare_note"]],
            ["key" => "discount_wallet","value"=>(double)$discount_wallet],
            ["key" => "corporate_amount","value"=>(double)$corporate_amount]]
            ,"createdby_username"=>$this->session->get('name')
    ];

    /* Sasidharan Jan 31 2023 */
    $oldFareDetails = $this->mongo_db->find_one(MDB_PASSENGERS_LOGS,['_id'=>(int)$id],['fare_detail' => 1, "_id" => 0 ]);
   ...
   /* }
    ...
    public function get_promocodes(){
        ...
        $current_time = convert_timezone('now', TIMEZONE);
         $match_arg =['$and'=>[['$or'=>[
         ['auto_generated_promocode'=>['$ne'=>(int)1]],
         ['corporate_dispatcher_enable'=>['$eq'=>(int)1]]
         ],
         'start_date'=>['$lte' => new \MongoDB\BSON\UTCDateTime(strtotime($current_time) * 1000)],
         'expire_date'=>['$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($current_time) * 1000)],
         /* Sasidharan apr 16 - 2022 */
    ...
    public function checkpromocode($promo_code = "", $customer_number = "", $company_id = "",$passenger_id="",$passenger_info = [])
    {
   ...
   {
         //echo $passenger_id.'--id--'.$current_lat.'-lat lng-'.$current_lng;exit;
        $match_query = ['passenger_id'=>(int)$passenger_id,'p_favourite_place'=>['$ne'=>''],'p_fav_latitude'=>['$ne'=>''],'p_fav_longtitute'=>['$ne'=>'']];
            ...
            if(count($res)){
                ...
                foreach($res as $k=>$val){
                    ...
                    if(!in_array($p_favourite_place,$total_places) ){
                        ...
                        $total_place_lat_long[] = ['place'=>$p_favourite_place,'latitude'=>$p_fav_latitude,'longitude'=>$p_fav_longtitute,'type'=>$fav_loction_type];
                    }                
                }
            }
            //echo 555;exit;
   ...
   public function get_corporate_list(){
     ...
     $results = $this->mongo_db->find(MDB_CORPORATE,['status'=>'A'],['id','corporate_name']);
     //print_r($result);exit;
     return (!empty($results))?iterator_to_array($results):[];
    }
    ...
    public function check_trip_requests_exits($driver_id)
    {
        ...
        $arguments = [
            ...
            [
                '$project' => [
                    '_id' => 1,
                    'updatetime_difference' => ['$subtract' => [new \MongoDB\BSON\UTCDateTime(strtotime($company_current_time) * 1000), '$createdate']]
                ]
            ],
            [
                '$match' => [
    ...
    public function getCorporatePromocode($like) {
        $current_time = convert_timezone('now', TIMEZONE);
        $condition =[
            'promocode' => new \MongoDB\BSON\Regex($like, 'i'),
            'corporate_promocode' => 1,
            'promo_limit' => [ '$gt' => 0 ],
        ...
        $results = $this->mongo_db->find(MDB_PASSENGERS_PROMO,$condition,['_id','promocode','total_used','total_airport_trips','promo_limit']);
        return (!empty($results))?iterator_to_array($results):[];
    }
