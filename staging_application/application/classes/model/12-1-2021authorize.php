...
class Model_Authorize extends Model
{
    ...
    public function login_details_by_passengerid($userid)
    {
        ...
        //MongoDB
        $result = $this->mongo_db->find_one(MDB_PASSENGERS,array('_id'=>(int)$userid),array('_id','name','lastname','email','address','phone','discount','pay_by','trip_amt_limit','vip_user','notes','country_code','lateral_start_date','lateral_end_date'));
        return (!empty($result)) ? $result : array();
    }
