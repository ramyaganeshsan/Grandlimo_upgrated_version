        ...
        $arguments   = array(
            ...
            array(
                '$project' => array(
                    ...
                    'surge_price_status'=>array('$ifNull'=>array('$surge_price_status',0)),//Kumaresh
                    'surge_price'=>array('$ifNull'=>array('$trans.surge_price',0)),//Kumaresh
                )
            ),
            array(
                '$sort' => array(
             ...
             'tfare'=>array('$cond'=>array(
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        'pending_amt'=>array('$cond'=>array(
                            array('$gt'=>array('$trans.driver_edit_status',0)),
                            array('$subtract'=>array('$trans.pending_amt','$trans.passenger_pending_amt')),
                            0,
                        )), 
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
           ...
           // $merge_arguments = array_merge($common_arguments, $count_arguments);
            ...
            'edit_status'=>array(
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        ...
                        'pickup_time'=>'$pickup_time',
                        //'fare'=>'$trans.fare',
			'wallet_amount_used'=>array('$cond'=>array(
                            array('$eq'=>array('$trans.advance_payment',1)),
                            0,
			    '$wallet_amount_used'
                        )),
                        //'wallet_amount_used'=>array('$ifNull'=>array('$trans.wallet_amount_used',0)),
                ...
                array(
                    '$group' => array(
                       ...
                       //  'pickup_time' => array('$push'=>'$pickup_time'),
                        'total_amount' => array('$sum' => '$fare'),
'total_wallet'=>array('$sum'=>'$wallet_amount_used'),
                        'total_pending_amt' => array('$sum' => '$pending_amt'),
                        'total_count' => array('$sum' => 1)
                    )
                ),
             ...
             'tfare'=>array('$cond'=>array(
                ...
                array(
                    '$group' => array(
                        ...
                        'total_amount' => array('$sum' => '$tfare'),
                        'total_pending_amt' => array('$sum' => '$pending_amt'),
                        'total_wallet_amt' => array('$sum' => '$wallet_amount_used'),
                        'total_count' => array('$sum' => 1)
                    )
                ),
               array('$skip'   => (int)$offset ),
                ...
                array(
                    '$project' => array(
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        ...
                        'deductedAmountFromTap'=>array('$ifNull'=>array('$deductedAmountFromTap',0)),
                        ...
                        //'total_edited_fare'=>array('$sum'=>'$fare_detail.value'),
                        'createdby_username'=>array('$ifNull'=>array('$createdby_username','-')),
                        'wallet_amt_before_payment'=>array('$ifNull'=>array('$wallet_amt_before_payment','-')),
                        'payment_type'=>array('$ifNull'=>array('$trans.payment_type',0)),
                        'advance_payment'=>array('$ifNull'=>array('$trans.advance_payment',0)),
                        'pending_amt'=> array('$cond'=>array(
                                        array('$gt'=>array('$trans.driver_edit_status',0)),
                        ...
                        'dispatcher_altered'=>array('$ifNull'=>array('$corporatePayment.dispatcher_altered',0)),
                        'tapAutoPayment'=>array('$ifNull'=>array('$tapAutoPayment',0)),
                        'corporate_name'=>array('$ifNull'=>array('$corporateDetails.group_name',"-")),
...
//echo $start_date.'---'.$end_date;exit;
 ...
 //print_r($result);exit;
    ...
    public function get_shift_drivers($analytics_date){
       ...
       return (!empty($result2['result'])) ? $result2['result'] : array();
    ...
    public function get_trips_list($analytics_date){
        ...
        $arguments = array(
            array('$match' => $match_query),
            array('$project' => array('_id'=>'$_id','createdate'=>'$createdate','travel_status'=>'$travel_status'))
        );
        //echo "<pre>"; print_r($arguments);
    ...
    public function passengers_unused_wallet_report($keyword = "", $status = "",$start_date,$end_date, $company = "",$offset = "", $val = "", $find_count = FALSE)
    {
       ...
       $match_query = array(
        ...
        if(!empty($keyword)) {
            $srch_query = array( "\$and" => array($match_query,array("\$or"=>array(array('pass.name' => new MongoRegex("/$keyword/i")) , ) ) ) );
        }else{
            $srch_query = $match_query;
        }
        $common_arguments = array(
    ...
    public function get_trips_fare_details($count) {
        ...
        $filter = array(
            'travel_status' => 1,
            'fare_details_updated' => array('$exists' => false),
            'actual_pickup_time' => array('$gte' => New MongoDate(strtotime($start_date))),
        );
        $project = array(
            "_id" => 1,
            "fare_detail" => 1,
    ...
    public function update_trips_fare_details($id, $fare_info) {
        ...
        $updateResult = $this->mongo_db->update(MDB_PASSENGERSLOGS_COMPLETED, $filter, array('$set'=>$updateArray), array("upsert" => false));
