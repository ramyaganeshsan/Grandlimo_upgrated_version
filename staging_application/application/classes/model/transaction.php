        ...
        $arguments   = [
            ...
            [
                '$project' => [
                    ...
                    'surge_price_status'=>['$ifNull'=>['$surge_price_status',0]],//Kumaresh
                    'surge_price'=>['$ifNull'=>['$trans.surge_price',0]],//Kumaresh
                ]
            ],
            [
                '$sort' => [
...
public function getAllInvoice($passenger,$pay_status,$offset = '', $val = '')
    ...
    public function driver_sales_report($keyword = "", $status = "",$start_date = "",$end_date = "",$year,$month, $company = "", $offset = "", $val = "", $find_count = FALSE)
    {
       ...
       // $current_time  = $get_company_time_details['current_time']; //Start time
        ...
        $match_query['pass.actual_pickup_time'] = [
            ...
            $field_arguments = [
             ...
             'tfare'=>['$cond'=>[
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        'pending_amt'=>['$cond'=>[
                            ['$gt'=>['$trans.driver_edit_status',0]],
                            ['$subtract'=>['$trans.pending_amt','$trans.passenger_pending_amt']],
                            0,
                        ]], 
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
           ...
           // $merge_arguments = array_merge($common_arguments, $count_arguments);
            ...
            'edit_status'=>[
                        ...
                        'fare'=>['$cond'=>[
                            ['$gt'=>['$trans.driver_edit_status',0]],
                            ['$sum'=>['$trans.tripfare','$trans.add_amt']],
                             ['$sum'=>['$trans.actual_paid_amt','$trans.wallet_amount_used','$trans.add_amt']]]
                        ],
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        'pending_amt'=>'$trans.pending_amt',
                        'driver_name'=>'$people.name',
                        ...
                        'pickup_time'=>'$pickup_time',
                        //'fare'=>'$trans.fare',
			'wallet_amount_used'=>['$cond'=>[
                            ['$eq'=>['$trans.advance_payment',1]],
                            0,
			    '$wallet_amount_used'
                        ]],
             		...
             		'pending_amt'=>['$cond'=>[
                        ...
                        'driver_edit_status'=>['$ifNull'=>['$trans.driver_edit_status',0]],
                        'actual_paid_amt'=>['$ifNull'=>['$trans.actual_paid_amt','']],
                ...
                [
                    '$group' => [
                        ...
                        'total_pass_pend_amt' => ['$sum' => '$passenger_pending_amt'],
                       ...
                       //  'pickup_time' => array('$push'=>'$pickup_time'),
                        'total_amount' => ['$sum' => '$fare'],
'total_wallet'=>['$sum'=>'$wallet_amount_used'],
                        'total_pending_amt' => ['$sum' => '$pending_amt'],
                        'total_count' => ['$sum' => 1]
                    ]
                ],
...
// echo "<pre>";
        ...
        $field_arguments = [
                ...
                [
                    '$project' => [
                        ...
                        'create_day'=>['$dateToString' => ['format'=> "%d/%m/%Y", 'date'=> '$shift_start']]
                    ]
                ],
                [
                    '$group' => [
    ...
    public function passenger_sales_report($keyword = "", $status = "",$start_date = "",$end_date = "",$year,$month, $company = "", $offset = "", $val = "", $find_count = FALSE)
    {
        $user_createdby                  = $this->userid;
        $keyword       = str_replace("%", "!%", $keyword);
        $keyword       = str_replace("_", "!_", $keyword);
...
//        print_r($match_query);exit;
             ...
             'tfare'=>['$cond'=>[
                ...
                [
                    '$group' => [
                        ...
                        'total_amount' => ['$sum' => '$tfare'],
                        'total_pending_amt' => ['$sum' => '$pending_amt'],
                        'total_wallet_amt' => ['$sum' => '$wallet_amount_used'],
                        'total_count' => ['$sum' => 1]
                    ]
                ],
               ['$skip'   => (int)$offset ],
    ...
    public function sales_report($keyword = "", $status = "",$start_date,$end_date, $company = "",$offset = "", $val = "", $find_count = FALSE)
    {
       ...
       $match_query['actual_pickup_time'] = [
        ...
        if(!empty($keyword)) {
            $srch_query = [ "\$and" => [$match_query,["\$or"=>[['people.name' => new \MongoDB\BSON\Regex($keyword, 'i')] , [ 'people.lastname' => new \MongoDB\BSON\Regex($keyword, 'i') ], [ 'people.email' => new \MongoDB\BSON\Regex($keyword, 'i') ], [ 'people.phone' => new \MongoDB\BSON\Regex($keyword, 'i') ],['pass.name' => new \MongoDB\BSON\Regex($keyword, 'i')] , ] ] ] ];
        }else{
            $srch_query = $match_query;
        }
        $common_arguments = [
            ...
            // array(
            //     '$lookup' => array(
            //         'from' => MDB_PROMOCODE_CORPORATE_GROUP,
            //         'localField' => 'promocodeDetails.corporate_group',
            //         'foreignField' => '_id',
            //         'as' => 'corporateDetails'
            //     )
            // ),
            // array(
            //     '$unwind' =>array('path'=>'$corporateDetails','preserveNullAndEmptyArrays'=>true)
            // ),
                ...
                [
                    '$project' => [
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        ...
                        //'total_edited_fare'=>array('$sum'=>'$fare_detail.value'),
                        'createdby_username'=>['$ifNull'=>['$createdby_username','-']],
                        'wallet_amt_before_payment'=>['$ifNull'=>['$wallet_amt_before_payment','-']],
                        'payment_type'=>['$ifNull'=>['$trans.payment_type',0]],
                        'advance_payment'=>['$ifNull'=>['$trans.advance_payment',0]],
                        'pending_amt'=> ['$cond'=>[
                                        ['$gt'=>['$trans.driver_edit_status',0]],
                        ...
                        'current_pending_amt'=>['$ifNull'=>['$trans.pending_amt',0]],//Kumaresh
                        'pass_wallet_amt'=>['$ifNull'=>['$pass.wallet_amount',0]],//Kumaresh
                        'driver_edit_status'=>['$ifNull'=>['$trans.driver_edit_status',0]],
                        'actual_paid_amt'=>['$ifNull'=>['$trans.actual_paid_amt','']],
                        'promocode'=>['$ifNull'=>['$promocode','']],
                        'passenger_discount'=>['$ifNull'=>['$trans.passenger_discount','']],
                        'taxi_no'=>['$ifNull'=>['$taxi.taxi_no',0]],
                        'trip_cost'=>['$ifNull'=>['$trans.tripfare',0]], 

                        /* Sasidharan Nov 28 2022 */                        
                        'corporate_amount'=>['$ifNull'=>['$corporatePayment.trip_amount',0]],
                        'dispatcher_altered'=>['$ifNull'=>['$corporatePayment.dispatcher_altered',0]],
                        'tapAutoPayment'=>['$ifNull'=>['$tapAutoPayment',0]],
                        'corporate_name'=>['$ifNull'=>['$corporateDetails.group_name',"-"]],
...
//echo $start_date.'---'.$end_date;exit;
       ...
       $match_query['actual_pickup_time'] = [
        ...
        if ($find_count == TRUE) {
            $count_arguments = [
                ...
                [
                    '$group' => [
                         '_id' => ['year' => ['$year'=>'$actual_pickup_time'],'month'=>['$month'=>'$actual_pickup_time'],'day'=>['$dayOfMonth'=>'$actual_pickup_time']],
                        //'count' => array('$sum' => 1)
                    ]
                ],
            ];
                ...
                [
                    '$project' => [
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        ...
                        'actual_pickup_time'=>'$actual_pickup_time',
                        'create_day'=>['$dateToString' => ['format'=> "%Y-%m-%d ", 'date'=> '$actual_pickup_time']],
                 ...
                 ['$group' => [
                          '_id' => ['year' => ['$year'=>['$add'=>['$actual_pickup_time',3 * 60 * 60 * 1000]]],'month'=>['$month'=>['$add'=>['$actual_pickup_time',3 * 60 * 60 * 1000]]],'day'=>['$dayOfMonth'=>['$add'=>['$actual_pickup_time',3 * 60 * 60 * 1000]]]],
                          'create_day'=>['$first'=>'$create_day'],
                          'actual_pickup_time'=>['$first'=>'$actual_pickup_time'],
                          'tripdet' =>['$push'=>'$$ROOT']
                          ]],
                ['$skip'   => (int)$offset ],
               ['$limit'  => (int)$val ],
 ...
 //print_r($result);exit;
    ...
    public function get_shift_drivers($analytics_date){
        ...
        $shift_query['shift_start'] = ['$gte'=> new \MongoDB\BSON\UTCDateTime(strtotime($analytics_date." 00:00:00") * 1000)];
        $shift_query['shift_end'] = ['$lte' => new \MongoDB\BSON\UTCDateTime(strtotime($analytics_date." 24:59:59") * 1000)];
        $arguments = [
            ['$match' => $shift_query],
            ['$project' => ['driver_id'=>'$driver_id','shift_start'=>'$shift_start','shift_end'=>'$shift_end']]
        ];
        //echo "<pre>"; print_r($arguments);
        $result2    = $this->mongo_db->aggregate(DRIVERSHIFTSERVICE, $arguments);
        //print_r($result2['result'][0]);exit;
       return (!empty($result2['result'])) ? $result2['result'] : [];

    }

    public function get_trips_list($analytics_date){

        $match_query['createdate'] = ['$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($analytics_date." 00:00:00") * 1000),'$lte' => new \MongoDB\BSON\UTCDateTime(strtotime($analytics_date." 24:59:59") * 1000)];

        $arguments = [
            ['$match' => $match_query],
            ['$project' => ['_id'=>'$_id','createdate'=>'$createdate','travel_status'=>'$travel_status']]
        ];
        //echo "<pre>"; print_r($arguments);
        $result2    = $this->mongo_db->aggregate(MDB_PASSENGERS_LOGS, $arguments);
        //print_r($result2['result']);exit;
       return (!empty($result2['result'])) ? $result2['result'] : [];

    }

    public function passengers_unused_wallet_report($keyword = "", $status = "",$start_date,$end_date, $company = "",$offset = "", $val = "", $find_count = FALSE)
    {
        $user_createdby                  = $this->userid;
        $keyword       = str_replace("%", "!%", $keyword);
        $keyword       = str_replace("_", "!_", $keyword);
       ...
       $match_query = [
        ...
        if(!empty($keyword)) {
            $srch_query = [ "\$and" => [$match_query,["\$or"=>[['pass.name' => new \MongoDB\BSON\Regex($keyword, 'i')] , ] ] ] ];
        }else{
            $srch_query = $match_query;
        }
        $common_arguments = [
            ...
            // ),
            //  array(
            //     '$unwind' =>array('path'=>'$logs','preserveNullAndEmptyArrays'=>true)
            // ),
            [
                '$match' => $srch_query
            ],
            ...
            $field_arguments = [
                ...
                [
                    '$project' => [
                        ...
                        'recharge_date'=>'$created_date',
                        'recharge_amount'=>'$change_amount',
                        //'trip_id'=>'$logs._id',
                        'recharge_id'=>'$_id'                 
            ...
            foreach($result['result'] as $key=>$value){
                ...
                if($value['pass_id']){
                    $result = $this->mongo_db->find_one(MDB_PASSENGERSLOGS_COMPLETED, [
                        ...
                        'passengers_id' => (int) $value['pass_id']
                    ], [
                        '_id',
                    ]);
                    ...
                    if(empty($result)){
                        array_push($unused_array,$value);
                    }
                }
            }
            $unused = $this->unique_multidim_array($unused_array,'pass_phone');

            if ($find_count == TRUE) {
                return count($unused);
            } else {
                // $limit = $val; //per page    
                // $totalPages = ceil( count($unused)/ $limit ); //calculate total pages
                // $page = max($page, 1); //get 1 page when $_GET['page'] <= 0
                // $page = min($page, $totalPages); //get last page when $_GET['page'] > $totalPages
                // $offset = ($page - 1) * $limit;
                // if( $offset < 0 ) $offset = 0;
            return array_slice($unused,$offset,$val);
            }
       // }
    }
    ...
    public function get_trips_fare_details($count) {
        $start_date = "2022-07-01 00:00:00";
        $filter = [
            'travel_status' => 1,
            'fare_details_updated' => ['$exists' => false],
            'actual_pickup_time' => ['$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($start_date) * 1000)],
        ];
        $project = [
            "_id" => 1,
            "fare_detail" => 1,
        ...
        $res = $this->mongo_db->find(MDB_PASSENGERSLOGS_COMPLETED, $filter, $project)->limit($count);
        return (!empty($res))?iterator_to_array($res):[];
    }

    public function update_trips_fare_details($id, $fare_info) {
        $filter = [
            "_id" => $id,
            'travel_status' => 1,
        ];
        $updateArray = [
            "fare_detail" => $fare_info,
            "fare_details_updated" => (int)1 
        ];
        $updateResult = $this->mongo_db->update(MDB_PASSENGERSLOGS_COMPLETED, $filter, ['$set'=>$updateArray], ["upsert" => false]);
