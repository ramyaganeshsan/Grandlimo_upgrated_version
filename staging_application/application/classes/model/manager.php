...
Class Model_Manager extends Model
{
	...
	public function get_availabletaxi_list($find_count = false)
    {
		...
		if ($currentdate!="" && $enddate!="") {
			$match_query['mapping.mapping_startdate'] = ['$gte' => new \MongoDB\BSON\UTCDateTime(strtotime($currentdate) * 1000)];
			$match_query['mapping.mapping_enddate'] = ['$lt' => new \MongoDB\BSON\UTCDateTime(strtotime($enddate) * 1000)];
		}
		$arguments = [
            [
				'$lookup' => [
	...
	public function gettransaction($id)
	{
		...
		$arguments=[
			...
			[
				'$group' => [
					'_id' => ['driver_id' => '$driver_id','name' => '$people.name'],
					'count' => ['$sum' => 1]
				] 
			],
			[
	...
	public function changegettransaction($id = '',$startdate = '',$enddate = '')
	{
		...
		if($startdate!="" && $enddate!=""){
			$match_query['createdate'] = ['$gte'=>new \MongoDB\BSON\UTCDateTime(strtotime($startdate) * 1000),'$lte'=>new \MongoDB\BSON\UTCDateTime(strtotime($enddate) * 1000)];
		}
		$match_query['travel_status'] = 1;
		$arguments=[
			[
    ...
    {		
		...
		$arguments = [['$unwind' => '$stateinfo'],['$match'=> ['stateinfo.state_status' => 'A']],['$project' => ['id' 	=> '$stateinfo.state_id']],['$group' =>['_id' => NULL,'count' => ['$sum' => 1]]]];
		...
		$arguments = [['$unwind' => '$stateinfo'],['$unwind' => '$stateinfo.cityinfo'],['$match'=> ['stateinfo.cityinfo.city_status' => 'A']],['$project' => ['id' 	=> '$stateinfo.cityinfo.city_id']],['$group' =>['_id' => NULL,'count' => ['$sum' => 1]]]];
		...
		$result["taxi"] = $this->mongo_db->count(MDB_TAXI,['taxi_company' => (int)$company_id, 'taxi_status' => 'A'],['_id']);
        return $result;
    }
	...
	public function get_activeusers_list($company_id)
    {
		$arguments = [
			[
				'$match' => ['login_status' => 'A', 'passenger_cid' => (int)$company_id]
			],
			[
				'$project' => [
					'name' => '$name',
	...
	public function total_trip_details($start='',$end='')
	{
		...
		if($start!=''&& $end!=''){
			$match_query['plog.pickup_time'] = ['$gte'=>new \MongoDB\BSON\UTCDateTime(strtotime($start) * 1000),'$lte'=>new \MongoDB\BSON\UTCDateTime(strtotime($end) * 1000)];
		}
        ...
        $arguments = [
			...
			[
                '$project' => [
					'year' => [ '$substr' => [ '$plog.pickup_time', 0, 4 ] ],
                    'month' => [ '$substr' => [ '$plog.pickup_time', 5, 2 ] ],
                    'day' => [ '$substr'=> [ '$plog.pickup_time', 8, 2 ] ],
					'fare' => '$trans.fare',
                ]
            ],
