    ...
    public function get_all_booking_list_all($array)
    {
      ...
      $project_query = [      [
                '$project' => [
                    ...
                    'passenger_wallet_amount'=>['$ifNull'=>['$trans.passenger_wallet_amount',0]],
                ]
            ]];
    ...
    public function directdispatch($pass_logid)
    {
        ...
        if ($pass_logid != '') {
            ...
            if(count($booking_details)>0)
            {
                ...
                $driver_details    = $this->search_driver_location($latitude, $longitude, $miles, $no_passengers, $_REQUEST, $taxi_fare_km, $taxi_model, $taxi_type, $maximum_luggage, $cityname, $pass_logid, $company_id, $search_driver,$dispatch_type);
 ...
 public function search_upcoming_trips()
  ...
  $arguments = [[
    ...
    if($prev_pending_pay > $pending_pay)
    {
        ...
        if($add_amt != 0)
        $update_wallet = $this->update_wallet_without_log($post['pass_id'],abs($add_amt),$negative,$post['trip_id'],'6');
    }
    else
    {
        $add_amt= $prev_pending_pay-$pending_pay;
    ...
    {
    $update_trans_array = ['add_amt'=>(double)$post['add_amt'],'pending_amt'=>(double)$pending_pay,'driver_edit_status'=>(int)$driver_edit_status,'actual_paid_amt'=>$actual_paid_amt,'payment_type'=>(int)$payment_type,"dispatcher_fare_edit"=>(int)1];
    }
    else
    {
     $update_trans_array = ['add_amt'=>(double)$post['add_amt'],'pending_amt'=>(double)$pending_pay,'driver_edit_status'=>(int)$driver_edit_status];
    }
