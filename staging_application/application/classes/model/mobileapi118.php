...
/*    public function get_taxi_speed($taxi_id = "")
    ...
    public function getpromodetails($promo_code = "", $passenger_id = "",$passeger_number="")
    {
        ...
        $promo_fetch = $this->mongo_db->find_one(MDB_PASSENGERS_PROMO,array('promocode'=>$promo_code,'customer_number'=>$passeger_number),array("promocode","package","promo_used","promo_limit"));
        if(count($promo_fetch) <= 0) 
        {
            $promo_fetch = $this->mongo_db->find_one(MDB_PASSENGERS_PROMO,array('promocode'=>$promo_code,'promo_type' => "1"),array("promocode","package","promo_used","promo_limit"));
        }    
      ...
      public function checkpromocode($promo_code = "", $customer_number = "", $company_id = "",$passenger_id="")
    {
        ...
        if (count($promo_fetch) > 0) {
            ...
            if (strtotime($promo_start) > strtotime($current_time)) {
				...
				$promo_user_count = $this->mongo_db->count(MDB_PASSENGERS_LOGS,$match);
				if ($promo_user_count > 0 && $promo_user_count >= $promo_limit) {
					return 2;
				} else {
					return 1;
