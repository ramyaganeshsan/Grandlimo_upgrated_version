...
public function getAllInvoice($passenger,$pay_status,$offset = '', $val = '')
    ...
    public function driver_sales_report($keyword = "", $status = "",$start_date = "",$end_date = "",$year,$month, $company = "", $offset = "", $val = "", $find_count = FALSE)
    {
       ...
       // $current_time  = $get_company_time_details['current_time']; //Start time
        ...
        $match_query['pass.pickup_time'] = array(
            ...
            $field_arguments = array(
                ...
                array(
                    '$project' => array(
                       ...
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                        'pending_amt'=>array('$cond'=>array(
                            array('$eq'=>array('$trans.driver_edit_status',0)),
                            array('$ifNull'=>array('$trans.pending_amt',0)),
                            0,
                         )), 
                        'createdate'=>'$createdate',
            ...
            $field_arguments = array(              
                array(
                    '$project' => array(
                        ...
                        'cash_amt' =>array('$arrayElemAt'=> array('$fare_detail', 0 ) ),
                        'card_amt' =>array('$arrayElemAt'=> array('$fare_detail', 1 ) ),
                        'knet_amt' =>array('$arrayElemAt'=> array('$fare_detail', 2 ) ),
                        'add_amt' =>array('$arrayElemAt'=> array('$fare_detail', 3 ) ),
                        'pend_amt' =>array('$arrayElemAt'=> array('$fare_detail', 4 ) ),
                        'wallet_amt' =>array('$arrayElemAt'=> array('$fare_detail', 5 ) ),
                       // 'wallet_amount_used'=>'$trans.wallet_amount_used',
                ...
                array(
                    '$group' => array(
                       ...
                       //  'pickup_time' => array('$push'=>'$pickup_time'),
                        'total_amount' => array('$sum' => '$fare'),
                        'total_pending_amt' => array('$sum' => '$pending_amt'),
                        'total_cash_amt' => array('$sum' => '$cash_amt.value'),
                        'total_card_amt' => array('$sum' => '$card_amt.value'),
                        'total_knet_amt' => array('$sum' => '$knet_amt.value'),
                        'total_wallet_amt' => array('$sum' => '$wallet_amt.value'),
                        'total_add_amt' => array('$sum' => '$add_amt.value'),
                        'total_count' => array('$sum' => 1)
                    )
                ),
               //array('$skip'   => (int)$offset ),
...
//echo "<pre>";
  //          print_r($result);exit;
    ...
    public function consolidated_sales_report($keyword = "", $status = "",$start_date,$end_date, $company = "",$offset = "", $val = "", $find_count = FALSE)
    {
        $user_createdby                  = $this->userid;
        $keyword       = str_replace("%", "!%", $keyword);
        $keyword       = str_replace("_", "!_", $keyword);
...
// echo "<pre>";
    ...
    public function driver_shift_logs($keyword = "", $status = "",$start_date="",$end_date=""){
