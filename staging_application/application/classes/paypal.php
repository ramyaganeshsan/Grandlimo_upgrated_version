	...
	public static function instance($type , $config="")
	{
		if ( ! isset(PayPal::$instances[$type]))
		{
			...
			$class = 'PayPal_'.$type;
                        ...
                        //ndot check configuration in db ,if not exists allow to use file config
...
*/
			$error_msg = 'PayPal API request for '.$method.' failed: '.$error.'('.$code.')';				      
			//	$this->session->set('paypal_error', $error_msg);
		...
		//if we get mass error need to allow return data if it response failure also
		if($method == "MassPay"){
		...
		//
				...
				*/
                                ...
                                $error_msg = 'PayPal API request for '.$method.' failed: '.$data['L_LONGMESSAGE0'].'('.$data['L_ERRORCODE0'].')';				      
				//$this->session->set('paypal_error', $error_msg);	
														Message::error($error_msg);
